CREATE OR REPLACE PACKAGE PLM.pkg_outside_material IS
  ...
  PROCEDURE p_upd_outmaterial(v_spu_id      IN VARCHAR2,
                              ...
                              v_cateid      IN VARCHAR2 DEFAULT NULL, --适用品类
...
WHERE  A.QUOTATION_ID =  ']' || p_quotation_id ||
  ...
  BEGIN
    ...
    INSERT INTO mrp.mrp_create_outside_material_sku_temp
      (temp_id,
       ...
       create_id, create_time, function_tag_id, quo_id,APPLICABLE_CATEGORY)
    VALUES
      (v_rec.temp_id,
       --SPU_ID,
       ...
       3, v_rec.create_id, SYSDATE, v_rec.function_tag_id, v_quotation_id,v_rec.applicable_category);
  ...
  BEGIN
    ...
    UPDATE mrp.mrp_create_outside_material_sku_temp t
       SET t.material_classification       = v_rec.material_classification,
           ...
           t.applicable_category           = v_rec.applicable_category
  ...
  BEGIN
   ...
   --适用品类
    IF v_rec.applicable_category IS NOT NULL THEN
     /* DELETE FROM mrp.mrp_outside_material_spu_function_tag_mapping b
       WHERE b.material_spu = spu_id;*/
      IF instr(v_rec.applicable_category, ';') > 0 THEN
        FOR b IN (SELECT regexp_substr(v_rec.applicable_category, '[^;]+', 1, rownum) cate_id
                    FROM dual
                  CONNECT BY rownum <=
                             length(v_rec.applicable_category) -
                             length(REPLACE(v_rec.applicable_category, ';', '')) + 1) LOOP
        
          INSERT INTO mrp.MRP_MATERIAL_SPU_APPLICABLE_CATEGORY_MAPPING
            (id, material_spu,
          ...
          VALUES
            (f_get_mrp_keyid(pi_pre     => 1,
                             pi_owner   => 'PLM',
                             pi_seqname => 'SEQ_MRPPIC_ID',
                             pi_seqnum  => 99), spu_id, b.cate_id);
        END LOOP;
      ELSE
        INSERT INTO mrp.MRP_MATERIAL_SPU_APPLICABLE_CATEGORY_MAPPING
          (id, material_spu,APPLICABLE_CATEGORY_ID)
        VALUES
          (f_get_mrp_keyid(pi_pre     => 1,
                           pi_owner   => 'PLM',
                           pi_seqname => 'SEQ_MRPPIC_ID',
                           pi_seqnum  => 99), spu_id, v_rec.applicable_category);
      END IF;
    ELSE
      NULL;
    END IF;
...
SELECT ''' || v_quo_material_id ||
 ...
 INNER JOIN MRP.MRP_TEMPORARY_SUPPLIER_ARCHIVES D
  ...
  LEFT JOIN (SELECT MC.MATERIAL_SPU,LISTAGG(MC.APPLICABLE_CATEGORY_ID,'';'') WITHIN GROUP(ORDER BY 1) APPLICABLE_CATEGORY
              FROM MRP.MRP_MATERIAL_SPU_APPLICABLE_CATEGORY_MAPPING MC
              GROUP BY MC.MATERIAL_SPU) MC ON MC.MATERIAL_SPU= A.MATERIAL_SPU  
 ...
 /* LEFT JOIN (SELECT T6.MATERIAL_SPU,
    ...
    ON FT.MATERIAL_SPU = A.MATERIAL_SPU*/
      LEFT JOIN (SELECT T6.MATERIAL_SPU,LISTAGG(t6.material_function_tag_id, '';'') WITHIN GROUP(ORDER BY T6.MATERIAL_SPU) FUNCTION_TAG,
                    LISTAGG(T7.COMPANY_DICT_NAME, '';'') WITHIN GROUP(ORDER BY T6.MATERIAL_SPU) FUNCTION_ID
               FROM MRP.MRP_OUTSIDE_MATERIAL_SPU_FUNCTION_TAG_MAPPING T6
              INNER JOIN SCMDATA.SYS_COMPANY_DICT T7
                 ON T6.MATERIAL_FUNCTION_TAG_ID = T7.COMPANY_DICT_ID
              GROUP BY T6.MATERIAL_SPU) FT
    ON FT.MATERIAL_SPU = A.MATERIAL_SPU
 ...
 WHERE B.MATERIAL_SKU =(SELECT t.quotation_material_sku
  ...
  BEGIN
    ...
    UPDATE mrp.mrp_outside_material_spu a
     ...
     --【物料SPU】+【创建成品供应商编号】入参，在【外部物料SPU信息表】中找出【物料名称】，将【表单编辑页-物料名称】覆盖到【外部物料SPU信息表-物料名称】中
    
     UPDATE mrp.mrp_outside_material_spu a
        SET  a.material_name           = v_ma_name
      WHERE a.material_spu  = v_spu_id
        AND a.create_finished_supplier_code=v_supp_id;
    
    
    --功能标签
    IF v_func_tag IS NOT NULL THEN
      DELETE FROM mrp.mrp_outside_material_spu_function_tag_mapping b  WHERE b.material_spu = v_spu_id;
    ...
    --适用品类
    IF v_cateid IS NOT NULL THEN
      DELETE FROM mrp.MRP_MATERIAL_SPU_APPLICABLE_CATEGORY_MAPPING b WHERE b.material_spu = v_spu_id;
      IF instr(v_cateid, ';') > 0 THEN
        FOR b IN (SELECT regexp_substr(v_cateid, '[^;]+', 1, rownum) cate_id
                    FROM dual
                  CONNECT BY rownum <=
                             length(v_cateid) -
                             length(REPLACE(v_cateid, ';', '')) + 1) LOOP
          ...
          VALUES
            (f_get_mrp_keyid(pi_pre     => 1,
                             pi_owner   => 'PLM',
                             pi_seqname => 'SEQ_MRPPIC_ID',
                             pi_seqnum  => 99), v_spu_id, b.cate_id);
        END LOOP;
      ELSE
        INSERT INTO mrp.MRP_MATERIAL_SPU_APPLICABLE_CATEGORY_MAPPING
          (id, material_spu,APPLICABLE_CATEGORY_ID)
        VALUES
          (f_get_mrp_keyid(pi_pre     => 1,
                           pi_owner   => 'PLM',
                           pi_seqname => 'SEQ_MRPPIC_ID',
                           pi_seqnum  => 99), v_spu_id, v_cateid);
      END IF;
    ELSE
      NULL;
    END IF;
    ...
    UPDATE mrp.mrp_outside_supplier_material d
     ...
     WHERE d.supplier_code = v_supp_code
       ...
       UPDATE mrp.mrp_outside_supplier_material d
          SET d.supplier_material_name        = v_ma_name
        WHERE D.MATERIAL_SKU IN (SELECT U.MATERIAL_SKU
                                   FROM mrp.mrp_outside_material_sku U WHERE U.MATERIAL_SPU = v_spu_id
                                    AND U.CREATE_FINISHED_SUPPLIER_CODE = v_supp_id )  
          AND D.CREATE_FINISHED_SUPPLIER_CODE  = v_supp_id ;                   
  ...
  BEGIN
    ...
    IF v_flag2 > 0 /*AND v_rec.material_sku IS NOT NULL*/ THEN
      raise_application_error(-20002,
  ...
  BEGIN
    ...
    FOR i IN (SELECT *
      ...
      IF v_flag > 0 /*AND i.material_sku IS NOT NULL*/ THEN
        raise_application_error(-20002,
                                '已存在 ' || v_exist_sku ||
                                '对应当前物料，请前往选择已有面料进行操作');
      ELSE
