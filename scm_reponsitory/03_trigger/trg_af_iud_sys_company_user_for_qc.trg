...
/* elsif updating('company_user_name') and :new.pause=0 then
    select nvl(max(1),0) into p_flag from scmdata.sys_company_user_dept cud inner join scmdata.sys_company_dept cd on cd.company_dept_id=cud.company_dept_id where cud.company_id=:new.company_id and cud.user_id=:new.user_id
    and cd.dept_name like '%QC%';
    if p_flag=1 then 
      update scmdata.t_qc*/
  end if;
