CREATE OR REPLACE PACKAGE SCMDATA.pkg_report_analy IS
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_report_analy IS
  --订单满足率-异常分布 a_report_abn_202
  FUNCTION f_get_abn_distribut_report(p_company_id       VARCHAR2,
                                      p_class_data_privs CLOB,
                                      p_abn_year         VARCHAR2,
  ...
  BEGIN
    ...
    CASE
      ...
      ELSE
        v_where := v_where || ' AND ga.group_config_id = ''' || v_group ||
                   ''' ';
    END CASE;
    --展示维度
    --group sql
    v_query_filed_a := q'[ '合计' group_name, null category_name,]';
    IF (v_cate = '1' AND v_group = '1') THEN
    ...
    ELSE
      ...
      ELSIF v_cate <> '1' AND v_group = '1' THEN
    ...
    v_sql := q'[ SELECT ]' || v_query_filed || q'[
       ...
       SUM(v.order_money)/ MAX(v.order_sum_money) order_sum_propotion,
       SUM(v.abn_money) / SUM(v.order_money) abn_money_propotion
  FROM (SELECT ga.group_name group_name,
               ga.group_config_id,
               t.category,
               t.category_name,
               t.samll_category,
               ...
               t.satisfy_money / ]' || v_rc || q'[ satisfy_money,
               t.order_money / ]' || v_rc || q'[ order_money,
               t.delivery_date,
               t.company_id,
               (t.order_money - t.satisfy_money)/]' || v_rc || q'[ abn_money,
               SUM((t.order_money - t.satisfy_money) /]' || v_rc || q'[)  over() abn_sum_money,
               SUM(t.order_money /]' || v_rc || q'[)  over() order_sum_money
          FROM scmdata.pt_ordered t
           LEFT JOIN scmdata.t_supplier_group_config ga
          ON ga.group_config_id = t.group_name
    ...
    --合计
        v_union_sql := 'SELECT * FROM (' || 'SELECT * FROM (' || v_sql ||
         ...
         SUM(order_money)/ SUM(order_money) order_sum_propotion,
  ...
  BEGIN
    ...
    FROM (SELECT ga.group_name group_name,
           ...
           LEFT JOIN scmdata.t_supplier_group_config ga
          ON ga.group_config_id = t.group_name ]' || v_qw ||
  ...
  BEGIN
    ...
    v_where_a := q'[ WHERE trunc(gd.create_time) BETWEEN  ']' ||
                 ...
                 q'[' and gs.amount > = 0 ]';
    --数据权限            
    v_data_privs := f_get_dataprivs_sql(p_class_data_privs => p_class_data_privs,
                                        p_pre              => 'zv');
...
select distinct nvl(zv.category_name, gsv.category_name) category_name,
                ...
                nvl(gsv.amt_price / sum(gsv.amt_price) over(), 0) AMT_MONEY_PROPOTION,
                nvl(gsv.amt_price, 0) amt_price,
                nvl(zv.abn_money / gsv.amt_price, 0) ABN_AMT_MONEY_PROPOTION,
   ...
   AND gsv.company_id = zv.company_id
      ...
      v_union_sql := 'SELECT * FROM (SELECT * FROM (' || v_sql ||
                     q'[) ORDER BY abn_money desc) union all SELECT '合计' category_name,'' product_subclass_name ,nvl(sum(abn_money)/decode(sum(abn_money),0,null,sum(abn_money)),0) abn_sum_propotion,
                      sum(abn_money) abn_money,nvl(sum(amt_price)/decode(sum(amt_price),0,null,sum(amt_price)),0) AMT_MONEY_PROPOTION,sum(amt_price) amt_price, nvl(sum(abn_money)/decode(sum(amt_price),0,null,sum(amt_price)),0) ABN_AMT_MONEY_PROPOTION,
...
select distinct nvl(zv.category_name, gsv.category_name) category_name,
  ...
  FULL JOIN (SELECT distinct sum(gs.amount * tc.price) over(partition by tc.category_name,  gd.supplier_code) amt_price,
   ...
   AND gsv.category = zv.category
   AND gsv.company_id = zv.company_id ]' || v_data_privs;
  ...
  BEGIN
    ...
    v_sql := q'[ SELECT ]' || v_query_filed || q'[,
           ...
           gv.owe_amount,
           gv.owe_price,
           nvl((gv.owe_price / sum(gv.owe_price) over()),0)  owe_price_prom,
           gv.act_owe_goo_cnt,
           gv.act_owe_amount,
           gv.act_owe_price,
    ...
    v_union_sql := 'SELECT * FROM (' || 'SELECT * FROM (' || v_sql ||
                   ...
                   q'[,SUM(sup_cnt) sup_cnt,
                      ...
                      SUM(owe_price)/SUM(owe_price)  owe_price_prom,
  ...
  BEGIN
    ...
    v_union_sql := 'SELECT * FROM (SELECT * FROM (' || 'SELECT * FROM (' ||
                   v_sql || ') ORDER BY owe_amount DESC) ' || q'[ UNION ALL SELECT DISTINCT NULL category,
                                       ...
                                       node_num
      FROM (]' || v_sql ||
                   ')uv) ORDER BY owe_amount desc NULLS LAST,node_num ASC';
    RETURN v_union_sql;
