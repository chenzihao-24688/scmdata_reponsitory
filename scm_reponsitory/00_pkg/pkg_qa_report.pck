CREATE OR REPLACE PACKAGE SCMDATA.PKG_QA_REPORT IS
  PROCEDURE P_T_QA_WAREHOUSE_INSPECT_FAILED;
  PROCEDURE P_T_QA_INSPECT_FAILED_FLW_QC;
  PROCEDURE P_T_QA_INGOOD;
  --获取数据权限sql
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.PKG_QA_REPORT IS
  PROCEDURE P_T_QA_WAREHOUSE_INSPECT_FAILED IS
  BEGIN
    MERGE INTO scmdata.t_qa_warehouse_inspect_failed tka
    USING (SELECT reprela.sho_id,
                  rep.company_id,
                  ...
                  NVL(probclass.cause_detail, rep.problem_classification) problem_classification, --问题分类
            ...
            inner join (select kt1.qa_report_id,
                              ...
                              listagg(distinct kt1.if_batch_sporadic, ';') if_batch_sporadic,
                              listagg(distinct kt1.skuprocessing_result, ';') skuprocessing_result,
                              pt.flw_order,
                         ...
                         from (select repsku.qa_report_id,
                                      ...
                                      repsku.asn_id,
                                      asned.order_id,
                                      listagg(distinct rest.group_dict_name,
                                ...
                                inner join scmdata.t_asnordered asned
                                 ...
                                 LEFT JOIN scmdata.sys_group_dict rest
                                   ON rest.group_dict_type = 'QA_PROCESSRESULT'
                                ...
                                group by repsku.qa_report_id,
                                         ...
                                         repsku.asn_id
                               union all
                               select qa_report_id,company_id,asn_id,order_id, 
                                     listagg(distinct group_dict_name, ';') skuprocessing_result,sum(qualdecrease_amount) qualdecrease_amount,if_batch_sporadic
                                from (select repsku.qa_report_id,
                                             repsku.company_id,
                                             repsku.asn_id,
                                             asned.order_id,
                                             ...
                                             (case when tdr.asn_documents_status in ('已关闭') then tdr.delivery_amount
                                               else repsku.unqual_amount end) qualdecrease_amount,
                                       ...
                                       inner join scmdata.t_asnordered asned
                                        ...
                                        LEFT JOIN scmdata.sys_group_dict rest
                                          ON rest.group_dict_type = 'QA_PROCESSRESULT'
                                         AND repsku.skuprocessing_result = rest.group_dict_value
                                       where repsku.status = 'AF'
                                         and repsku.skucheck_result = 'NP'
                                         and repsku.skuprocessing_result in ('DSR', 'AWR', 'WD'))
                               group by qa_report_id, company_id, order_id, asn_id,if_batch_sporadic ) kt1
                         left join (select p.product_gress_code,
                                          p.company_id,
                                          p.flw_order,
                                          p.flw_order_manager,
                                          p.qc,
                                    ...
                                    where p.qc is not null) pt
                           on pt.product_gress_code = kt1.order_id
                          and kt1.company_id = pt.company_id
    ...
    when matched then
      update
         set tka.qualfinishtime         = tkb.qualfinishtime,
             ...
             tka.group_name            =
             (case
               when tkb.group_name is null then
                '1'
               else
                tkb.group_name
             end),
             tka.flw_order             =
             (case
               when tkb.flw_order is null then
                '1'
               else
                tkb.flw_order
             end),
             tka.flw_order_manager     =
             (case
               when tkb.flw_order_manager is null then
                '1'
               else
                tkb.flw_order_manager
             end),
             tka.qc                    =
             (case
               when tkb.qc is null then
                '1'
               else
                tkb.qc
             end),
             tka.qc_manager            =
             (case
               when tkb.qc_manager is null then
                '1'
               else
                tkb.qc_manager
             end),
             ...
             tka.update_time            = sysdate
              ...
              WHEN NOT MATCHED THEN INSERT(tka.qa_warehouse_inspect_failed_id, tka.company_id, tka.qualfinishtime, tka.sho_id, tka.rela_goo_id, tka.style_number, tka.style_name, tka.category_id, tka.product_cate, tka.samll_category, tka.price, tka.supplier_code, tka.inside_supplier_code, tka.factory_code, tka.qualdecrease_amount, tka.if_batch_sporadic, tka.problem_classification, tka.problem_descriptions, tka.review_comments, tka.memo, tka.skuprocessing_result, tka.group_name, tka.flw_order, tka.flw_order_manager, tka.qc, tka.qc_manager, tka.is_need_qc_quality, tka.qa_report_id, tka.order_id, tka.asn_id, tka.create_id, tka.create_time, tka.update_id, tka.update_time) VALUES(scmdata.f_get_uuid(), tkb.company_id, tkb.qualfinishtime, tkb.sho_id, tkb.rela_goo_id, tkb.style_number, tkb.style_name, tkb.category_id, tkb.product_cate, tkb.samll_category, tkb.price, tkb.supplier_code, tkb.inside_supplier_code, tkb.factory_code, tkb.qualdecrease_amount, tkb.if_batch_sporadic, tkb.problem_classification, tkb.problem_descriptions, tkb.review_comments, tkb.memo, tkb.skuprocessing_result, tkb.group_name, tkb.flw_order, tkb.flw_order_manager, tkb.qc, tkb.qc_manager, tkb.is_need_qc_quality, tkb.qa_report_id, tkb.order_id, tkb.asn_id, 'ADMIN', sysdate, 'ADMIN', sysdate);
    ---更新时间
    begin
      merge into scmdata.t_qa_warehouse_inspect_failed tka
 ...
 v_sql_w := q'[   merge into scmdata.t_qa_ingood tka
    ...
    begin
      ...
      using (select company_id, create_time,category_id,group_id, statistical_dimension,
               ...
               from (select pt.company_id, pt.product_gress_code,
                            ...
                            regexp_substr(pt.flw_order, '[^,]+', 1, level) flw_order,
                      ...
                      inner join (select t.document_no, t.company_id, trunc(t.create_time) create_time, sum(tl.amount) amount, tl.goo_id
                       ...
                       left join scmdata.t_commodity_info goo
                         on p1.goo_id = goo.goo_id
                        and p1.company_id = goo.company_id
                     connect by prior pt.pt_ordered_id = pt.pt_ordered_id
                            and level <= length(pt.flw_order) - length(regexp_replace(pt.flw_order, ',', '')) + 1
                            and prior dbms_random.value is not null
                            and pt.qc is not null)
     ...
     v_sql_w := q'[      merge into scmdata.t_qa_ingood tka
      using (select company_id,create_time,category_id,group_id, statistical_dimension,
               ...
               from (select pt.company_id,pt.category category_id,(case when pt.group_name is null then '1' else pt.group_name end) group_id ,
                            ...
                            goo.price,
                            p1.amount,
                            regexp_substr(pt.flw_order_manager, '[^,]+',  1, level) flw_order_manager,
                            (p1.amount / count(p1.document_no) over(partition by p1.document_no, p1.create_time)) sum1
                       from scmdata.pt_ordered pt
                      inner join (select t.document_no, t.company_id, trunc(t.create_time) create_time, sum(tl.amount) amount, tl.goo_id
                                   from scmdata.t_ingood t
                     ...
                     connect by prior pt.pt_ordered_id = pt.pt_ordered_id
                            and level <=
                                length(pt.flw_order_manager) -
                                length(regexp_replace(pt.flw_order_manager,  ',',  '')) + 1
                            and prior dbms_random.value is not null
                            and pt.qc is not null)
    ...
    begin
      ...
      using (select company_id, create_time,category_id,group_id, statistical_dimension, qc sort_dimension,
               ...
               from (select pt.company_id,pt.category category_id,(case when pt.group_name is null then '1' else pt.group_name end) group_id ,
                            ...
                            goo.price,
                            p1.amount,
                            regexp_substr(pt.qc, '[^,]+', 1, level) qc,
                            (p1.amount / count(p1.document_no) over(partition by p1.document_no, p1.create_time)) sum1
                       from scmdata.pt_ordered pt
                      inner join (select t.document_no, t.company_id, trunc(t.create_time) create_time, sum(tl.amount) amount, tl.goo_id
                                   from scmdata.t_ingood t
                       ...
                       left join scmdata.t_commodity_info goo
                        ...
                        and p1.company_id = goo.company_id
                     connect by prior pt.pt_ordered_id = pt.pt_ordered_id
                            and level <=length(pt.qc) - length(regexp_replace(pt.qc, ',', '')) + 1
                            and prior dbms_random.value is not null
                            and pt.qc is not null)
    ...
    begin
      ...
      using (select company_id,create_time,category_id,group_id, statistical_dimension,
               ...
               from (select pt.company_id,pt.category category_id,(case when pt.group_name is null then '1' else pt.group_name end) group_id ,
                            ...
                            goo.price,
                            p1.amount,
                            regexp_substr(pt.qc_manager, '[^,]+', 1, level) qc_manager,
                            (p1.amount / count(p1.document_no) over(partition by p1.document_no, p1.create_time)) sum1
                       from scmdata.pt_ordered pt
                      inner join (select t.document_no, t.company_id, trunc(t.create_time) create_time, sum(tl.amount) amount, tl.goo_id
                                   from scmdata.t_ingood t
                       ...
                       left join scmdata.t_commodity_info goo
                        ...
                        and p1.company_id = goo.company_id
                     connect by prior pt.pt_ordered_id = pt.pt_ordered_id
                            and level <= length(pt.qc_manager) - length(regexp_replace(pt.qc_manager, ',', '')) + 1
                            and prior dbms_random.value is not null
                            and pt.qc is not null)
              group by company_id,
                       create_time,
  ...
  BEGIN
    ...
    v2_dimension := '''{"COL_1":' || K_TIME || ',"COL_2":' || KPI_GROUP ||
                    ',"COL_3":' || KPI_CATEGORY || ' ,"COL_4":' ||
                    KPI_DIMENSION || ' ,"COL_5":''' || '|| td.sort_dimension' ||
                    '||'' }''';
...
/*      v_c_g        := ' tsup.supplier_code,tsup.supplier_company_name, ';*/
      v_b_g1       := ' t1.inside_supplier_code, ';
    ...
    CASE
      ...
      ELSE
        ...
        v2_b     := ' and  t1.category_id  =  '''|| KPI_CATEGORY || ''' ';
    END CASE;
    --过滤条件：区域组
    CASE
      ...
      ELSE
        ...
        v2_b     :=  v2_b ||' and  t1.group_name  =  '''|| KPI_GROUP || ''' ';
    END CASE;
    ---汇总字段条件
    if K_TIME = '本月' then
      K_TIME := to_char(sysdate, 'yyyy-mm');
    ...
    IF KPI_TIMETYPE = '本月' then
      ...
      v1_where  := v1_where || ' and td.year || ''-'' || lpad(td.month,2,0) = ''' ||  K_TIME || '''';
      ...
      v_c       := q'[ t1.year, t1.month , ]';
      v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'), to_char(t.purchase_time,'mm') ,  to_char(t.purchase_time,'yyyy-mm'),]';
      v_c_w     := '  and tp.year = td.year  and tp.month = td.month  ';
    elsif KPI_TIMETYPE = '月度' then
      v_z       := q'[ to_char(t.purchase_time,'yyyy') || '年' || to_char(t.purchase_time,'mm') ||'月'  total_time ,]';
      v1_where  := v1_where || ' and td.year || ''年'' || lpad(td.month,2,0) || ''月'' = ''' || kpi_time || '''';
      v_b_s     := q'[ on tq1.year = tq.year  and  tq1.month = tq.month ]';
      --v_c       := q'[ to_char(t.purchase_time,'yyyy')year, to_char(t.purchase_time,'mm') month, ]';
      v_c       := q'[ t1.year, t1.month, ]';
      v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'), to_char(t.purchase_time,'mm') , ]';
      v_c_w     := '  and tp.year = td.year  and tp.month = td.month  ';
    elsif KPI_TIMETYPE = '季度' then
      v_z       := q'[ to_char(t.purchase_time,'yyyy') || '年第' || to_char(t.purchase_time,'Q')||'季度'  total_time ,]';
     ...
     -- v_c       := q'[ to_char(t.purchase_time,'yyyy')year , to_char(t.purchase_time,'Q')quarter , ]';
      v_c       := q'[ t1.year , t1.quarter , ]';
      v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'), to_char(t.purchase_time,'Q') , ]';
      v_c_w     := '    and tp.year = td.year  and tp.quarter = td.quarter  ';
    elsif KPI_TIMETYPE = '半年度' then
      v_z       := q'[ to_char(t.purchase_time,'yyyy') || '年' || decode(to_char(t.purchase_time,'Q'),1,'上',2,'上',3,'下',4,'下') ||'半年'  total_time ,]';
      v1_where  := v1_where || ' and td.total_time = ''' || kpi_time || '''';
      ...
      v_c       := q'[ t1.year, t1.halfyear, ]';      
      v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'),decode(to_char(t.purchase_time,'Q'),1,'上',2,'上',3,'下',4,'下'), decode(to_char(t.purchase_time,'Q'),1,1,2,1,3,2,4,2) , ]';
      v_c_w     := '  and tp.year = td.year  and tp.halfyear = td.halfyear  ';
    
    elsif KPI_TIMETYPE = '年度' then
      v_z       := q'[ to_char(t.purchase_time,'yyyy') || '年'  total_time ,]';
      v1_where  := v1_where || ' and td.year || ''年'' = ''' || kpi_time || '''';
      v_b_s     := q'[ on tq1.year = tq.year  ]';
      --v_c       := q'[ to_char(t.purchase_time,'yyyy') year, ]';
      v_c       := q'[ t1.year, ]';
      v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'),]';
...
left join ( select tq.company_id,
 ...
 inner join (select t1.company_id, ]'|| v_c ||  q'[ sum(t1.qualdecrease_money) qualdecrease_money
               from scmdata.t_qa_warehouse_inspect_failed t1
              where t1.qc is not null ]' || v2_b ||'
              group by  '|| v_c  ||' t1.company_id  ) tq1 '|| v_b_s ||
q'[ where tq.qc is not null 
) tp 
    ...
    v1_sql := q'[ 
left join ( select tq.company_id,
       tq.order_id,trunc(tq.qualfinishtime)qualfinishtime,
       tq.year,
       tq.month,
       tq.quarter,
       tq.halfyear,
       ...
       tq1.qualdecrease_money 
  from scmdata.t_qa_warehouse_inspect_failed tq
 inner join (select t1.company_id, ]'|| v_c || v_b_g1 || q'[ sum(t1.qualdecrease_money) qualdecrease_money
               from scmdata.t_qa_warehouse_inspect_failed t1
              where t1.qc is not null ]' || v2_b ||'
              group by  '|| v_c || v_b_g1 ||' t1.company_id  ) tq1 '|| v_b_s ||v_b_g2||
q'[ where tq.qc is not null 
) tp 
...
left join (select tq.company_id,
       ...
       tq.sort_dimension,
       tq1.qualdecrease_money
  from scmdata.t_qa_inspect_failed_flw_qc tq
 inner join (select t1.company_id, ]'|| v_c  || q'[t1.statistical_dimension, sum(t1.qualdecrease_money ) qualdecrease_money 
               from scmdata.t_qa_inspect_failed_flw_qc t1
              where 1 = 1]' || v2_b ||'
              group by  '|| v_c  ||' t1.statistical_dimension, t1.company_id  ) tq1 '|| v_b_s ||
q'[    and tq1.statistical_dimension = tq.statistical_dimension
 ) tp 
    on td.company_id = tp.company_id ]'|| v_c_w || v_c_i1;
...
left join (select tq.company_id,
       ...
       tq1.qualdecrease_money
  from scmdata.t_qa_inspect_failed_flw_qc tq
 inner join (select t1.company_id, ]'|| v_c  || q'[t1.statistical_dimension,t1.sort_dimension, sum(t1.qualdecrease_money ) qualdecrease_money 
               from scmdata.t_qa_inspect_failed_flw_qc t1
              where 1 = 1]' || v2_b ||'
              group by  '|| v_c  ||' t1.statistical_dimension,t1.sort_dimension, t1.company_id  ) tq1 '|| v_b_s ||
q'[    and tq1.statistical_dimension = tq.statistical_dimension
       and tq1.sort_dimension = tq.sort_dimension 
 ) tp 
    on td.company_id = tp.company_id ]'|| v_c_w || v_c_i;


 v_c_sql := ' select distinct t1.company_id,t1.total_time, t1.category_id,t1.category_name,t1.group_id,t1.group_name,'|| v_c || v_c_k ||' t1.statistical_dimension,t1.sort_dimension,t2.purchase_money from  ( select t.company_id, ' || v_z ||
               q'[ to_char(t.purchase_time, 'yyyy') year,
               to_char(t.purchase_time, 'mm') month,
               to_char(t.purchase_time, 'Q') quarter,
               decode(to_char(t.purchase_time,'Q'),1,1,2,1,3,2,4,2) halfyear, 
               ...
               t.category_id,
               (case when ts.group_name is null then '1' else ts.group_name end) group_name,
               t.group_id,
       ...
       where t.company_id = ']' || COMPANY_ID || '''' ||  v_c_where;

  v_h2 :=  ' )t1  inner join (select '|| v_z || q'[
                    t.statistical_dimension,
                    t.sort_dimension,
                    sum(purchase_money) purchase_money
               from scmdata.t_qa_ingood t  where t.company_id = ']' || COMPANY_ID || '''' ||v1_b || v_c_where ||v_c_group || q'[
  ...
  BEGIN
    ...
    CASE
      ...
      ELSE
        ...
        v1_b     := ' and category_id  =  '''|| V_CATEGORY || ''' ';
    ...
    CASE
      ...
      ELSE
        ...
        v1_b     := v1_b || ' and group_name  =  '''|| V_GROUP || ''' ';
    ...
    IF SUBSTR(V_TIME, 5, 1) = '-' THEN
      ...
      V_S        := 'to_char(t.purchase_time,''yyyy'') || ''年'' || to_char(t.purchase_time,''mm'') || ''月'' total_time,';
      v_c_w      := '  and tp.year = td.year  and tp.month = td.month  ';
      v_s1       := q'[ to_char(t.purchase_time,'yyyy')year, to_char(t.purchase_time,'mm')month , ]';
      v_s2       := ' to_char(t.purchase_time,''yyyy'') || ''年'' || to_char(t.purchase_time,''mm'') || ''月'' ';
      v_s3       := ' t1.year, t1.month, ';
    ...
    ELSE
      IF SUBSTR(V_TIME, -1) = '月' THEN
        ...
        V_S       := ' to_char(t.purchase_time,''yyyy'')||to_char(t.purchase_time,''mm'') total_date,(to_char(t.purchase_time,''yyyy'') || ''年'' || to_char(t.purchase_time,''mm'') || ''月'') total_time,';
        v_c_w     := '  and tp.year = td.year  and tp.month = td.month  ';
        v_s1      := q'[ to_char(t.purchase_time,'yyyy')year, to_char(t.purchase_time,'mm') month, ]';
        v_s2      := ' to_char(t.purchase_time,''yyyy'') || ''年'' || to_char(t.purchase_time,''mm'') || ''月'' ';
        v_s3       := ' t1.year, t1.month, ';
      ...
      ELSIF SUBSTR(V_TIME, -2) = '季度' THEN
        ...
        V_S       := ' to_char(t.purchase_time,''yyyy'')|| to_char(t.purchase_time,''Q'') total_date,(to_char(t.purchase_time,''yyyy'') || ''年第'' || to_char(t.purchase_time,''Q'') || ''季度'') total_time,';
        v_c_w     := '    and tp.year = td.year  and tp.quarter = td.quarter  ';
        v_s1      := q'[ to_char(t.purchase_time,'yyyy')year , to_char(t.purchase_time,'Q')quarter , ]';
        v_s2      := ' to_char(t.purchase_time,''yyyy'') || ''年第'' || to_char(t.purchase_time,''Q'') || ''季度'' ';
        v_s3       := ' t1.year, t1.quarter, ';
      ...
      ELSIF SUBSTR(V_TIME, -2) = '半年' THEN
        ...
        V_S       := ' to_char(t.purchase_time,''yyyy'')||decode(to_char(t.purchase_time,''Q''),1,1,2,1,3,2,4,2) total_date,(to_char(t.purchase_time,''yyyy'') || ''年'' ||  decode(to_char(t.purchase_time,''Q''),1,''上'',2,''上'',3,''下'',4,''下'') || ''半年'') total_time,';
        v_c_w     := '  and tp.year = td.year  and tp.halfyear = td.halfyear  ';
        v_s1      := q'[ to_char(t.purchase_time,'yyyy')year,decode(to_char(t.purchase_time,'Q'),1,1,2,1,3,2,4,2) halfyear, ]';
        v_s2      := ' to_char(t.purchase_time,''yyyy'') || ''年'' ||  decode(to_char(t.purchase_time,''Q''),1,''上'',2,''上'',3,''下'',4,''下'') || ''半年'' ';
        v_s3       := ' t1.year, t1.halfyear, ';
        v_s4       :=  ' on tq1.year = tq.year  and tq1.halfyear = tq.halfyear ';
        v_c_group := q'[ group by  to_char(t.purchase_time,'yyyy'), decode(to_char(t.purchase_time,'Q'),1,1,2,1,3,2,4,2) , ]';
      ELSIF LENGTH(V_TIME) = '5' THEN
        V_C       := SCMDATA.PKG_KPIPT_ORDER.F_KPI_YEAR(TOTAL_TIME => V_TIME);
        V_S       := ' to_char(t.purchase_time,''yyyy'') total_date,(to_char(t.purchase_time,''yyyy'') || ''年'') total_time,';
        v_c_w     := '  and tp.year = td.year ';
        v_s1      := q'[ to_char(t.purchase_time,'yyyy') year, ]';
        v_s2      := ' to_char(t.purchase_time,''yyyy'') || ''年'' ';
        v_s3       := ' t1.year, ';
...
left join ( select tq.company_id,
       ...
       tq1.qualdecrease_money
  from scmdata.t_qa_warehouse_inspect_failed tq
 inner join (select t1.company_id, '|| v_s3 || '
                   sum(t1.qualdecrease_money) qualdecrease_money
              from scmdata.t_qa_warehouse_inspect_failed t1
             where t1.qc is not null ' || v1_b ||'
             group by ' || v_s3 || ' t1.company_id ) tq1 '|| v_s4 ||
' where tq.qc is not null ) tp
  on td.company_id = tp.company_id 
 /*and tp.category_id = td.category_id
 and tp.group_id = td.group_id */'|| v_c_w || v_c_i1;
...
left join ( select tq.company_id,
       ...
       tq1.qualdecrease_money
  from scmdata.t_qa_warehouse_inspect_failed tq
 inner join (select t1.company_id, '|| v_s3 || v_b_g1|| '
                   sum(t1.qualdecrease_money) qualdecrease_money
              from scmdata.t_qa_warehouse_inspect_failed t1
             where t1.qc is not null ' || v1_b ||'
             group by ' || v_s3 ||v_b_g1 || ' t1.company_id ) tq1 '|| v_s4 || v_b_g2||
'  where tq.qc is not null ) tp
  on td.company_id = tp.company_id 
 /*and tp.category_id = td.category_id
 and tp.group_id = td.group_id*/ '|| v_c_w || v_c_i;
    ...
    v2_sql := q'[ left join ( select tq.company_id,
       ...
       tq1.qualdecrease_money
  from scmdata.t_qa_inspect_failed_flw_qc tq 
 inner join (select t1.company_id, ]'|| v_s3  || q'[t1.statistical_dimension, sum(t1.qualdecrease_money ) qualdecrease_money 
               from scmdata.t_qa_inspect_failed_flw_qc t1
              where 1 = 1 ]' || v1_b ||'
              group by  '|| v_s3  ||' t1.statistical_dimension, t1.company_id  ) tq1 '|| v_s4 ||
q'[    and tq1.statistical_dimension = tq.statistical_dimension
 ) tp
    on td.company_id = tp.company_id 
 /*  and tp.category_id = td.category_id
   and tp.group_id = td.group_id*/ ]'|| v_c_w || v_c_i1;
    ...
    v2_sql_a := q'[ left join ( select tq.company_id,
       ...
       tq1.qualdecrease_money
  from scmdata.t_qa_inspect_failed_flw_qc tq 
 inner join (select t1.company_id, ]'|| v_s3  || q'[t1.statistical_dimension,t1.sort_dimension,  sum(t1.qualdecrease_money ) qualdecrease_money 
               from scmdata.t_qa_inspect_failed_flw_qc t1
              where 1 = 1 ]' || v1_b ||'
              group by  '|| v_s3  ||' t1.statistical_dimension,t1.sort_dimension,  t1.company_id  ) tq1 '|| v_s4 ||
q'[    and tq1.statistical_dimension = tq.statistical_dimension
       and tq1.sort_dimension = tq.sort_dimension 
 ) tp
    on td.company_id = tp.company_id 
   ...
   and tp.group_id = td.group_id*/ ]'|| v_c_w || v_c_i;
    ...
    v_c_sql := ' select t.company_id, ' || V_S1 || V_S ||
        ...
        inner join (select t.company_id, ]'|| V_S1 || V_S || q'[
                          ...
                          sum(purchase_money) purchase_money
                     from scmdata.t_qa_ingood t 
                    where t.company_id = ']' || V_COMPANY_ID ||'''' || v_c_where || v2_b || v_c_group || ' t.company_id,t.statistical_dimension ) t2  
    ...
    v_c_sql_a := ' select t.company_id, ' || V_S1 || V_S ||
        ...
        inner join (select t.company_id, ]'|| V_S1 || V_S || q'[
                          ...
                          sum(purchase_money) purchase_money
                     from scmdata.t_qa_ingood t 
                    where t.company_id = ']' || V_COMPANY_ID ||'''' || v_c_where || v2_b || v_c_group || ' t.company_id,t.sort_dimension,t.statistical_dimension ) t2  
           on ' || v_s2 || q'[ = t2.total_time
          and t.statistical_dimension = t2.statistical_dimension
    ...
    if V_SORT is null then
      ...
      if V_DIMENSION = '00' THEN
        ...
        if V_SORT = '1' then
          ...
          V_WHERE := V_WHERE || ' and tq.flw_order_manager like ''%' || V_SORT ||
                     '%''';
        end if;
      elsif V_DIMENSION = '08' THEN
        --统计维度：QC
