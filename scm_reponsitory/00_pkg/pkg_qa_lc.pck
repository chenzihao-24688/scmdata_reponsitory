...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_qa_lc IS
  ...
  BEGIN
    ...
    --【条件Sql】构建
    IF v_shocond IS NULL AND v_catecond IS NULL THEN
      v_cond := v_isadmincond;
    ELSIF v_shocond IS NOT NULL AND v_catecond IS NULL THEN
      v_cond := ' (' || v_isadmincond || ' or ' || v_shocond || ') ';
    ELSIF v_shocond IS NULL AND v_catecond IS NOT NULL THEN
      v_cond := ' (' || v_isadmincond || ' or ' || v_catecond || ') ';
    ELSIF v_shocond IS NOT NULL AND v_catecond IS NOT NULL THEN
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE v_sql
      INTO v_asnid, v_compid, v_ordid, v_pcomeamt, v_gotamt, v_qualdecreamt, v_qualpassamt, v_qualresult, v_unqualproresult, v_arriveretnum, v_aereason, v_pcomedate, v_scantime, v_receivetime, v_qualfinishtime
      USING v_inp_asnid, v_inp_compid;
  ...
  BEGIN
    ...
    IF v_jugnum = 0 THEN
      ...
      scmdata.pkg_qa_ld.p_ins_qareportskudim(v_inp_qarepid             => v_inp_qarepid,
                                             ...
                                             v_inp_gotamount           => v_inp_gotamount,
  ...
  BEGIN
    ...
    IF v_jugnum = 0 THEN
      ...
      scmdata.pkg_qa_ld.p_ins_qarepnumdim(v_inp_qarepid                  => v_inp_qarepid,
                                          ...
                                          v_inp_gotsumamount             => v_inp_gotsumamount,
          ...
          IF v_asnid IS NOT NULL THEN
            ...
            p_iu_qareportskudim(v_inp_qarepid             => v_qarepid,
                                ...
                                v_inp_pcomeamount         => NVL(v_pcomeamount,
                                                                 0),
                                v_inp_gotamount           => NVL(v_gotamount,
                                                                 0),
  ...
  BEGIN
    ...
    --质检结果校验
    IF v_qarepstatus IN ('PE', 'AF') AND
       NVL(v_checkresult, ' ') <> NVL(v_inp_checkresult, ' ') THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    --审核意见校验
    IF v_qarepstatus <> 'PA' AND
       NVL(v_reviewcomments, ' ') <> NVL(v_inp_reviewcomments, ' ') THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    IF v_inp_checkresult IN ('ANP', 'PNP') THEN
      ...
      --问题描述校验
      IF v_inp_problemdescription IS NULL AND v_qarepstatus = 'PC' THEN
        v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    --样衣核对-不符合项校验
    IF v_inp_yyresult = 'UQ' AND v_inp_yyuqualsubjects IS NULL THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    --面辅料查验-不符合项校验
    IF v_inp_mflresult = 'UQ' AND v_inp_mfluqualsubjects IS NULL THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    --工艺查验-不符合项校验
    IF v_inp_gyresult = 'UQ' AND v_inp_gyuqualsubjects IS NULL THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
    ...
    --版型查验-不符合项校验
    IF v_inp_bxresult = 'UQ' AND v_inp_bxuqualsubjects IS NULL THEN
      v_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_errinfo,
  ...
  BEGIN
    ...
    ELSE
      ...
      --如果Sku质检结果唯一值等于1
      IF v_skucheckresultnum <= 1 AND v_checkresult = 'PNP' THEN
  ...
  BEGIN
    ...
    ELSIF v_inp_processtype = 'WB' AND v_inp_processresult IS NULL THEN
      raise_application_error(-20002,
  ...
  BEGIN
    ...
    IF NVL(v_processtype, ' ') <> 'PBCC' OR NVL(v_skustatus, ' ') <> 'PE' THEN
      raise_application_error(-20002,
    ...
    LOOP
      ...
      IF v_asnid IS NOT NULL THEN
        ...
        p_iu_qareportskudim(v_inp_qarepid             => v_qarepid,
                            ...
                            v_inp_pcomeamount         => NVL(v_pcomeamount,
                                                             0),
                            v_inp_gotamount           => NVL(v_gotamount, 0),
                            v_inp_qualdecreaseamount  => 0,
                            v_inp_unqualamount        => NULL,
  ...
  BEGIN
    ...
    --判断，对消息类型取值
    IF v_shoid IN ('GZZ', 'GDZ') THEN
      v_msgcode := 'QA_LD_GZ';
    ELSIF v_shoid = 'YWZ' THEN
      v_msgcode := 'QA_LD_YW';
    END IF;
  ...
  BEGIN
    ...
    IF INSTR(v_cates, '07') > 0 THEN
      v_msgcode := 'QA_UQ_TP';
    ELSE
      IF INSTR(v_shoids, 'GZZ') > 0 OR INSTR(v_shoids, 'GDZ') > 0 THEN
        v_msgcode := 'QA_UQ_GZ';
      ELSIF INSTR(v_shoids, 'YWZ') > 0 THEN
        v_msgcode := 'QA_UQ_YW';
      END IF;
    END IF;
    ...
    SELECT MAX(msginfo_id), MAX(config_code), MAX(msg_type), MAX(msg_info)
      ...
      FROM (SELECT msginfo_id, config_code, msg_type, msg_info
             ...
             WHERE company_id = v_inp_compid
               AND msg_status = 'R'
             ORDER BY nvl(update_time, create_time)
  ...
  BEGIN
    ...
    IF v_hasmsgnotsend > 0 AND
       (v_lastupdtime IS NULL OR
  ...
  BEGIN
    ...
    ELSE
      ...
      --尺寸录入人校验
      IF v_operuserid <> v_inp_curuserid AND v_operuserid IS NOT NULL THEN
