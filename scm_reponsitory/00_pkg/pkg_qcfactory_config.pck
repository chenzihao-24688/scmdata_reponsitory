...
create or replace package body scmdata.PKG_QCFACTORY_CONFIG is
  ...
  begin
    ...
    p_sql := p_sql || q'[  ) b
    on (a.qc_group_id = b.qc_group_id and a.factory_code = b.factory_code and a.company_id = b.company_id and a.industry_classification = b.industry_classification and a.production_category = b.production_category and a.product_subclass = b.product_subclass)
    when not matched then
      insert
  ...
  begin
    select listagg(qc_group_id)
      into p_qc_group_id
      from (select si.supplier_code factory_code,
                   bb.category      industry_classification,
                   aa.qc_group_id
              from scmdata.t_qc_group_config aa
             inner join scmdata.t_qc_group_cate_config bb
    ...
    if p_category is null then
      ...
      update scmdata.t_qc_factory_config_category a
         set a.production_category =
             (select listagg(distinct w.production_category, ';')
                from scmdata.t_qc_factory_config_detail w
               where w.qc_group_id = a.qc_group_id
                 and w.industry_classification = p_category
                 and w.factory_code = a.factory_code)
       where a.qc_group_id = p_qc_group_id
         and a.industry_classification = p_category;
  ...
  begin
    for x in (select si.company_id,
      ...
      --先禁用之前的配置
      if p_new_qc_group_id <> p_old_qc_group_id then
