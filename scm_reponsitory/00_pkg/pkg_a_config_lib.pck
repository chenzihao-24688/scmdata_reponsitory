CREATE OR REPLACE PACKAGE SCMDATA.pkg_a_config_lib IS
  ...
  PROCEDURE p_check_cate_repeat_progress_v2(pi_company_id              IN VARCHAR2,
                                            ...
                                            pi_new_id                  IN VARCHAR2);
  --生产节点配置校验
  PROCEDURE p_check_pnode_config(p_company_id              VARCHAR2,
                                 p_progress_config_id      VARCHAR2,
                                 p_progress_node_config_id VARCHAR2,
  ...
  --分组配置-产业区域分组配置tab 当“失效时间”小于系统当前时间时，“状态”系统自动更新为‘停用’； by dyy153 20220422
  PROCEDURE p_autoupd_pause_byendtime;
  PROCEDURE p_reset_sup_group_by_pause(p_company_id      VARCHAR2,
                                       p_group_config_id VARCHAR2,
                                       p_user_id         VARCHAR2 DEFAULT 'ADMIN',
                                       p_pause           INT);
  --分组配置发生变动，则重置供应商分组
  PROCEDURE p_reset_sup_group(p_company_id VARCHAR2,
                              p_user_id    VARCHAR2 DEFAULT 'ADMIN');
  -------------------------------czh add 所在分组配置  end --------------------------------

  --校验模板是否可以保存
  PROCEDURE p_check_enable_model_deduction(p_config_id VARCHAR2,
                                           p_oper      VARCHAR2 DEFAULT '保存');
END pkg_a_config_lib;
/

CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_a_config_lib IS
  ...
  PROCEDURE p_check_cate_repeat_deduction_v2(pi_company_id              IN VARCHAR2,
    ...
    p_model_name          VARCHAR2(1000);
    p_i                   INT;
    p_pause               INT;
    p_validity_begin_date DATE;
  ...
  BEGIN
    ...
    --20220823 如果模型时禁用的，不做其他模块的校验，
    SELECT MAX(pause), MAX(validity_begin_date), MAX(validity_end_date)
      INTO p_pause, p_validity_begin_date, p_validity_end_date
      FROM scmdata.t_deduction_config a
     WHERE a.deduction_config_id = pi_config_id;
    IF p_pause = 1 THEN
      RETURN;
    END IF;
    ---20220823 补充逻辑，其他模板如果是禁用，不做校验
    --如果其他的模板没有时间范围的重合，不做校验
    SELECT listagg('“' || b.deduction_name || '“', '、')
      INTO p_model_name
      FROM scmdata.t_deduction_range_config a
     INNER JOIN scmdata.t_deduction_config b
        ON a.deduction_config_id = b.deduction_config_id
     WHERE a.deduction_range_config_id <> pi_new_id
       AND a.deduction_config_id <> pi_config_id
       AND a.company_id = pi_company_id
       AND a.industry_classification = pi_industry_classification
       ...
       AND b.pause = 0
       AND (b.validity_begin_date BETWEEN p_validity_begin_date AND
           p_validity_end_date OR
           b.validity_end_date BETWEEN p_validity_begin_date AND
           p_validity_end_date
           
           OR p_validity_begin_date BETWEEN b.validity_begin_date AND
           b.validity_end_date OR
           p_validity_end_date BETWEEN b.validity_begin_date AND
           b.validity_end_date);
    IF p_model_name IS NOT NULL THEN
      raise_application_error(-20002,
                              '保存失败！当前模型【适用范围】与' || p_model_name ||
                              '“存在相同的“生产类别+生产类别+产品子类”，“生效日期 - 失效日期“的时间范围不允许重复，请检查。');
    END IF;
  ...
  PROCEDURE p_check_cate_repeat_progress_v2(pi_company_id              IN VARCHAR2,
    ...
    p_model_name INT;
  BEGIN
    --czh add 统一模板下不能存在多个分类
    SELECT COUNT(DISTINCT a.industry_classification)
      INTO p_model_name
      FROM scmdata.t_progress_range_config a
     WHERE a.progress_config_id = pi_config_id
       AND a.company_id = pi_company_id;
    IF p_model_name > 1 THEN
      raise_application_error(-20002, '保存失败，存在多个分类，请检查');
    END IF;
    --czh add end
    SELECT nvl(MAX(1), 0)
      INTO p_model_name
  ...
  /*============================================*
  * Author   : CZH
  * Created  : 2022-05-16 10:38:51
  * ALERTER  : 
  * ALERTER_TIME  : 
  * Purpose  :  生产节点配置校验
  * Obj_Name    : P_CHECK_PNODE_CONFIG
  * Arg_Number  : 4
  * < IN PARAMS >  
  * P_COMPANY_ID :  企业ID
  * P_PROGRESS_CONFIG_ID : 生产节点配置 主表ID
  * P_PROGRESS_NODE_CONFIG_ID : 节点配置 从表ID
  * P_NODE_NAME : 节点编码
  *============================================*/
  ...
  PROCEDURE p_check_pnode_config(p_company_id              VARCHAR2,
                                 p_progress_config_id      VARCHAR2,
                                 p_progress_node_config_id VARCHAR2,
                                 p_node_name               VARCHAR2) IS
    v_i    INT;
    v_cate VARCHAR2(32);
  BEGIN
    SELECT COUNT(*)
      INTO v_i
      FROM t_progress_node_config a
     WHERE a.progress_config_id = p_progress_config_id
       AND progress_node_name = p_node_name
       AND a.progress_node_config_id <> p_progress_node_config_id;
  
    IF v_i > 0 THEN
      raise_application_error(-20002, '已存在相同的生产节点名称，请检查！');
    END IF;
  
    SELECT DISTINCT t.industry_classification
      INTO v_cate
      FROM scmdata.t_progress_range_config t
     WHERE t.progress_config_id = p_progress_config_id
       AND t.company_id = p_company_id;
  
    IF substr(p_node_name, 3, 2) = v_cate THEN
      NULL;
    ELSE
      raise_application_error(-20002,
                              '保存失败，所选节点不符适用范围，请检查！');
    END IF;
  END p_check_pnode_config;
...
SELECT t.group_area_config_id,
 ...
 WHERE  t.pause = 1
  ...
  BEGIN
    ...
    VALUES
      (p_gc_rec.group_config_id,
       ...
       p_gc_rec.company_id);
    --重置供应商分组
    scmdata.pkg_a_config_lib.p_reset_sup_group(p_company_id => p_gc_rec.company_id,
                                               p_user_id    => p_gc_rec.update_id);
  ...
  BEGIN
    ...
    UPDATE t_supplier_group_area_config t
       SET t.is_nationwide       = p_gc_rec.is_nationwide,
           ...
           t.update_id   = p_gc_rec.update_id,
           t.update_time = p_gc_rec.update_time,
           t.group_type  = p_gc_rec.group_type
  ...
  BEGIN
    ...
    DELETE FROM scmdata.t_supplier_group_subcate_config a
     WHERE a.group_category_config_id =
           (SELECT b.group_category_config_id
              FROM scmdata.t_supplier_group_category_config b
             WHERE b.group_config_id = p_group_config_id
    ...
    DELETE FROM scmdata.t_supplier_group_config t
     WHERE t.group_config_id = p_group_config_id
       AND t.company_id = p_company_id;
    --重置供应商分组
    scmdata.pkg_a_config_lib.p_reset_sup_group(p_company_id => p_company_id);
  EXCEPTION
    WHEN OTHERS THEN
  ...
  BEGIN
    DELETE FROM scmdata.t_supplier_group_subcate_config a
     WHERE a.company_id = p_company_id
       AND a.group_category_config_id = p_gc_config_id;
    DELETE FROM scmdata.t_supplier_group_category_config t
  ...
  pkg_plat_comm.p_pause(p_table       => :t_supplier_group_config,
    ...
    --1.重置供应商档案自动生成的分组
    p_reset_sup_group(p_company_id => p_company_id, p_user_id => p_user_id);
    --2.重置供应商档案手动生成的分组
    p_reset_sup_group_by_pause(p_company_id      => p_company_id,
                               p_group_config_id => p_group_config_id,
                               p_user_id         => p_user_id,
                               p_pause           => p_status);
  ...
  BEGIN
    ...
    SELECT COUNT(1)
     ...
     WHERE t.company_id = p_gc_rec.company_id
       ...
       AND t.group_type = p_gc_rec.group_type;
    IF v_flag > 0 THEN
  ...
  --分组配置-产业区域分组配置tab 当“失效时间”小于系统当前时间时，“状态”系统自动更新为‘停用’； by dyy153 20220422
  PROCEDURE p_autoupd_pause_byendtime IS
  
  BEGIN
  
    FOR i IN (SELECT * FROM scmdata.t_supplier_group_config) LOOP
      IF i.end_time < trunc(SYSDATE) THEN
        UPDATE scmdata.t_supplier_group_config t
           SET t.pause = 0
         WHERE t.group_config_id = i.group_config_id
  ...
  END p_autoupd_pause_byendtime;
  --分组配置发生变动，则重置自动生成的供应商分组
  PROCEDURE p_reset_sup_group_by_pause(p_company_id      VARCHAR2,
                                       p_group_config_id VARCHAR2,
                                       p_user_id         VARCHAR2 DEFAULT 'ADMIN',
                                       p_pause           INT) IS
  ...
  BEGIN
    --配置停用
    IF p_pause = 0 THEN
      --获取配置发生变动后，手动编辑分组的供应商
      FOR gp_rec IN (SELECT t.supplier_info_id, t.company_id, t.group_name
                       FROM scmdata.t_supplier_info t
                      WHERE t.group_name = p_group_config_id
                        AND t.company_id = p_company_id
                        AND t.group_name_origin = 'MA') LOOP
        SELECT COUNT(1)
          INTO v_flag
          FROM scmdata.t_supplier_group t
         WHERE t.supplier_info_id = gp_rec.supplier_info_id
           AND t.company_id = p_company_id;
        IF v_flag > 0 THEN
          UPDATE scmdata.t_supplier_group t
             SET t.group_name = gp_rec.group_name
           WHERE t.supplier_info_id = gp_rec.supplier_info_id
             AND t.company_id = gp_rec.company_id;
        ELSE
          INSERT INTO scmdata.t_supplier_group
          VALUES
            (scmdata.f_get_uuid(),
             gp_rec.company_id,
             gp_rec.supplier_info_id,
             gp_rec.group_name,
             'ADMIN',
             SYSDATE,
             'ADMIN',
        ...
        UPDATE scmdata.t_supplier_info t
           SET t.group_name = NULL, t.update_id = p_user_id
         WHERE t.supplier_info_id = gp_rec.supplier_info_id
           AND t.company_id = gp_rec.company_id;
      END LOOP;
      --配置启用
    ELSIF p_pause = 1 THEN
      FOR gp_rec IN (SELECT t.supplier_info_id, t.company_id, t.group_name
                       FROM scmdata.t_supplier_group t
                      WHERE t.group_name = p_group_config_id
                        AND t.company_id = p_company_id) LOOP
        UPDATE scmdata.t_supplier_info t
           SET t.group_name = gp_rec.group_name, t.update_id = p_user_id
         WHERE t.supplier_info_id = gp_rec.supplier_info_id
           AND t.company_id = gp_rec.company_id;
      END LOOP;
    ELSE
  ...
  END p_reset_sup_group_by_pause;

  --分组配置发生变动，则重置自动生成的供应商分组
  PROCEDURE p_reset_sup_group(p_company_id VARCHAR2,
                              p_user_id    VARCHAR2 DEFAULT 'ADMIN') IS
    vo_group_config_id VARCHAR2(32);
  BEGIN
    FOR sup_rec IN (SELECT t.supplier_info_id, t.company_id, t.group_name
                      FROM scmdata.t_supplier_info t
                     WHERE t.company_id = p_company_id
      ...
      IF scmdata.pkg_plat_log.f_is_check_fields_eq(p_old_field => sup_rec.group_name,
                                                   p_new_field => vo_group_config_id) = 0 THEN
        UPDATE scmdata.t_supplier_info t
           SET t.group_name        = vo_group_config_id,
               t.group_name_origin = 'AA',
               t.update_id         = p_user_id,
               t.update_date       = SYSDATE
         WHERE t.company_id = sup_rec.company_id
           AND t.supplier_info_id = sup_rec.supplier_info_id;
      ELSE
        NULL;
      END IF;
    END LOOP;
  END p_reset_sup_group;
  -------------------------------czh end 所在分组配置--------------------------------
  --校验模板是否可以保存
  PROCEDURE p_check_enable_model_deduction(p_config_id VARCHAR2,
                                           p_oper      VARCHAR2 DEFAULT '保存') IS
    --如果存在其他模板有重复的生产类别+生产类别+产品子类，且生效范围有重合的，报错
    p_model_name          VARCHAR2(1000);
    p_pause               NUMBER(1);
  ...
  BEGIN
    ...
    SELECT MAX(a.pause),
           MAX(a.company_id),
           MAX(validity_begin_date),
           MAX(validity_end_date)
      INTO p_pause,
           p_company_id,
           p_validity_begin_date,
           p_validity_end_date
      FROM scmdata.t_deduction_config a
     WHERE a.deduction_config_id = p_config_id;
    IF p_pause = 1 THEN
      RETURN;
    END IF;
    ...
    SELECT listagg('“' || b.deduction_name || '“', '、')
     ...
     INNER JOIN scmdata.t_deduction_config b
        ON a.deduction_config_id = b.deduction_config_id
     INNER JOIN scmdata.t_deduction_range_config c
        ON c.deduction_config_id = p_config_id
       AND c.industry_classification = a.industry_classification
       AND c.production_category = a.production_category
     WHERE a.deduction_config_id <> p_config_id
       AND a.company_id = p_company_id
       AND regexp_like(c.product_subclass || ';',
                       REPLACE(a.product_subclass, ';', ';|'))
       AND b.pause = 0
       AND (b.validity_begin_date BETWEEN p_validity_begin_date AND
           p_validity_end_date OR
    ...
    IF p_model_name IS NOT NULL THEN
      raise_application_error(-20002,
                              p_oper || '失败！当前模型【适用范围】与' || p_model_name ||
                              '“存在相同的“生产类别+生产类别+产品子类”，且“生效日期 - 失效日期“的时间范围存在重叠，请检查。');
    END IF;
  END p_check_enable_model_deduction;
