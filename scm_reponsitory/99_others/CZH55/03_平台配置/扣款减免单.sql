...
SELECT order_id              "订单单据号",
       style_number          "特征",
       delay_amount          "数量",
       discount_unit_price   "单价",
       actual_discount_price "总价",
       delivery_date         "约定交期",
       arrival_date          "实际交期",
  ...
  FROM (SELECT td.order_id order_id,
         ...
         WHERE td.company_id = %default_company_id% --'a972dd1ffe3b3a10e0533c281cac8fd7'
           AND td.order_id = :order_code); --'FAKEINFO0871';
