  ...
  BEGIN
    ...
    ELSIF upper(p_method) = 'U' THEN
      SELECT 'UPDATE ' || listagg(DISTINCT(t.table_name)) || ' t ' ||
             ' SET ' || rtrim(xmlagg(xmlparse(content ' t.' || t.column_name || ' =  ' || p_pre || t.column_name || ',' wellformed) ORDER BY t.column_id).getclobval(),
                              ',') || nvl(p_where_sql, ' WHERE 1 = 0;')
        INTO v_sql
        FROM user_tab_columns t
       WHERE t.table_name = upper(p_table_name);
    ...
    ELSIF upper(p_method) = 'S' THEN
      SELECT 'SELECT ' || rtrim(listagg('t.' || t.column_name || ',', '')
                                within GROUP(ORDER BY t.column_id),
                                ',') || ' FROM ' || p_table_name || ' t ' ||
    ...
    ELSIF upper(p_method) = 'CK' THEN
      DECLARE
        ...
        v_desc VARCHAR2(256);
      BEGIN
        v_rtn := scmdata.pkg_plat_comm.f_parse_json(get_json_str(v_ucc_rec.comments),
                                                    p_key => 'REQUERED');
        IF v_rtn = '1' THEN
          v_desc := scmdata.pkg_plat_comm.f_parse_json(get_json_str(v_ucc_rec.comments),
                                                       p_key => 'DESC');
...
BEGIN 
      ...
      IF p_method_type IN (1, 3) THEN
        ...
        ELSE
          vo_sql := vo_sql || 'PROCEDURE P_' || p_method || (CASE
                      ...
                      ELSE
                       '_' || p_table_name
                    END) || v_method_type || v_params || v_var_define1;
        END IF;
      ELSE
        --是否是声明
        IF p_is_dec = 1 THEN
  ...
  BEGIN
    ...
    --1.包头
    vo_sql := '/*************包头*************/';
    vo_sql := vo_sql || chr(13) || 'CREATE OR REPLACE PACKAGE PKG_' ||
              p_table_name || ' IS';
    --查询
    ...
    --所有必填项校验
    FOR ck_rec IN (SELECT c.column_name, c.comments
                     FROM user_tab_columns t
                    INNER JOIN user_col_comments c
                       ON t.table_name = c.table_name
                      AND t.column_name = c.column_name
      ...
      v_rtn := scmdata.pkg_plat_comm.f_parse_json(get_json_str(ck_rec.comments),
                                                  p_key => 'RUN');
      IF v_rtn = '1' THEN
        get_func_proc_init_begin_sql(p_table_name  => p_table_name,
    ...
    vo_sql := vo_sql || chr(13) || 'CREATE OR REPLACE PACKAGE BODY PKG_' ||
              p_table_name || ' IS';
...
END;
  ...
  --初始化所有基础sql  待做
  /*  FUNCTION get_init_all_base_sql() RETURN CLOB IS
  BEGIN
    --1.初始化包
    get_init_pkg(p_pre        => '',
                 p_table_name => '',
