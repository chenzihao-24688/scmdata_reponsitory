  ...
  FROM (SELECT a.*,
          ...
          LEFT JOIN (SELECT *
           ...
           left join (select  object_id,
                        ...
                        from scmdata.t_ask_scope group by object_id,
                             company_id) c
             on c.object_id=a.ask_record_id 
           AND c.company_id = decode(a.origin, 'MA', a.be_company_id,a.company_id)
         WHERE a.be_company_id = %default_company_id%
           AND instr('CA03,CA00,', coor_ask_flow_status || ',') = 0) tar
         where ((%is_company_admin% = 1) 
       OR scmdata.instr_priv(p_str1  => scmdata.pkg_data_privs.parse_json(p_jsonstr => %data_privs_json_strs%,p_key => 'COL_2'),
                             p_str2  => tar.cate,
                             p_split => ';')>0)
