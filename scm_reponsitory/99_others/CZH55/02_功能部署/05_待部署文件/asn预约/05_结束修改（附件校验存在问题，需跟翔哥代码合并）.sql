...
BEGIN
  ...
  ELSE
    ...
    --czh add 附件校验 begin
    SELECT (CASE
             WHEN a.attachname IS NOT NULL THEN
              1
             ELSE
              0
           END)
   ...
   AND a.barcode = :barcode;
    ...
    IF v_has_file = 0 THEN
      raise_application_error(-20001,
                              ''ASN单据号['' || v_asn_id || '']中附件不可为空'');
    END IF;
    --end czh 
    ...
    UPDATE asnordered a
       SET a.finishtime = SYSDATE, a.inserttime = SYSDATE
     WHERE EXISTS (SELECT 1
              FROM asnorders cc
             WHERE cc.asn_id = a.asn_id
               AND cc.asn_id || cc.goo_id IN (@selection));
    ...
    UPDATE asnordered a
       SET a.audittime = SYSDATE, a.auditorid = %inner_user_id%
     WHERE EXISTS (SELECT 1
              FROM asnorders cc
             WHERE cc.asn_id = a.asn_id
               AND cc.asn_id || cc.goo_id IN (@selection));
