...
SELECT t.quality_control_id,
 ...
 ORDER BY v.create_time DESC]';
  ...
  BEGIN
    ...
    --23053115_zc314_change_add_pause_field
    SELECT COUNT(1), MAX(t.coop_factory_id), MAX(t.pause)
      INTO v_cnt, v_coop_factory_id, v_old_pause
    ...
    IF v_sup_rec.is_our_factory = 1 THEN
      ...
      IF v_cnt > 0 THEN
        ...
        IF nvl(v_old_pause, 1) <> (CASE WHEN v_sup_rec.pause IN (0, 2) THEN 0 ELSE 1 END) THEN
          --23053115_zc314_add
          scmdata.pkg_capacity_inqueue_prd.p_common_inqueue(v_curuserid => p_user_id,
      ...
      ELSE
        ...
        scmdata.pkg_capacity_inqueue_prd.p_common_inqueue(v_curuserid => p_user_id,
                                                      ...
                                                      v_conds     => 'COOP_FACTORY_ID = ''' ||
                                                                     v_fac_rec.coop_factory_id ||
                                                                     ''' AND COMPANY_ID = ''' ||
                                                                     v_fac_rec.company_id || '''',
                                                      v_method    => 'INS',
                                                      v_viewlogic => NULL,
                                                      v_queuetype => 'CAPC_SUPFILE_COOPFACINFO_IU');
      END IF;
    ELSE
      IF v_cnt > 0 THEN
        ...
        scmdata.pkg_capacity_inqueue_prd.p_common_inqueue(v_curuserid => p_user_id,
                                                      v_compid    => p_company_id,
                                                      v_tab       => 'SCMDATA.T_COOP_FACTORY',
                                                      v_viewtab   => 'SCMDATA.T_COOP_FACTORY',
                                                      v_unqfields => 'COOP_FACTORY_ID,COMPANY_ID',
                                                      v_ckfields  => 'COOP_FACTORY_ID,COMPANY_ID,UPDATE_ID,UPDATE_TIME',
                                                      v_conds     => 'COOP_FACTORY_ID = ''' ||
                                                                     v_coop_factory_id ||
                                                                     ''' AND COMPANY_ID = ''' ||
                                                                     p_company_id || '''',
