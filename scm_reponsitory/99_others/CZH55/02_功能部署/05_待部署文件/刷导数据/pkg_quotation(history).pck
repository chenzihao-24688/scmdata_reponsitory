...
CREATE OR REPLACE PACKAGE BODY pkg_quotation IS
  ...
  BEGIN
    ...
    IF p_type = 0 THEN
      v_sql := q'[SELECT t.quotation_status,
       ...
       t.sanfu_article_no,
       va.quotation_classification_n,
       t.quotation_classification,]' || --男鞋、女鞋
               CASE
  ...
  LEFT JOIN (SELECT v.quotation_classification_n1 || '/' ||
    ...
    ELSIF p_type = 2 THEN
      ...
      IF v_quotation_class_code IN
         ('PLM_READY_PRICE_CLASSIFICATION00010001',
          'PLM_READY_PRICE_CLASSIFICATION00010003') THEN
        v_sql := q'[SELECT t.freight freight_ot,t.management_expense ,t.development_fee ,
    ...
    ELSIF p_type = 3 THEN
      ...
      SELECT to_blob(null)   style_picture_fj,
       ...
       fa.file_name    style_picture_name,
       fb.file_name    pattern_file_name,
       fc.file_name    marker_file_name,
       fa.file_md5     style_picture_id,
       fb.file_md5     pattern_file_id,
       fc.file_md5     marker_file_id      
  FROM plm.quotation t
 LEFT JOIN plm.plm_file fa
    ON fa.thirdpart_id = t.quotation_id
   AND fa.file_type = 1
 ...
 LEFT JOIN plm.plm_file fc
  ...
  BEGIN
    ...
    INSERT INTO quotation
      (quotation_id,
       ...
       cooperation_mode,
       quotation_source,
       whether_sanfu_fabric,
       whether_add_color_quotation,
       related_colored_quotation,
       quotation_total,
       material_amount,
       consumables_amount,
       craft_amount,
       ...
       consumables_quotation_remark,
       working_procedure_machining_total,
       crop_salary,
       skiving_salary,
       forming_salary,
       working_procedure_machining_remark,
       ...
       whether_del)
    VALUES
      (p_quota_rec.quotation_id,
       p_quota_rec.quotation_status,
       p_quota_rec.cancel_reason,
       p_quota_rec.create_time,
       p_quota_rec.create_id,
       p_quota_rec.update_time,
       p_quota_rec.update_id,
       p_quota_rec.cooperation_mode,
       p_quota_rec.quotation_source,
       p_quota_rec.whether_sanfu_fabric,
       p_quota_rec.whether_add_color_quotation,
       p_quota_rec.related_colored_quotation,
       p_quota_rec.quotation_total,
       p_quota_rec.material_amount,
       p_quota_rec.consumables_amount,
       p_quota_rec.craft_amount,
       p_quota_rec.working_procedure_amount,
       p_quota_rec.other_amount,
       p_quota_rec.affreightment_amount,
       p_quota_rec.platform_unique_key,
       p_quota_rec.item_no,
       p_quota_rec.color,
       p_quota_rec.sanfu_article_no,
       p_quota_rec.quotation_classification,
       p_quota_rec.paper_quotation_picture,
       p_quota_rec.style_picture,
       p_quota_rec.pattern_file,
       p_quota_rec.marker_file,
       ...
       p_quota_rec.crop_salary,
       p_quota_rec.skiving_salary,
       p_quota_rec.forming_salary,
       p_quota_rec.working_procedure_machining_remark,
       p_quota_rec.bag_paper_lattice_number,
       p_quota_rec.management_expense,
       p_quota_rec.development_fee,
       p_quota_rec.euipment_depreciation,
       p_quota_rec.rent_and_utilities,
       p_quota_rec.processing_profit,
       p_quota_rec.freight,
       p_quota_rec.design_fee,
       p_quota_rec.whether_del);
  ...
  BEGIN
    --男鞋、女鞋
    IF p_type = 0 THEN
      IF p_qt_rec.freight IS NULL OR p_qt_rec.management_expense IS NULL OR
  ...
  BEGIN
    ...
    IF v_quotation_class_code IN
      ...
      IF p_shoe_rec.shoe_packing_type = 'PLM_READY_PACKAGES_TYPE01' THEN
        IF p_shoe_rec.shoe_box_amount IS NULL THEN
          v_str := ' “鞋盒装-金额“ ';
        END IF;
      ELSIF p_shoe_rec.shoe_packing_type = 'PLM_READY_PACKAGES_TYPE02' THEN
 ...
 WHERE ROWNUM = 1]';
  ...
  BEGIN
    v_sql := q'[SELECT t.consumables_name_id,
       ...
       t.suggested_purchase__price]' || CASE
  ...
  LEFT JOIN mrp.mrp_determine_supplier_archives sa
    ON sa.supplier_code = ms.supplier_code
  LEFT JOIN (SELECT mp.thirdpart_id, MAX(mp.url) url
  ...
  BEGIN
    ...
    INSERT INTO consumables_consumption_detail
      (consumables_name_id,
       consumables_material_no,
       consumables_material_name,
       consumables_material_source,
       consumables_material_consumption,
       consumables_material_unit,
       suggested_purchase_unit_price,
       consumables_material_sku,
       consumables_material_supplier_code,
       suggested_purchase__price,
       quotation_id,
       whether_del,
       create_time,
       create_id,
       ...
       update_id)
    VALUES
      (p_consu_rec.consumables_name_id,
       v_num,
       ...
       p_consu_rec.suggested_purchase__price,
       p_consu_rec.quotation_id,
       p_consu_rec.whether_del,
       p_consu_rec.create_time,
       p_consu_rec.create_id,
       p_consu_rec.update_time,
       p_consu_rec.update_id);
    ...
    ELSIF p_type = 1 THEN
      UPDATE consumables_consumption_detail t
         SET t.consumables_material_no            = p_consu_rec.consumables_material_no,
             ...
             t.whether_del                        = p_consu_rec.whether_del,
             t.update_time                        = p_consu_rec.update_time,
             t.update_id                          = p_consu_rec.update_id
       WHERE t.consumables_name_id = p_consu_rec.consumables_name_id
         AND t.quotation_id = p_consu_rec.quotation_id;
  ...
  BEGIN
    SELECT MAX(va.craft_classification_desc)
      INTO v_craft_classification_desc
    ...
    RETURN v_craft_classification_desc;
 ...
 WHERE v.lv = 2]';
  ...
  BEGIN
    ...
    ELSE
      v_price := (p_craft_unit_price * p_craft_consumption) / 100;
    END IF;
    RETURN v_price;
    ...
    IF p_speci_rec.craft_classification IS NOT NULL THEN
      ...
      IF v_process_name_pt_1 IN ('面料洗水', '成衣洗水', '牛仔洗水') THEN
        IF p_speci_rec.washing_percent IS NULL THEN
          raise_application_error(-20002,
                                  '当“特殊工艺”一级分类=“面料洗水/成衣洗水/牛仔洗水”时, “洗水%” 必填');
        END IF;
      END IF;
    END IF;
 ...
 ORDER BY t.create_time DESC]';
  ...
  BEGIN
    INSERT INTO special_craft_quotation
      (quotation_special_craft_detail_id,
       related_colored_detail_id,
       quotation_special_craft_detail_no,
    ...
    VALUES
      (p_speci_rec.quotation_special_craft_detail_id,
       ...
       p_speci_rec.craft_classification,
       p_speci_rec.craft_unit_price,
       p_speci_rec.craft_consumption,
       p_speci_rec.craft_price,
       p_speci_rec.washing_percent,
       p_speci_rec.craft_factory_name,
       p_speci_rec.craft_quotation_remark,
       p_speci_rec.quotation_id,
       p_speci_rec.whether_del,
       p_speci_rec.create_time,
       p_speci_rec.create_id,
       p_speci_rec.update_time,
       p_speci_rec.update_id);
  ...
  BEGIN
    INSERT INTO shoes_fee
      (shoe_fee_id,
       ...
       shoe_packing_type,
       shoe_packing_number,
       shoe_transportation_route,
       shoe_box_material_name,
       shoe_box_specifications,
       quotation_id,
       whether_del,
       create_time,
       create_id,
       ...
       is_update_field)
    VALUES
      (p_shoes_rec.shoe_fee_id,
       p_shoes_rec.shoe_craft_category,
       p_shoes_rec.shoe_packing_type,
       p_shoes_rec.shoe_packing_number,
       p_shoes_rec.shoe_transportation_route,
       p_shoes_rec.shoe_box_material_name,
       p_shoes_rec.shoe_box_specifications,
       p_shoes_rec.quotation_id,
       p_shoes_rec.whether_del,
       p_shoes_rec.create_time,
       p_shoes_rec.create_id,
       p_shoes_rec.update_time,
       p_shoes_rec.update_id,
  ...
  BEGIN
    INSERT INTO shoes_fee
      (shoe_fee_id,
       ...
       shoe_box_specifications,
       shoe_box_amount,
       shoe_box_quotation_remark,
       egg_lattice_amount,
       egg_lattice_quotation_remark,
       outer_box_amount,
       outer_box_quotation_remark,
       freight,
       freight_quotation_remark,
       quotation_id,
       whether_del,
       create_time,
       create_id,
       update_time,
       update_id,
       shoe_examin_epriceresult,
       is_update_field)
    VALUES
      (p_shoe_rec.shoe_fee_id,
       p_shoe_rec.shoe_craft_category,
       p_shoe_rec.shoe_packing_type,
       p_shoe_rec.shoe_packing_number,
       p_shoe_rec.shoe_transportation_route,
       p_shoe_rec.shoe_box_material_name,
       p_shoe_rec.shoe_box_specifications,
       p_shoe_rec.shoe_box_amount,
       p_shoe_rec.shoe_box_quotation_remark,
       p_shoe_rec.egg_lattice_amount,
       p_shoe_rec.egg_lattice_quotation_remark,
       p_shoe_rec.outer_box_amount,
       p_shoe_rec.outer_box_quotation_remark,
       p_shoe_rec.freight,
       p_shoe_rec.freight_quotation_remark,
       p_shoe_rec.quotation_id,
       p_shoe_rec.whether_del,
       p_shoe_rec.create_time,
       p_shoe_rec.create_id,
       p_shoe_rec.update_time,
       p_shoe_rec.update_id,
       p_shoe_rec.shoe_examin_epriceresult,
       p_shoe_rec.is_update_field);
  ...
  BEGIN
    INSERT INTO material_detail_quotation
      (material_detail_quotation_id,
       ...
       quotation_material_detail_no,
       quotation_material_type,
       quotation_application_site,
       whether_inner_material,
       quotation_material_sku,
       quotation_material_supplier_code,
       quotation_unit,
    ...
    VALUES
      (p_material_rec.material_detail_quotation_id,
       ...
       p_material_rec.quotation_unit,
       p_material_rec.quotation_unit_price,
       p_material_rec.quotation_unit_price_consumption,
       p_material_rec.quotation_loss_rate,
       p_material_rec.quotation_amount,
       p_material_rec.quotation_remark,
       p_material_rec.quotation_id,
       p_material_rec.whether_del,
       p_material_rec.create_time,
       p_material_rec.create_id,
       p_material_rec.update_time,
  ...
  BEGIN
    IF p_quotation_id IS NOT NULL THEN
      ...
      IF v_flag > 0 THEN
        ...
        p_new_qt_rec.quotation_status := 0;
  ...
  BEGIN
    INSERT INTO examine_price
      (examine_price_id,
       ...
       whether_del)
    VALUES
      (p_exprice_rec.examine_price_id,
       p_exprice_rec.relate_quotation_id,
       p_exprice_rec.relate_add_color_examine_price_id,
       p_exprice_rec.current_quotation,
       p_exprice_rec.examine_price_status,
       p_exprice_rec.cancel_reason,
       p_exprice_rec.cooperation_mode,
       p_exprice_rec.platform_unique_key,
       p_exprice_rec.create_time,
       p_exprice_rec.finish_time,
       p_exprice_rec.matching_status,
       p_exprice_rec.matching_goods_skc_code,
       p_exprice_rec.manual_order_number,
       p_exprice_rec.whether_special_adopt,
       p_exprice_rec.whether_cancel,
       p_exprice_rec.current_tier_price,
       p_exprice_rec.total_amount,
       p_exprice_rec.material_amount,
       p_exprice_rec.consumables_amount,
       p_exprice_rec.craft_amount,
       p_exprice_rec.working_procedure_amount,
       p_exprice_rec.other_amount,
       p_exprice_rec.affreightment_amount,
       ...
       p_exprice_rec.crop_salary,
       p_exprice_rec.skiving_salary,
       p_exprice_rec.forming_salary,
       p_exprice_rec.crop_process_cost,
       p_exprice_rec.sew_process_cost,
       p_exprice_rec.standard_sewing_hours,
       ...
       p_exprice_rec.management_expense,
       p_exprice_rec.development_fee,
       p_exprice_rec.euipment_depreciation,
       p_exprice_rec.rent_and_utilities,
       p_exprice_rec.processing_profit,
       p_exprice_rec.freight,
       ...
       p_exprice_rec.tier_category_code,
       p_exprice_rec.tier_category_name,
       p_exprice_rec.call_back_remark,
       p_exprice_rec.create_id,
       p_exprice_rec.update_time,
       p_exprice_rec.update_id,
       p_exprice_rec.whether_del);
  ...
  BEGIN
    INSERT INTO special_craft_examine_price
      (examine_price_special_craft_detail_id,
       related_colored_detail_id,
       special_craft_detail_no,
       craft_classification,
       craft_classification_department,
       special_craft_examine_price_result,
       craft_unit,
       craft_unit_price,
       craft_consumption,
       craft_price,
       washing_percent,
       craft_factory_name,
       craft_examine_price_remark,
       related_colored_quotation_detail_id,
       examine_price_id)
    VALUES
      (p_special_craft.examine_price_special_craft_detail_id,
       ...
       p_special_craft.craft_unit,
       p_special_craft.craft_unit_price,
       p_special_craft.craft_consumption,
       p_special_craft.craft_price,
       p_special_craft.washing_percent,
       p_special_craft.craft_factory_name,
       p_special_craft.craft_examine_price_remark,
       p_special_craft.related_colored_quotation_detail_id,
       p_special_craft.examine_price_id);
    ...
    BEGIN
      ...
      IF v_quotation_class_code IN
        ...
        p_exprice_rec.fabric_depart_examine_price_job_id     := 'M' ||
                                                                v_exprice_id;
        p_exprice_rec.fabric_depart_examine_price_job_status := 0;
        --核价部核价任务ID
    ...
    BEGIN
      FOR craft_rec IN (SELECT t.quotation_special_craft_detail_id,
        ...
        p_special_craft.examine_price_special_craft_detail_id := plm.pkg_plat_comm.f_get_keycode(v_table_name  => 'SPECIAL_CRAFT_EXAMINE_PRICE',
                                                                                                 ...
                                                                                                 v_serail_num  => 2);
        p_special_craft.related_colored_detail_id             := craft_rec.quotation_special_craft_detail_id;
        --关联加色核价特殊工艺明细ID
        IF craft_rec.related_colored_detail_id IS NOT NULL THEN
          SELECT MAX(t.examine_price_special_craft_detail_id)
           ...
           WHERE t.related_colored_detail_id =
                 craft_rec.related_colored_detail_id;
        ELSE
          v_related_colored_detail_id := NULL;
        END IF;
        p_special_craft.related_colored_quotation_detail_id := v_related_colored_detail_id;
  ...
  BEGIN
    ...
    VALUES
      (p_plm_f_rec.file_id,
       p_plm_f_rec.thirdpart_id,
       p_plm_f_rec.file_name,
       p_plm_f_rec.source_name,
       p_plm_f_rec.url,
       p_plm_f_rec.bucket,
       p_plm_f_rec.upload_time,
       p_plm_f_rec.file_type,
       p_plm_f_rec.file_blob,
       p_plm_f_rec.file_md5,
       p_plm_f_rec.file_size);
  ...
  BEGIN
    UPDATE plm_file t
       SET t.file_name   = p_plm_f_rec.file_name,
           ...
           t.file_md5    = p_plm_f_rec.file_md5,
