  ...
  --根据供应商档案id获取企业id
   ...
   BEGIN
     EXECUTE IMMEDIATE 'SELECT A.SUPPLIER_COMPANY_ID
                          ...
                          FROM SCMDATA.T_SUPPLIER_INFO A WHERE A.SUPPLIER_INFO_ID ='''||V_SUPINFO_ID||'''
                          AND A.COMPANY_ID='''||V_NEED_COMPID||''''
...
WHERE  A.QUOTATION_ID =  ']' || p_quotation_id ||
  ...
  BEGIN
   ...
   --二级分类
    ...
    SELECT name1
      INTO v_category2
      FROM (SELECT nvl(nvl(t4.company_dict_id, t3.company_dict_id),t2.company_dict_id) category_id,
  ...
  IF v_category2 IN ('MRP_MATERIAL_CLASSIFICATION00020001','MRP_MATERIAL_CLASSIFICATION00020002','MRP_MATERIAL_CLASSIFICATION00020003',
    'MRP_MATERIAL_CLASSIFICATION00020004') THEN
    --当【物料分类-二级分类】=【 针织/梭织/牛仔面料/毛呢类】时，
      --【供应商名称】+【供应商物料名称】+【供应商颜色】+【创建成品供应商编号】+【克重】在【MRP-外部物料档案】中相同时
    
    
   SELECT COUNT(1),MAX(A.MATERIAL_SPU) INTO v_flag2,v_exist_spu
  FROM MRP.MRP_OUTSIDE_MATERIAL_SPU A
 INNER JOIN MRP.MRP_OUTSIDE_MATERIAL_SKU B ON A.MATERIAL_SPU=B.MATERIAL_SPU
 INNER JOIN MRP.MRP_OUTSIDE_SUPPLIER_MATERIAL C ON C.MATERIAL_SKU=B.MATERIAL_SKU
...
ELSE
     ...
     INNER JOIN MRP.MRP_OUTSIDE_MATERIAL_SKU B ON B.MATERIAL_SKU = Z.MATERIAL_SKU AND B.CREATE_FINISHED_SUPPLIER_CODE=Z.CREATE_FINISHED_SUPPLIER_CODE
     INNER JOIN MRP.MRP_OUTSIDE_MATERIAL_SPU A ON A.MATERIAL_SPU = B.MATERIAL_SPU 
 ...
 END IF;
  ...
  BEGIN
    ...
    SELECT t1.company_dict_value category,
     ...
     LEFT JOIN scmdata.sys_company_dict t3
      ...
      LEFT JOIN scmdata.sys_company_dict t4
        ON t3.company_id = t4.company_id
       AND t4.company_dict_type = t3.company_dict_value
     WHERE nvl(nvl(t4.company_dict_id, t3.company_dict_id),t2.company_dict_id) =
           v_rec.material_classification;
    ...
    INSERT INTO plm.material_detail_quotation
      (material_detail_quotation_id, quotation_material_detail_no,QUOTATION_MATERIAL_TYPE,
       whether_inner_material, quotation_material_sku, quotation_unit_price,
       quotation_unit,
       --quotation_loss_rate,
    ...
    VALUES
      (v_material_id, v_num2,'面料', 0, v_sku_id,
       nvl(v_mijia, v_rec.supplier_large_good_net_price), v_unit2 || '/元', 0,
       (CASE WHEN v_rec.supplier_code = 'NEW' THEN v_supp_code ELSE
    ...
    SELECT t1.company_dict_value category,
     ...
     LEFT JOIN scmdata.sys_company_dict t3
      ...
      LEFT JOIN scmdata.sys_company_dict t4
        ON t3.company_id = t4.company_id
       AND t4.company_dict_type = t3.company_dict_value
     WHERE nvl(nvl(t4.company_dict_id, t3.company_dict_id),t2.company_dict_id) = v_cate_id;
...
ELSE
  ...
  BEGIN
    IF p_sup_material IS NULL THEN
  ...
  PROCEDURE p_calculate_inner_price_comparison_recommen(p_examine_price_id                 VARCHAR2,
    ...
    /*    --推荐结果记录
    TYPE recommend_result_rec IS RECORD(
      material_spu   VARCHAR2(32),
      recommendation NUMBER(6, 2));
    --数组
    TYPE recommend_result_type IS TABLE OF recommend_result_rec;
    recommend_result recommend_result_type := recommend_result_type();*/
    --推荐物料
...
BEGIN
 ...
 IF  v_flag2 >0 AND V_REC.MATERIAL_SKU IS NULL THEN 
   raise_application_error(-20002,
                              V_REC.SUPP_MATERIAL_NAME||'已存在 ' || v_exist_spu ||
                              '对应当前【物料名称】，请前往【SPU加色】或【SKU加供】进行操作');
...
ELSE   
  ...
  V_QUOTATION_ID := scmdata.pkg_variable.f_get_varchar(v_objid   => v_rec.create_id,
   ...
   INSERT INTO MRP.MRP_CREATE_OUTSIDE_ACCESSORIES_TEMP(AC_TEMP_ID,SUPPLIER_NAME,SUPP_MATERIAL_NAME,MATERIAL_CLASSIFICATION,UNIT,UNIT_PRICE,TYPE,APP_SITE,UNIT_CONSUMPTION,
   LOSS_RATE,AMOUNT,CONTACT_PHONE,MATERIAL_SKU,MATERIAL_SPECIFICATION,CREATE_FINISHED_PRODUCT_SUPPLIER_CODE,CREATE_ID,CREATE_TIME,QUO_ID,SUPPLIER_CODE,REMARKS)
   VALUES(V_REC.AC_TEMP_ID,V_REC.SUPPLIER_NAME,V_REC.SUPP_MATERIAL_NAME,V_REC.MATERIAL_CLASSIFICATION,V_REC.UNIT,V_REC.UNIT_PRICE,V_REC.TYPE,V_REC.APP_SITE,V_REC.UNIT_CONSUMPTION,
   V_REC.LOSS_RATE,V_REC.AMOUNT,V_REC.CONTACT_PHONE,V_REC.MATERIAL_SKU,V_REC.MATERIAL_SPECIFICATION,V_REC.CREATE_FINISHED_PRODUCT_SUPPLIER_CODE,V_REC.CREATE_ID,SYSDATE,V_QUOTATION_ID,V_REC.SUPPLIER_CODE,V_REC.REMARKS) ;
END IF;   
...
BEGIN
  ...
  FOR I IN (SELECT * FROM MRP.MRP_CREATE_OUTSIDE_ACCESSORIES_TEMP A 
   ...
   --先校验是否已存在物料
   SELECT COUNT(1),MAX(Z.MATERIAL_SKU)INTO V_FLAG,v_exist_sku 
     FROM MRP.MRP_OUTSIDE_SUPPLIER_MATERIAL Z 
    INNER JOIN MRP.MRP_TEMPORARY_SUPPLIER_ARCHIVES Y ON Z.SUPPLIER_CODE=Y.SUPPLIER_CODE
    INNER JOIN MRP.MRP_OUTSIDE_MATERIAL_SKU B ON B.MATERIAL_SKU = Z.MATERIAL_SKU AND B.CREATE_FINISHED_SUPPLIER_CODE=Z.CREATE_FINISHED_SUPPLIER_CODE
    INNER JOIN MRP.MRP_OUTSIDE_MATERIAL_SPU A ON A.MATERIAL_SPU = B.MATERIAL_SPU 
    ...
    END IF; 
       ...
       IF I.SUPPLIER_CODE='NEW'  THEN
         ...
         --IF I.SUPPLIER_NAME='未填写' THEN
          ...
          SELECT COUNT(1),MAX(A.CONTACT_PHONE),MAX(a.supplier_code) INTO V_NUM,V_PHONE,V_MSUPP
            FROM MRP.MRP_TEMPORARY_SUPPLIER_ARCHIVES A
           ...
           ELSE
            ...
            VALUES(V_MSUPP,'ODM供应商报价', 1,i.supplier_name,V_SUPP_ID,'',i.contact_phone,'',i.create_time,i.create_id,0);
            --创建外部供应商日志
      INSERT INTO mrp.mrp_log
        (id, user_name, operate_content, operate, create_time, class_name,
         method_name, thirdpart_id)
    ...
    END IF; 
        ...
        spu_id     := plm.pkg_plat_comm.f_get_keycode(v_table_name  => 'MRP.MRP_OUTSIDE_MATERIAL_SPU',
                                                  ...
                                                  v_serail_num  => 4);
        INSERT INTO MRP.MRP_OUTSIDE_MATERIAL_SPU
          (MATERIAL_SPU,FEATURES,
           MATERIAL_NAME,
           UNIT,
           MATERIAL_CLASSIFICATION,
           MATERIAL_SPECIFICATIONS,
           CREATE_FINISHED_SUPPLIER_CODE,
           CREATE_TIME,
           CREATE_ID,
           WHETHER_DEL)
        VALUES
          (SPU_ID,SPU_ID,
           I.SUPP_MATERIAL_NAME,
            SUBSTR(I.UNIT,0,INSTR(I.UNIT,'/')-1),
           I.MATERIAL_CLASSIFICATION,
        ...
        v_sku_id := plm.pkg_plat_comm.f_get_keycode(v_table_name  => 'MRP.MRP_OUTSIDE_MATERIAL_SKU',
                                                v_column_name => 'MATERIAL_SKU',
                                                v_pre         => spu_id || '0' || to_char(SYSDATE,'YY'),
                                                v_serail_num  => 3);
                                                
        --外部供应商物料      
        INSERT INTO MRP.MRP_OUTSIDE_SUPPLIER_MATERIAL
          (SKU_ABUTMENT_CODE,
           SUPPLIER_CODE,COLOR_CARD_PICTURE,
           MATERIAL_SKU,MATERIAL_SOURCE,
           SUPPLIER_MATERIAL_NAME,
           SUPPLIER_COLOR,
           SUPPLIER_SHADES,supplier_material_status,
           OPTIMIZATION,create_finished_supplier_code, create_time, creater, whether_del, 
           SUPPLIER_LARGE_GOOD_QUOTE,SUPPLIER_LARGE_GOOD_NET_PRICE 
           )
        VALUES
          ((CASE WHEN I.SUPPLIER_CODE = 'NEW' THEN V_MSUPP ELSE
           ...
           '白色',
           '',1,
           1,V_SUPP_ID,SYSDATE,i.create_id,0,
           I.UNIT_PRICE,V_DAHUO_JINJIA);
   
         INSERT INTO MRP.MRP_OUTSIDE_MATERIAL_SKU
           (MATERIAL_SKU, MATERIAL_COLOR,MATERIAL_SPU,SKU_STATUS,COLOR_PICTURE,
            CREATE_TIME, CREATE_FINISHED_SUPPLIER_CODE,
            WHETHER_DEL, CREATE_ID,
            PREFERRED_NET_PRICE_OF_LARGE_GOOD)
         VALUES
           (V_SKU_ID, '白色', SPU_ID, 1,V_SKU_ID,
            SYSDATE,  V_SUPP_ID,  0,  I.CREATE_ID,
            V_DAHUO_JINJIA);
       ...
       ELSE
        --已有的供应商
          --供应商信息
         SELECT Y.CONTACT_PHONE INTO V_PHONE
           FROM MRP.MRP_TEMPORARY_SUPPLIER_ARCHIVES Y
        ...
        IF NVL(V_PHONE,1) <> (I.CONTACT_PHONE) THEN 
         ...
         WHERE u.supplier_code = I.supplier_code
          ...
          --物料部分
        SELECT A.MATERIAL_SPU,A.MATERIAL_SPECIFICATIONS,A.MATERIAL_NAME,C.SUPPLIER_LARGE_GOOD_NET_PRICE,C.SUPPLIER_LARGE_GOOD_QUOTE,C.SUPPLIER_MATERIAL_NAME
     ...
     WHERE t.quotation_id = i.quo_id;
       ...
       quotation_loss_rate,
       quotation_unit_price_consumption,
       quotation_amount, quotation_material_supplier_code, quotation_id,QUOTATION_REMARK,
       create_time, create_id, update_time, update_id, whether_del)
       VALUES 
       (v_material_id,v_no,I.TYPE,I.APP_SITE,0,(CASE WHEN i.supplier_code='NEW'THEN v_sku_id 
       ELSE I.MATERIAL_SKU END),I.UNIT_PRICE,SUBSTR(I.UNIT,0,INSTR(I.UNIT,'/')-1),I.LOSS_RATE,I.UNIT_CONSUMPTION,I.AMOUNT,(CASE WHEN i.supplier_code='NEW'THEN V_MSUPP
         ELSE i.supplier_code END),i.quo_id,I.REMARKS,
        SYSDATE, i.create_id,SYSDATE,i.create_id,0);
       END IF;   
       
      DELETE FROM  MRP.MRP_CREATE_OUTSIDE_ACCESSORIES_TEMP t WHERE T.AC_TEMP_ID=I.AC_TEMP_ID;
      scmdata.pkg_plat_log.p_record_plat_log(p_company_id         => 'b6cc680ad0f599cde0531164a8c0337f',
                                           p_apply_module       => 'a_quotation_110',
                                           p_base_table         => 'mrp.mrp_outside_material_spu',
                                           ...
                                           p_log_type           => '01',
                                           p_log_msg            => '创建新物料:' ||
                                                                   (CASE WHEN i.supplier_code='NEW'THEN v_sku_id  ELSE I.MATERIAL_SKU END),
                                           p_operate_field      => 'MATERIAL_DETAIL_QUOTATION_ID',
                                           p_field_type         => 'VARCHAR2',
                                           p_old_code           => NULL,
                                           p_new_code           => NULL,
