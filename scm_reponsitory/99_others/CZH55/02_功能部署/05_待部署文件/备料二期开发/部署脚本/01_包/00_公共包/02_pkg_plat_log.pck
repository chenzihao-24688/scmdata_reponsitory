CREATE OR REPLACE PACKAGE scmdata.pkg_plat_log IS
  ...
  PROCEDURE p_insert_plat_log_to_plm_or_mrp(p_log_id      VARCHAR2,
                                            ...
                                            p_type        INT DEFAULT 0);
  --获取日志字典名称
  FUNCTION f_get_log_type_dict_name(p_log_type VARCHAR2) RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY scmdata.pkg_plat_log IS
  ...
  *============================================*/
  ...
  BEGIN
    ...
    IF p_type IN (1, 2) THEN
      v_method_name := f_get_log_type_dict_name(p_log_type => p_log_type);
  ...
  *============================================*/
  ...
  BEGIN
    ...
    IF p_type IN (1, 2) THEN
      ...
      v_method_name := f_get_log_type_dict_name(p_log_type => v_log_type);
  ...
  *============================================*/
  ...
  --获取日志字典名称
  FUNCTION f_get_log_type_dict_name(p_log_type VARCHAR2) RETURN VARCHAR2 IS
    v_method_name VARCHAR2(256);
  BEGIN
    SELECT MAX(a.group_dict_name)
      INTO v_method_name
      FROM scmdata.sys_group_dict a
     WHERE a.group_dict_type = 'QUOTATION_LOG'
  ...
  * P_MEMO : 备注编码（选填）
  * P_MEMO_DESC : 备注描述（选填）
  * P_TYPE : 是否需要同步plm、mrp。0：scm ，1：plm，2：mrp
  *============================================*/
