...
BEGIN
  V_SQL := q'[SELECT A.PRODUCT_ORDER_ID, --生产单号
       ...
       A.BATCH_FINISH_PERCENT*100 || '%' CP_FINISHED_RATE_N, --完成率
...
BEGIN
  V_SQL :=q'[SELECT A.PREPARE_ORDER_ID, --备料单号
       (CASE  WHEN A.PREPARE_STATUS IN (1, 2, 4) AND
         ...
         ELSE '否'   END) CR_IS_DELAY_N, --是否逾期       
       A.ORDER_NUM CP_ORDER_NUM_N, --订单量
       B.BATCH_FINISH_PERCENT*100 || '%' CP_FINISHED_RATE_N, --完成率
       A.ORDER_NUM * (B.BATCH_FINISH_PERCENT) COMPLETE_AMOUNT_PR, --完成数量
       D.STYLE_NUMBER, --款号
       D.RELA_GOO_ID, --货号
       A.GOODS_SKC CR_GOODS_SKC_N, --SKC
       C.COLORNAME, --颜色
  ...
  UPDATE MRP.GREY_PREPARE_PRODUCT_ORDER T
   ...
   WHERE T.PRODUCT_ORDER_ID =  PI_PROID;    
    SCMDATA.PKG_PLAT_LOG.P_RECORD_PLAT_LOG(P_COMPANY_ID         => 'b6cc680ad0f599cde0531164a8c0337f',
                                         ...
                                         P_LOG_TYPE           => '06',
                                         P_LOG_MSG            => '完成生产单号:' ||
                                                                 PI_PROID||';完成数量:'||PI_AMOUNT||'【操作前:' || v_old_batch_finish_num || '】；' || chr(10) ||
                                                                      (CASE
                                         ...
                                         P_NEW_CODE           => 2,
                                         P_OLD_VALUE          => '生产中',
                                         P_NEW_VALUE          => (CASE WHEN PI_ISFINISH = 1 THEN '已完成'  ELSE   '生产中'  END),
                                         P_USER_ID            => PI_USERID,
       ...
       scmdata.pkg_plat_log.p_insert_plat_log_to_plm_or_mrp(p_log_id      => VO_LOG_ID,
                                                        p_log_msg     => '完成生产单号:' ||PI_PROID||';完成数量:'||PI_AMOUNT,
                                                        p_class_name  => '坯布备料单管理',
                                                        p_method_name => '完成订单',
                                                        p_type        => 2)  ;                                   
...
V_SGIOB_REC2.NUM :=  ABS(V_BLFENPI);--数量
 ...
 --坯布库存明细
  --用【成品供应商编号】+【物料供应商编号】+【物料SPU】+【单位】入参，判断在【供应商库存-坯布仓库存明细】中是否对应数据
   ...
   ELSE 
    ...
    P_SUPPL_REC.TOTAL_STOCK := V_BLFENPI ;--总库存数
    P_SUPPL_REC.BRAND_STOCK := V_BLFENPI ;--品牌仓库存数
...
V_SGIOB_REC.NUM :=  ABS(V_BLFENPI);--数量
...
P_MATER_REC.TOTAL_STOCK :=  V_BLFENPI;--总库存数
P_MATER_REC.BRAND_STOCK :=  V_BLFENPI;--品牌仓库存数
