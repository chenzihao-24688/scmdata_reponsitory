CREATE OR REPLACE PACKAGE pkg_color_prepare_order_sup IS
  ...
  FUNCTION f_check_color_fabric_is_enough(p_brand_stock      NUMBER,
...
END pkg_color_prepare_order_sup;
/
CREATE OR REPLACE PACKAGE BODY pkg_color_prepare_order_sup IS
       ...
       va.order_num               cr_order_num_n, --订单数量
               ...
               WHEN p_prepare_status IN (3) THEN
                ' va.finish_num cr_finish_num_n, --已完成数量
                  (va.finish_num/va.order_num)*100 cr_finish_rate_n, --完成率'
               ELSE
                NULL
             END) || q'[
 ...
 ORDER BY va.order_time DESC
  ...
  BEGIN
    v_sql := q'[SELECT t.product_order_id, --色布生产单号
       t.supplier_material_name cr_supplier_material_name_n, --供应商物料名称
       decode(sign(to_number(SYSDATE - va.min_expect_arrival_time)),
              1,
              '是',
              '否') cr_is_delay_n, --是否逾期 待做
       t.features cr_features_n, --特征图，图片ID，第一张
       t.supplier_color cr_supplier_color_n, --供应商颜色
       t.supplier_shades cr_supplier_shades_n, --供应商色号
       t.unit cr_unit_n, --单位
       t.contain_color_prepare_num cr_contain_color_prepare_num_n, --含色布备料单数
       t.plan_product_quantity cr_plan_product_quantity_n, --计划生产数量
       t.batch_finish_num cr_batch_finish_num_n, --分批完成累计数量
       t.batch_finish_percent cr_batch_finish_percent_n, --分批完成累计百分比
 ...
 ORDER BY t.receive_time DESC
  ...
  BEGIN
    ...
    IF v_brand_stock > 0 THEN
      ...
      v_num := f_check_color_fabric_is_enough(p_brand_stock      => v_brand_stock,
  ...
  BEGIN
    ...
    END IF;
    RETURN v_num;
  END f_check_color_fabric_is_enough;
  ...
  BEGIN
    ...
    --校验“供应商仓坯布库存”转化为色布是否够用
    v_sup_num := f_check_color_fabric_is_enough(p_brand_stock      => p_brand_stock,
                                                p_plan_product_num => p_cpop_rec.plan_product_quantity,
  ...
  BEGIN
    ...
    mrp.pkg_color_prepare_product_order.p_check_more_less_clause(p_cur_finished_num     => p_cur_finished_num,
                                                                 p_unfinished_num       => v_cppo_rec.complete_num,
                                                                 p_rate                 => 0.03,
    ...
    BEGIN
      ...
      v_cpbfo_rec.batch_finish_num        := p_cur_finished_num; --分批完成数量
      v_cpbfo_rec.batch_finish_percent    := round(p_cur_finished_num /
  ...
  BEGIN
    ...
    IF v_flag > 0 THEN
      UPDATE mrp.supplier_color_cloth_stock t
         SET t.total_stock = t.total_stock + p_sciob_rec.num,
             t.brand_stock = t.brand_stock + p_sciob_rec.num,
             t.update_id   = p_user_id,
             t.update_time = SYSDATE
    ...
    ELSE
      ...
      v_sccs_rec.total_stock          := v_sccs_rec.total_stock +
                                         p_sciob_rec.num; --总库存数
      v_sccs_rec.brand_stock          := v_sccs_rec.brand_stock +
                                         p_sciob_rec.num; --品牌仓库存数
  ...
  BEGIN
    ...
    IF v_flag > 0 THEN
      UPDATE mrp.material_color_cloth_stock t
         SET t.total_stock = t.total_stock + p_mciob_rec.num,
             t.brand_stock = t.brand_stock + p_mciob_rec.num,
             t.update_id   = p_user_id,
             t.update_time = SYSDATE
    ...
    ELSE
      ...
      v_mccs_rec.total_stock          := v_mccs_rec.total_stock +
                                         p_mciob_rec.num; --总库存数
      v_mccs_rec.brand_stock          := 0; --品牌仓库存数
      v_mccs_rec.supplier_stock       := v_mccs_rec.supplier_stock +
                                         p_mciob_rec.num; --供应商仓库存数
