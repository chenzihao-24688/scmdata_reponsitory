...
CREATE OR REPLACE PACKAGE BODY scmdata.pkg_db_job IS
  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT MAX(fc.inside_supplier_code),
    ...
    IF v_insup_code IN ('30928', '73036') THEN
      vo_group_name  := NULL;
      vo_area_leader := NULL;
      ...
      USING (
        ...
        SELECT DISTINCT *
          FROM (SELECT to_number(extract(YEAR FROM po.delivery_date)) YEAR,
                       ...
                       (CASE
                         WHEN sp_b.inside_supplier_code NOT IN
                              ('30928', '73036') THEN
                          (CASE
          ...
          UPDATE
             SET pta.year                   = ptb.year,
                 ...
                 WHEN NOT MATCHED THEN 
                   INSERT(pta.pt_ordered_id, pta.company_id, pta.order_id, pta.year, pta.quarter, pta.month, pta.supplier_code, 
                   pta.supplier_company_name, pta.factory_code, pta.factory_company_name, pta.product_gress_code, pta.goo_id, 
                   pta.category, pta.category_name, pta.product_cate, pta.coop_product_cate_name, pta.samll_category, pta.product_subclass_name,
                   pta.style_name, pta.style_number, pta.flw_order, pta.flw_order_manager, pta.qc, pta.qc_manager, pta.is_twenty, pta.delivery_status,
                   pta.is_quality, pta.actual_delay_days, pta.delay_section, pta.is_sup_duty, pta.responsible_dept, pta.responsible_dept_sec, 
                   pta.delay_problem_class, pta.delay_cause_class, pta.delay_cause_detailed, pta.problem_desc, pta.purchase_price, pta.fixed_price,
                   pta.order_amount, pta.est_arrival_amount, pta.delivery_amount, pta.satisfy_amount, pta.order_money, pta.delivery_money,
                   pta.satisfy_money, pta.delivery_date, pta.order_create_date,
                 --pta.arrival_date,
                 --pta.sort_date,
                  pta.is_first_order, pta.remarks, pta.order_finish_time, pta.create_id, pta.create_time, pta.update_id, pta.update_time, 
                  pta.goo_id_pr, pta.sho_id,pta.group_name,pta.area_locatioin,pta.area_gp_leader,pta.latest_planned_delivery_date,pta.is_product_order) 
