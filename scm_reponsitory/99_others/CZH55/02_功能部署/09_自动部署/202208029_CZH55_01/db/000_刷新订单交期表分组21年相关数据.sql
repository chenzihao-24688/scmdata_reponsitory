  ...
  USING (
    ...
    SELECT DISTINCT *
      FROM (SELECT (CASE
        ...
        ON (pta.company_id = ptb.company_id AND pta.order_id = ptb.order_id AND pta.year = 2021) WHEN
     MATCHED THEN
      UPDATE
...
/
BEGIN
 UPDATE scmdata.pt_ordered t SET t.is_sup_duty = NULL WHERE t.responsible_dept IS NULL AND t.is_sup_duty IS NOT NULL; 
END;
/
