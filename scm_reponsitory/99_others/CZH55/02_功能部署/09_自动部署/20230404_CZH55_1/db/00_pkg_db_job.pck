...
CREATE OR REPLACE PACKAGE BODY scmdata.pkg_db_job IS
  ...
  *============================================*/
  ...
  BEGIN
    ...
    --1）当月月初至当月5号 同步上月订单
    v_date := CASE
                WHEN trunc(SYSDATE) BETWEEN p_begin_date AND
  ...
  BEGIN
    ...
    IF to_char(SYSDATE, 'dd') < '10' THEN
      ...
      IF to_char(SYSDATE, 'yyyy-mm-dd') = v_seal_day THEN
        --1.月度__每月执行
        ----1)kpi指标报表——维度:分类
        pkg_kpipt_order_proc.p_t_kpiorder_month_proc(t1_type => 0,
                                                     p1_type => 1);
        ----2)kpi指标报表——维度:区域组
        pkg_kpipt_order_proc.p_t_kpiorder_qu_month_proc(t1_type => 0,
                                                        p1_type => 1);
        ----3)总结类报表
        ...
        IF to_char(SYSDATE, 'mm') = '01' THEN
          --2.季度__每季度执行
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_quarter_proc(t1_type => 0,
                                                         p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_quarter_proc(t1_type => 0,
                                                            p1_type => 1);
          ----3)总结类报表
          ...
          --3.半年度
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_halfyear_proc(t1_type => 0,
                                                          p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_halfyear_proc(t1_type => 0,
                                                             p1_type => 1);
          ----3)总结类报表
          pkg_indicator_reached.p_indicator_reached_halfyear;
          ----4)多维度交期指标查询报表
          scmdata.pkg_kpipt_order.p_t_kpi_halfyear(t_type => 0,
                                                   p_type => 1);
          ----5)供应商绩效盘点报表——维度:供应商
          ...
          --4.年度
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_year_proc(t1_type => 0,
                                                      p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_year_proc(t1_type => 0,
                                                         p1_type => 1);
          ----3)总结类报表
