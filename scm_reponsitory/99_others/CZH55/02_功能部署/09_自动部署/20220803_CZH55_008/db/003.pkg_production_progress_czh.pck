      ...
      IF v_suc_count = v_pr_tol THEN
        ...
        SELECT MAX(province || city || county) location_area,
          ...
          LEFT JOIN scmdata.t_supplier_group_config sg
           ...
           AND sg.pause = 1
         WHERE sp.supplier_code = p_pro_rec.supplier_code
  ...
  BEGIN
    ...
    if v_deds_flag > 0 then
     if v_product_order = 0 then
       update scmdata.t_ordered po
          set po.approve_status = '01',po.is_free_deduction = '1'
        where po.company_id = p_company_id
          and po.order_code = p_order_id;
     else 
       update scmdata.t_ordered po
  ...
  BEGIN
    ...
    select count(1)
     ...
     where not exists (select 1
             ...
             where tr.responsible_dept not in
                   (select t.company_dept_id
                      from scmdata.sys_company_dept t
                     where t.company_dept_id in
       ...
       and not exists (select 1
              from (SELECT SUM(td.actual_discount_price) sum_price
                     ...
                     WHERE td.company_id = po.company_id
                       AND td.order_id = po.order_id) ta
             where sum_price = 0)
