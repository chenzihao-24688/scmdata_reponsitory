...
BEGIN
  FOR order_rec IN (SELECT t.order_id, t.company_id, t.factory_code
                     ...
                     WHERE t.order_finish_time BETWEEN
    ...
    IF v_group_leader IS NULL THEN
      SELECT MAX(t.area_group_leader)
        INTO v_group_leader
        FROM scmdata.t_supplier_group_config t
       WHERE t.group_config_id = v_group_name;
    
      UPDATE scmdata.t_supplier_info a
         SET a.area_group_leader = v_group_leader
       WHERE a.company_id = order_rec.company_id
         AND a.supplier_code = order_rec.factory_code;
    END IF;
