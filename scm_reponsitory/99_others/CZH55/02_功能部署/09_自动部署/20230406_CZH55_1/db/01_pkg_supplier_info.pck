CREATE OR REPLACE PACKAGE scmdata.pkg_supplier_info IS
  ...
  --合作范围  begin
  --select_sql
  FUNCTION f_query_sup_coop_list(p_item_id VARCHAR2, p_supp_id VARCHAR2)
    RETURN CLOB;
  --insert sql
  FUNCTION f_insert_sup_coop_list(p_item_id VARCHAR2, p_supp_id VARCHAR2)
    RETURN CLOB;
  --update sql
  FUNCTION f_update_sup_coop_list(p_item_id VARCHAR2, p_supp_id VARCHAR2)
    RETURN CLOB;
  --delete sql
  FUNCTION f_delete_sup_coop_list(p_item_id VARCHAR2, p_supp_id VARCHAR2)
    RETURN CLOB;
  --合作范围  end
  --合作工厂 begin 'a_supp_151_7'
  --select_sql
  FUNCTION f_query_coop_factory_list(p_item_id VARCHAR2,
                                     p_supp_id VARCHAR2) RETURN CLOB;
  --insert sql
  FUNCTION f_insert_coop_factory_list(p_item_id VARCHAR2,
                                      p_supp_id VARCHAR2) RETURN CLOB;
  --获取合作工厂 pick
  ...
  * Obj_Name    : p_check_t_supplier_info
  * Arg_Number  : 1
  * p_supplier_info_id : 当前供应商编号
  * p_default_company_id ： 默认企业编号
  *============================================*/
  PROCEDURE p_check_t_supplier_info(p_supplier_info_id VARCHAR2);
  --校验统一信用代码
  PROCEDURE p_check_social_credit_code(p_sp_data scmdata.t_supplier_info%ROWTYPE);

  --校验供名称在准入流程中是否重复
  PROCEDURE p_check_sup_name(p_sp_data scmdata.t_supplier_info%ROWTYPE);
  --校验供应商简称在准入流程中是否重复
  PROCEDURE p_check_supabbr_name(p_sp_data scmdata.t_supplier_info%ROWTYPE);
  ...
  * ALERTER_TIME  :
  * Purpose  : （新增，更新）保存时校验
  * Obj_Name    : p_check_save_t_supplier_info
  * Arg_Number  : 2
  * P_SP_DATA : 供应商档案数据
  ...
  *============================================*/
  ...
  PROCEDURE p_check_save_t_supplier_info(p_sp_data scmdata.t_supplier_info%ROWTYPE);
  --校验工厂信息
  ...
  *============================================*/
  FUNCTION f_get_supplier_code_by_rule(p_supplier_info_id   VARCHAR2,
  ...
  *============================================*/
  ...
  PROCEDURE p_create_t_supplier_info(p_company_id      VARCHAR2,
                                     p_factory_ask_id  VARCHAR2,
                                     p_user_id         VARCHAR2,
  ...
  * Obj_Name    : P_SUBMIT_T_SUPPLIER_INFO
  * Arg_Number  : 3
  * P_SUPPLIER_INFO_ID :供应商档案编号
  * p_default_company_id ：
  * p_user_id ：
  *============================================*/
  PROCEDURE p_submit_t_supplier_info(p_supplier_info_id   VARCHAR2,
                                     p_default_company_id VARCHAR2,
                                     p_user_id            VARCHAR2);
  ...
  *============================================*/
  ...
  PROCEDURE p_insert_supplier_info(p_sp_data scmdata.t_supplier_info%ROWTYPE);
  /*============================================*
  * Author   : SANFU
  * Created  : 2020-09-28 10:15:31
  * ALERTER  :
  ...
  *============================================*/
  ...
  PROCEDURE p_update_supplier_info(p_sp_data t_supplier_info%ROWTYPE);
  ...
  *============================================*/
  ...
  PROCEDURE p_delete_supplier_info(p_supplier_info_id   VARCHAR2,
                                   p_default_company_id VARCHAR2);
  /*校验合同批量导入*/
...
WITH dic AS
 (SELECT group_dict_value,
   ...
   WHERE pause = 0)
SELECT v.supplier_company_name         sp_sup_com_name_n,
       v.supplier_company_abbreviation sp_sup_com_abbr_name_n,       
       v.coop_classification           sp_coop_class_n,
       v.sp_coop_class_desc_n,
       v.sp_coop_model_n,
       v.company_type                  sp_company_type_n,  
       v.social_credit_code            sp_social_credit_code_n,   
       v.ask_date                      fa_ask_date_n,
       v.check_date                    fr_check_date_n,
       decode(v.origin_val,'AA',v.create_date,null)    sp_audit_date_n,
       v.admit_result                  sp_admit_result_n,
       v.supplier_info_id,
       v.supplier_info_origin,
       v.factory_ask_id
  FROM (SELECT sp.supplier_company_name,
               ...
               (SELECT listagg(DISTINCT t.group_dict_name, ';') within GROUP(ORDER BY t.group_dict_value)
                 ...
                 WHERE t.group_dict_type = sp.cooperation_type
                   AND instr(';' || va.coop_classification || ';',
                             ';' || t.group_dict_value || ';') > 0) sp_coop_class_desc_n,
               (SELECT listagg(group_dict_name, ';') within GROUP(ORDER BY group_dict_value)
                 ...
                 WHERE group_dict_type = 'SUPPLY_TYPE'
                   AND instr(';' || sp.cooperation_model || ';',
                             ';' || group_dict_value || ';') > 0) sp_coop_model_n,
               ...
               sp.admit_result,
               sp.supplier_info_id,
               sp.supplier_info_origin origin_val,
               e.group_dict_name supplier_info_origin,
               fa.factory_ask_id
...
SELECT v.supplier_info_id,
  ...
  FROM (SELECT e.group_dict_name supplier_info_origin,
               ...
               f.group_dict_name company_type,
               sp.group_name,
               dp1.province||dp2.city||dp3.county pcc,
               dp4.vill company_vill_desc
          FROM scmdata.t_supplier_info sp
          LEFT JOIN scmdata.t_factory_ask fa
            ON sp.supplier_info_origin_id = fa.factory_ask_id
          LEFT JOIN scmdata.t_factory_report fr
          ...
          LEFT JOIN (SELECT listagg(DISTINCT sa.coop_classification, ';') within GROUP(ORDER BY sa.coop_classification) coop_classification,
                ...
                ON va.supplier_info_id = sp.supplier_info_id
               AND va.company_id = sp.company_id
          left join dic_province dp1 on sp.company_province=dp1.provinceid
          left join dic_city dp2 on sp.company_city=dp2.cityno
          left join dic_county dp3 on sp.company_county=dp3.countyid
          left join dic_village dp4 on sp.company_vill=dp4.villid 
         WHERE sp.company_id = %default_company_id%
           AND sp.status = 1
 ...
 WHERE tc.supplier_info_id = ']' || p_supp_id || q'['
  ...
  v_supp_id VARCHAR2(32) := ']' || p_supp_id || q'[';
BEGIN
...
END;]';
 ...
 LEFT JOIN group_dict gde ON gde.group_dict_type = 'FA_PRODUCT_TYPE' AND gde.group_dict_value = sp.product_type
 WHERE f.supplier_info_id = ']' || p_supp_id || q'['
 ...
 ORDER BY f.factory_type ASC]';
  ...
  BEGIN
    v_insert_sql := q'[DECLARE
      ...
      v_supp_id VARCHAR2(32) := ']' || p_supp_id ||
                    q'[';
    BEGIN
...
END;}';
  ...
  BEGIN
    ...
    VALUES
      (p_fac_rec.coop_factory_id, p_fac_rec.company_id,
       p_fac_rec.supplier_info_id, p_fac_rec.fac_sup_info_id,
       p_fac_rec.factory_code, p_fac_rec.factory_name,
       p_fac_rec.factory_type, p_fac_rec.pause, p_fac_rec.create_id,
       p_fac_rec.create_time, p_fac_rec.update_id, p_fac_rec.update_time,
       p_fac_rec.memo, nvl(p_fac_rec.pause_type, 'COOP_UNPAUSE'));
  ...
  * p_default_company_id ： 默认企业编号
  *============================================*/
  PROCEDURE p_check_t_supplier_info(p_supplier_info_id VARCHAR2) IS
  ...
  BEGIN
    ...
    --2.提交 =》校验数据是否有效
    p_check_save_t_supplier_info(p_sp_data => supp_info_rec);
  ...
  END p_check_t_supplier_info;
  --校验统一信用代码
  --p_company_id :企业ID
  --p_supplier_info_id ：供应商编号 主键
  --p_social_credit_code ：统一社会信用代码
  PROCEDURE p_check_social_credit_code(p_sp_data scmdata.t_supplier_info%ROWTYPE) IS
  BEGIN
  ...
  --校验供名称
  PROCEDURE p_check_sup_name(p_sp_data scmdata.t_supplier_info%ROWTYPE) IS
  BEGIN
    --1 供应商名称不能为空
    ...
    ELSE
      ...
      END IF;
    END IF;
  END p_check_sup_name;

  --校验供应商简称
  PROCEDURE p_check_supabbr_name(p_sp_data scmdata.t_supplier_info%ROWTYPE) IS
  BEGIN
    IF p_sp_data.supplier_company_abbreviation IS NULL THEN
      raise_application_error(-20002, '供应商简称不能为空！');
    ELSE
      --供应商档案：供应商名称可编辑。
      --1） 限制仅能填写中文及中文括号；
      IF pkg_check_data_comm.f_check_varchar(pi_data => p_sp_data.supplier_company_abbreviation,
                                             pi_type => 0) <> 1 THEN
        raise_application_error(-20002,
                                '供应商简称填写错误，仅能填写中文及中文括号！');
      END IF;
    END IF;
  END p_check_supabbr_name;
  ...
  *============================================*/
  ...
  PROCEDURE p_check_save_t_supplier_info(p_sp_data scmdata.t_supplier_info%ROWTYPE) IS
  BEGIN
    --校验基础数据
    --1 供应商名称
    p_check_sup_name(p_sp_data => p_sp_data);
    --2.供应商简称 
    p_check_supabbr_name(p_sp_data => p_sp_data);
    --3.手动新增 且 待建档
    IF p_sp_data.supplier_info_origin = 'MA' AND p_sp_data.status = 0 THEN
      --校验 统一社会信用代码
      p_check_social_credit_code(p_sp_data => p_sp_data);
    ELSE
      NULL;
    END IF;
    --4.校验流程字段在流程中是否重复
    scmdata.pkg_ask_record_mange.p_check_flow_fields_is_repeat(p_company_id     => p_sp_data.company_id,
                                                               p_object_id      => p_sp_data.supplier_info_id,
                                                               p_flow_field     => 'COMPANY_NAME;COMPANY_ABBREVIATION;SOCIAL_CREDIT_CODE',
                                                               p_flow_field_val => p_sp_data.supplier_company_name || ';' ||
    ...
    ELSE
      IF p_sp_data.cooperation_model <> 'OF' AND
         (p_sp_data.pay_term IS NULL OR instr(p_sp_data.pay_term, ' ') > 0) THEN
        raise_application_error(-20002,
    ...
    END IF;
  END p_check_save_t_supplier_info;
  ...
  *============================================*/
  ...
  END f_get_supplier_code_by_rule;
  --通过区域pick 分部获取分组配置id
  FUNCTION f_get_category_config_by_pick(p_company_id VARCHAR2,
                                         p_supp_id    VARCHAR2,
                                         p_province   VARCHAR2,
  ...
  BEGIN
    FOR sup_rec IN (SELECT t.company_province,
      ...
      vo_group_config_id := pkg_supplier_info.f_get_group_config_id(p_company_id => sup_rec.company_id,
                                                                    p_supp_id    => sup_rec.supplier_info_id,
                                                                    p_is_by_pick => 1,
      ...
      --一致，则更新
      --不一致，不做处理
      IF vo_group_config_id IS NOT NULL THEN
        IF p_group_config_id = vo_group_config_id THEN
  ...
  BEGIN
    --获取供应商分组
    SELECT MAX(t.group_name), MAX(t.group_name_origin)
      INTO v_group_name, v_origin
      FROM scmdata.t_supplier_info t
     WHERE t.supplier_info_id = p_supplier_info_id
       AND t.company_id = p_company_id;
    --判断分组配置是否停用
    --是，则判断 供应商分组存值是自动还是手动存值
    --手动存值，不做处理
    --自动存值，赋值为空
    IF p_pause = 0 THEN
      IF v_origin = 'MA' THEN
    ...
    UPDATE scmdata.t_supplier_info t
       SET t.group_name        = vo_group_config_id,
           t.group_name_origin = CASE
                                   WHEN p_pause = 0 AND v_origin = 'MA' THEN
                                    'MA'
                                   ELSE
                                    'AA'
                                 END,
  ...
  * ALERTER_TIME  :
  * Purpose  : 生成未建档的供应商档案
  * Obj_Name    : P_CREATE_T_SUPPLIER_INFO
  * Arg_Number  : 3
  * P_COMPANY_ID : 当前企业编号
  * P_FACTORY_ASK_ID : 申请单编号
  * p_user_id:当前登录用户编号
  *============================================*/
  ...
  PROCEDURE p_create_t_supplier_info(p_company_id      VARCHAR2,
    ...
    iv_supp_info               scmdata.t_supplier_info%ROWTYPE; --供应商信息 
    fask_rec                   scmdata.t_factory_ask%ROWTYPE; --验厂申请
    fr_rec                     scmdata.t_factory_report%ROWTYPE; --验厂报告  
    iv_sp_scope_info           scmdata.t_coop_scope%ROWTYPE; --验厂申请  供应商信息 合作范围
    v_flag                     INT;
    v_factrory_ask_flow_status VARCHAR2(48);
  BEGIN
    --数据源
    --验厂申请单
    SELECT COUNT(1), MAX(fa.factrory_ask_flow_status)
      INTO v_flag, v_factrory_ask_flow_status
      FROM scmdata.t_factory_ask fa
     WHERE fa.factory_ask_id = p_factory_ask_id
       AND fa.company_id = p_company_id;
  
    --判断验厂申请单是否存在
    IF v_flag > 0 THEN
      IF v_factrory_ask_flow_status IN ('FA12', 'FA31') THEN
        SELECT fa.*
          INTO fask_rec
          FROM scmdata.t_factory_ask fa
         WHERE fa.factory_ask_id = p_factory_ask_id
        ...
        --基本信息
        iv_supp_info.supplier_company_name         := fask_rec.company_name; --公司名称
        iv_supp_info.supplier_company_abbreviation := fask_rec.company_abbreviation; --公司简称
        iv_supp_info.social_credit_code            := fask_rec.social_credit_code; --统一社会信用代码
        --iv_supp_info.supplier_code                 := fask_rec.supplier_code; --供应商编号
        --iv_supp_info.inside_supplier_code  := fask_rec.inside_supplier_code; --内部供应商编号        
        iv_supp_info.company_regist_date := fask_rec.company_regist_date; --公司注册日期
        iv_supp_info.company_province    := fask_rec.company_province; --公司省
        iv_supp_info.company_city        := fask_rec.company_city; --公司市
        iv_supp_info.company_county      := fask_rec.company_county; --公司区
        iv_supp_info.company_vill        := fask_rec.company_vill; --公司乡镇
        iv_supp_info.company_address     := fask_rec.company_address; --公司地址
        --iv_supp_info.group_name            := fask_rec.group_name; --分组名称（v9.9版 存值改为分组ID）
        iv_supp_info.legal_representative  := fask_rec.legal_representative; --法定代表人
        iv_supp_info.company_contact_phone := fask_rec.company_contact_phone; --公司联系电话
        iv_supp_info.fa_contact_name       := fask_rec.contact_name; --业务联系人     
        iv_supp_info.fa_contact_phone      := fask_rec.contact_phone; --业务联系人手机
        iv_supp_info.company_type          := fask_rec.company_type; --公司类型
        iv_supp_info.is_our_factory        := fask_rec.is_our_factory; --是否本厂
        iv_supp_info.factroy_area          := fask_rec.factroy_area; --工厂面积（m2）
        ...
        SELECT MAX(fc.company_id)
         ...
         WHERE fc.company_id = p_company_id
           AND fc.licence_num = fask_rec.social_credit_code;
      
        iv_supp_info.supplier_company_id := v_supp_company_id; --供应商在平台的企业id   
        iv_supp_info.cooperation_type    := fask_rec.cooperation_type; --合作类型   
        iv_supp_info.cooperation_model   := fask_rec.cooperation_model; --合作模式 
        --iv_supp_info.coop_position       := fask_rec.coop_position; --合作定位       
        iv_supp_info.pay_term := fask_rec.pay_term; --付款条件
        --生产信息  
        iv_supp_info.product_type := fask_rec.product_type; --生产类型
        iv_supp_info.product_link := fask_rec.product_link; --生产环节
        ...
        IF fask_rec.factory_ask_type = 0 THEN
          iv_supp_info.remarks := fask_rec.memo; --备注
        
          --生产信息       
          iv_supp_info.product_line        := fask_rec.product_line; --生产线类型
          iv_supp_info.product_line_num    := fask_rec.product_line_num; --生产线数量
          iv_supp_info.quality_step        := fask_rec.quality_step; --质量等级
          iv_supp_info.work_hours_day      := fask_rec.work_hours_day; --上班时数/天
          iv_supp_info.worker_total_num    := fask_rec.worker_total_num; --总人数
          iv_supp_info.worker_num          := fask_rec.worker_num; --车位人数
          iv_supp_info.machine_num         := fask_rec.machine_num; --织机台数
          iv_supp_info.form_num            := fask_rec.form_num; --成型人数_鞋类
          iv_supp_info.product_efficiency  := fask_rec.product_efficiency; --产能效率
          iv_supp_info.pattern_cap         := fask_rec.pattern_cap; --打版能力
          iv_supp_info.fabric_purchase_cap := fask_rec.fabric_purchase_cap; --面料采购能力
          iv_supp_info.fabric_check_cap    := fask_rec.fabric_check_cap; --面料检测能力
        
          iv_supp_info.brand_type        := fask_rec.brand_type; --合作品牌/客户 类型
          iv_supp_info.cooperation_brand := fask_rec.cooperation_brand; --合作品牌/客户
          iv_supp_info.product_link      := fask_rec.product_link; --生产环节
          iv_supp_info.factroy_area      := fask_rec.factroy_area; --工厂面积
        
          --附件资料
          iv_supp_info.certificate_file  := fask_rec.certificate_file; --上传营业执照
          iv_supp_info.supplier_gate     := fask_rec.supplier_gate; --公司大门附件地址
          iv_supp_info.supplier_office   := fask_rec.supplier_office; --公司办公室附件地址
          iv_supp_info.supplier_site     := fask_rec.supplier_site; --生产现场附件地址
          iv_supp_info.supplier_product  := fask_rec.supplier_product; --产品图片附件地址
          iv_supp_info.other_information := fask_rec.ask_files; --其他资料
        
          p_insert_supplier_info(p_sp_data => iv_supp_info);
          --2.从表
          DECLARE
            --验厂申请单 意向合作范围
            ...
            FOR fscope_rec IN fask_scope_cur(fask_rec.factory_ask_id) LOOP
              ...
              iv_sp_scope_info.sharing_type := CASE
                                                 ...
                                                 ELSE
                                                  '02'
                                               END;
              iv_sp_scope_info.coop_state          := NULL;
              insert_coop_scope(p_cp_data => iv_sp_scope_info, p_type => 0);
            END LOOP;
            --2.2.准入时，同步验厂申请时生成的人员、机器配置
            scmdata.pkg_supplier_info_a.p_generate_person_machine_config_fa(p_company_id     => p_company_id,
                                                                            p_user_id        => p_user_id,
                                                                            p_factory_ask_id => p_factory_ask_id,
                                                                            p_supp_id        => v_supply_id);
          END rela_tbl_fa;
        ELSE
          --3.验厂  来源：验厂报告，能力评估
          --判断是否有验厂报告
          ...
          IF fr_rec.factory_report_id IS NOT NULL THEN
            --生产信息       
            iv_supp_info.product_line        := fr_rec.product_line; --生产线类型
            iv_supp_info.product_line_num    := fr_rec.product_line_num; --生产线数量
            iv_supp_info.quality_step        := fr_rec.quality_step; --质量等级
            iv_supp_info.work_hours_day      := fr_rec.work_hours_day; --上班时数/天
            iv_supp_info.worker_total_num    := fr_rec.total_number; --总人数
            iv_supp_info.worker_num          := fr_rec.worker_num; --车位人数
            iv_supp_info.machine_num         := fr_rec.machine_num; --织机台数
            iv_supp_info.form_num            := fr_rec.molding_number; --成型人数_鞋类
            iv_supp_info.product_efficiency  := fr_rec.product_efficiency; --产能效率
            iv_supp_info.pattern_cap         := fr_rec.pattern_cap; --打版能力
            iv_supp_info.fabric_purchase_cap := fr_rec.fabric_purchase_cap; --面料采购能力
            iv_supp_info.fabric_check_cap    := fr_rec.fabric_check_cap; --面料检测能力
            iv_supp_info.brand_type          := fr_rec.brand_type; --合作品牌/客户 类型
            iv_supp_info.cooperation_brand   := fr_rec.cooperation_brand; --合作品牌/客户
            iv_supp_info.factroy_area        := fr_rec.factory_area; --工厂面积
          
            --附件资料
            iv_supp_info.certificate_file  := fr_rec.certificate_file; --上传营业执照
            iv_supp_info.supplier_gate     := fr_rec.supplier_gate; --公司大门附件地址
            iv_supp_info.supplier_office   := fr_rec.supplier_office; --公司办公室附件地址
            iv_supp_info.supplier_site     := fr_rec.supplier_site; --生产现场附件地址
            iv_supp_info.supplier_product  := fr_rec.supplier_product; --产品图片附件地址
            iv_supp_info.other_information := fr_rec.ask_files; --其他资料
            ...
            BEGIN
              ...
              FOR faskrp_ability_rec IN faskrp_ability_cur(fask_rec.factory_ask_id) LOOP
                ...
                insert_coop_scope(p_cp_data => iv_sp_scope_info,
                                  p_type    => 0);
              END LOOP;
              --4.2.准入时，同步验厂报告时生成的人员、机器配置、品控体系
              scmdata.pkg_supplier_info_a.p_generate_person_machine_quality_config_fr(p_company_id        => p_company_id,
                                                                                      p_user_id           => p_user_id,
                                                                                      p_factory_report_id => fr_rec.factory_report_id,
                                                                                      p_supp_id           => v_supply_id);
            END;
          ELSE
            NULL;
        ...
        --5.生成合作工厂
        --生成规则：
        --1、若"是否本厂”=’是‘，则自动生成一条本厂数据；"是否本厂”=’否‘，则不再生成本厂数据；
        --2、"来源”=‘准入审核’ and “合作模式“绝对匹配‘外协厂’的供应商：
        --在生成档案时，会在关联供应商的“合作工厂”tab列表中，系统自动新增一条该供应商的外协厂记录；
        --关联供应商：为【申请验厂】中的“关联供应商”数据      
        DECLARE
          v_fac_rec scmdata.t_coop_factory%ROWTYPE;
        BEGIN
          IF fask_rec.is_our_factory = 1 THEN
            --准入后自动生成关联供应商的外协厂 
            v_fac_rec.coop_factory_id  := scmdata.f_get_uuid();
            v_fac_rec.company_id       := p_company_id;
            v_fac_rec.fac_sup_info_id  := v_supply_id;
            v_fac_rec.factory_name     := fask_rec.company_name;
            v_fac_rec.factory_type     := '00'; --外协厂
          ...
          IF fask_rec.cooperation_model = 'OF' THEN
      ...
      ELSE
        raise_application_error(-20002,
                                '不可操作！当前单据状态已发生变更，请重新加载页面');
      END IF;
    ELSE
      raise_application_error(-20002,
                              '不可操作！验厂申请单不存在，请联系开发排查');
    END IF;
  END p_create_t_supplier_info;
  /*============================================*
  * Author   : SANFU
  * Created  : 2020-09-03 17:10:14
  * ALERTER  :
  ...
  *============================================*/
  ...
  BEGIN
    --1.校验数据
    p_check_t_supplier_info(p_supplier_info_id);
  
    --2.生成供应商档案编号
    v_supplier_code := f_get_supplier_code_by_rule(p_supplier_info_id,
                                                   p_default_company_id);
    ...
    ELSE
      ...
      IF v_origin = 'AA' THEN
        ...
        --更新单据状态同时记录流程操作日志
        scmdata.pkg_ask_record_mange.p_update_flow_status_and_logger(p_company_id       => p_default_company_id,
                                                                     p_user_id          => p_user_id,
                                                                     p_fac_ask_id       => v_fac_id, --验厂申请单ID
                                                                     p_ask_record_id    => NULL, --合作申请单ID
                                                                     p_flow_oper_status => 'SP_FILED', --流程操作方式编码
                                                                     p_flow_af_status   => 'SP_01', --操作后流程状态
                                                                     p_memo             => NULL);
      ELSE
        NULL;
      END IF;
  ...
  BEGIN
    ...
    INSERT INTO scmdata.t_supplier_info_oper_log
      (log_id, supplier_info_id, oper_type, reason, create_id, create_time,
       company_id)
    VALUES
      (scmdata.f_get_uuid(), p_supplier_info_id, oper_type, p_reason,
       p_user_id, p_create_time, p_company_id);
  END insert_oper_log;
  ...
  *============================================*/
  ...
  PROCEDURE p_insert_supplier_info(p_sp_data scmdata.t_supplier_info%ROWTYPE) IS
  BEGIN
    INSERT INTO t_supplier_info
      (supplier_info_id, company_id, supplier_code, inside_supplier_code,
       supplier_company_id, supplier_company_name,
       supplier_company_abbreviation, legal_representative,
       company_create_date, regist_address, certificate_validity_start,
       certificate_validity_end, regist_price, social_credit_code,
       company_type, company_person, company_contact_person,
       company_contact_phone, taxpayer, company_say, certificate_file,
       organization_file, contract_stop_date, contract_file,
       cooperation_method, cooperation_model, production_mode,
       cooperation_type, cooperation_classification, cooperation_subcategory,
       sharing_type, public_accounts, public_payment, public_bank, public_id,
       public_phone, personal_account, personal_payment, personal_bank,
       personal_idcard, personal_phone, pay_type, settlement_type,
       reconciliation_user, reconciliation_phone, contract_start_date,
       create_supp_date, company_address, company_province, company_city,
       company_county, group_name, coop_position, product_line,
       product_line_num, worker_num, machine_num, quality_step, pattern_cap,
       fabric_purchase_cap, fabric_check_cap, cost_step, brand_type,
       product_type, product_link, fa_contact_name, fa_contact_phone,
       cooperation_brand, gendan_perid, supplier_info_origin,
       supplier_info_origin_id, status, coop_state, bind_status, pause,
       publish_id, publish_date, create_id, create_date, update_id,
       update_date, remarks, supplier_gate, supplier_office, supplier_site,
       supplier_product, file_remark, reserve_capacity, product_efficiency,
       work_hours_day, area_group_leader, ask_files, admit_result,
       pause_cause, group_name_origin, company_vill, pay_term,
       company_regist_date, is_our_factory, factroy_area, worker_total_num,
       form_num, other_information)
    VALUES
      (p_sp_data.supplier_info_id, p_sp_data.company_id,
       p_sp_data.supplier_code, p_sp_data.inside_supplier_code,
       p_sp_data.supplier_company_id, p_sp_data.supplier_company_name,
       p_sp_data.supplier_company_abbreviation,
       p_sp_data.legal_representative, p_sp_data.company_create_date,
       p_sp_data.regist_address, p_sp_data.certificate_validity_start,
       p_sp_data.certificate_validity_end, p_sp_data.regist_price,
       p_sp_data.social_credit_code, p_sp_data.company_type,
       p_sp_data.company_person, p_sp_data.company_contact_person,
       p_sp_data.company_contact_phone, p_sp_data.taxpayer,
       p_sp_data.company_say, p_sp_data.certificate_file,
       p_sp_data.organization_file, p_sp_data.contract_stop_date,
       p_sp_data.contract_file, p_sp_data.cooperation_method,
       p_sp_data.cooperation_model, p_sp_data.production_mode,
       p_sp_data.cooperation_type, p_sp_data.cooperation_classification,
       p_sp_data.cooperation_subcategory, p_sp_data.sharing_type,
       p_sp_data.public_accounts, p_sp_data.public_payment,
       p_sp_data.public_bank, p_sp_data.public_id, p_sp_data.public_phone,
       p_sp_data.personal_account, p_sp_data.personal_payment,
       p_sp_data.personal_bank, p_sp_data.personal_idcard,
       p_sp_data.personal_phone, p_sp_data.pay_type,
       p_sp_data.settlement_type, p_sp_data.reconciliation_user,
       p_sp_data.reconciliation_phone, p_sp_data.contract_start_date,
       p_sp_data.create_supp_date, p_sp_data.company_address,
       p_sp_data.company_province, p_sp_data.company_city,
       p_sp_data.company_county, p_sp_data.group_name,
       p_sp_data.coop_position, p_sp_data.product_line,
       p_sp_data.product_line_num, p_sp_data.worker_num,
       p_sp_data.machine_num, p_sp_data.quality_step, p_sp_data.pattern_cap,
       p_sp_data.fabric_purchase_cap, p_sp_data.fabric_check_cap,
       p_sp_data.cost_step, p_sp_data.brand_type, p_sp_data.product_type,
       p_sp_data.product_link, p_sp_data.fa_contact_name,
       p_sp_data.fa_contact_phone, p_sp_data.cooperation_brand,
       p_sp_data.gendan_perid, p_sp_data.supplier_info_origin,
       p_sp_data.supplier_info_origin_id, p_sp_data.status,
       p_sp_data.coop_state, p_sp_data.bind_status, p_sp_data.pause,
       p_sp_data.publish_id, p_sp_data.publish_date, p_sp_data.create_id,
       p_sp_data.create_date, p_sp_data.update_id, p_sp_data.update_date,
       p_sp_data.remarks, p_sp_data.supplier_gate, p_sp_data.supplier_office,
       p_sp_data.supplier_site, p_sp_data.supplier_product,
       p_sp_data.file_remark, p_sp_data.reserve_capacity,
       p_sp_data.product_efficiency, p_sp_data.work_hours_day,
       p_sp_data.area_group_leader, p_sp_data.ask_files,
       p_sp_data.admit_result, p_sp_data.pause_cause,
       p_sp_data.group_name_origin, p_sp_data.company_vill,
       p_sp_data.pay_term, p_sp_data.company_regist_date,
       p_sp_data.is_our_factory, p_sp_data.factroy_area,
       p_sp_data.worker_total_num, p_sp_data.form_num,
       p_sp_data.other_information);
  ...
  *============================================*/
  ...
  PROCEDURE p_update_supplier_info(p_sp_data t_supplier_info%ROWTYPE) IS
  
  BEGIN
    --当页面为待建档 已建档详情页 更新保存时校验数据
    p_check_save_t_supplier_info(p_sp_data => p_sp_data);
  
    UPDATE scmdata.t_supplier_info sp
    --基本信息
       SET sp.supplier_company_name         = p_sp_data.supplier_company_name, --公司名称
           sp.supplier_company_abbreviation = p_sp_data.supplier_company_abbreviation, --公司简称
           sp.social_credit_code            = p_sp_data.social_credit_code, --统一社会信用代码
           sp.inside_supplier_code          = p_sp_data.inside_supplier_code, --内部供应商编号
           sp.company_regist_date           = p_sp_data.company_regist_date, --公司注册日期
           sp.company_province              = p_sp_data.company_province, --公司省
           sp.company_city                  = p_sp_data.company_city, --公司市
           sp.company_county                = p_sp_data.company_county, --公司区
           sp.company_vill                  = p_sp_data.company_vill, --公司乡镇
           sp.company_address               = p_sp_data.company_address, --公司地址
           sp.group_name                    = p_sp_data.group_name, --分组名称（v9.9版 存值改为分组ID）
           sp.legal_representative          = p_sp_data.legal_representative, --法定代表人
           sp.company_contact_phone         = p_sp_data.company_contact_phone, --公司联系电话
           sp.fa_contact_name               = p_sp_data.fa_contact_name, --业务联系人
           sp.fa_contact_phone              = p_sp_data.fa_contact_phone, --业务联系人手机
           sp.company_type                  = p_sp_data.company_type, --公司类型
           sp.is_our_factory                = p_sp_data.is_our_factory, --是否本厂
           sp.factroy_area                  = p_sp_data.factroy_area, --工厂面积（m2）
           sp.remarks                       = p_sp_data.remarks, --备注    
           --生产信息
           sp.product_type        = p_sp_data.product_type, --生产类型
           sp.brand_type          = p_sp_data.brand_type, --合作品牌/客户 类型
           sp.cooperation_brand   = p_sp_data.cooperation_brand, --合作品牌/客户
           sp.product_link        = p_sp_data.product_link, --生产环节    
           sp.product_line        = p_sp_data.product_line, --生产线类型
           sp.product_line_num    = p_sp_data.product_line_num, --生产线数量
           sp.quality_step        = p_sp_data.quality_step, --质量等级
           sp.work_hours_day      = p_sp_data.work_hours_day, --上班时数/天
           sp.worker_total_num    = p_sp_data.worker_total_num, --总人数
           sp.worker_num          = p_sp_data.worker_num, --车位人数
           sp.machine_num         = p_sp_data.machine_num, --织机台数
           sp.form_num            = p_sp_data.form_num, --成型人数_鞋类
           sp.product_efficiency  = p_sp_data.product_efficiency, --产能效率
           sp.pattern_cap         = p_sp_data.pattern_cap, --打版能力
           sp.fabric_purchase_cap = p_sp_data.fabric_purchase_cap, --面料采购能力
           sp.fabric_check_cap    = p_sp_data.fabric_check_cap, --面料检测能力    
           --合作信息
           sp.pause               = p_sp_data.pause, --状态：0 启用 1 停用 2 试单     
           sp.supplier_company_id = p_sp_data.supplier_company_id, --供应商在平台的企业id   
           sp.cooperation_type    = p_sp_data.cooperation_type, --合作类型   
           sp.cooperation_model   = p_sp_data.cooperation_model, --合作模式
           sp.coop_position       = p_sp_data.coop_position, --合作定位
           sp.pay_term            = p_sp_data.pay_term, --付款条件        
           --附件资料
           sp.certificate_file  = p_sp_data.certificate_file, --上传营业执照
           sp.supplier_gate     = p_sp_data.supplier_gate, --公司大门附件地址
           sp.supplier_office   = p_sp_data.supplier_office, --公司办公室附件地址
           sp.supplier_site     = p_sp_data.supplier_site, --生产现场附件地址
           sp.supplier_product  = p_sp_data.supplier_product, --产品图片附件地址
           sp.other_information = p_sp_data.other_information, --其他资料    
           --其它
           sp.update_id   = p_sp_data.update_id, --修改人ID
           sp.update_date = p_sp_data.update_date --修改时间
     WHERE sp.supplier_info_id = p_sp_data.supplier_info_id;
  END p_update_supplier_info;
    ...
    FOR data_sub IN (SELECT *
      ...
      ELSE
        ...
        INSERT INTO scmdata.t_contract_info
          (contract_info_id, supplier_info_id, company_id,
           contract_start_date, contract_stop_date, contract_sign_date,
           contract_file, contract_type, contract_num, operator_id,
           operate_time, change_id, change_time)
        VALUES
          (p_sub_id, p_sub_supp_id, data_sub.company_id,
           data_sub.contract_start_date, data_sub.contract_stop_date,
           data_sub.contract_sign_date, data_sub.contract_file,
           data_sub.contract_type, data_sub.contract_num,
           data_sub.operator_id, data_sub.operate_time, data_sub.change_id,
  ...
  *============================================*/
  ...
  BEGIN
    ...
    INSERT INTO t_contract_info
      (contract_info_id, supplier_info_id, company_id, contract_start_date,
       contract_stop_date, contract_sign_date, contract_file, contract_type,
       contract_num, operator_id, operate_time, change_id, change_time)
    VALUES
      (scmdata.f_get_uuid(), p_contract_rec.supplier_info_id,
       p_contract_rec.company_id, p_contract_rec.contract_start_date,
       p_contract_rec.contract_stop_date, p_contract_rec.contract_sign_date,
       p_contract_rec.contract_file, p_contract_rec.contract_type,
       p_contract_rec.contract_num, p_contract_rec.operator_id, SYSDATE,
       p_contract_rec.change_id, SYSDATE);
  ...
  BEGIN
    ...
    FOR data_rec IN import_data_cur LOOP
      ...
      ELSE
        ...
        --将临时数据提交到业务表中,待建档
        p_insert_supplier_info(p_sp_data => p_sp_data);
        -- 提交=》已建档
        p_submit_t_supplier_info(p_supplier_info_id   => p_sp_data.supplier_info_id,
  ...
  *============================================*/
  ...
  BEGIN
    ...
    IF v_err_num > 0 THEN
      ...
      VALUES
        (v_msg_id, data_rec.company_id, data_rec.user_id, 'E',
         v_import_flag || v_msg, SYSDATE);
      --清空错误记录
      /*      v_num     := 0;
      v_err_num := 0;
      v_msg     := NULL;*/
    ELSE
      ...
      INSERT INTO scmdata.t_supplier_info_import_msg
      VALUES
        (v_msg_id, data_rec.company_id, data_rec.user_id, 'Y',
         v_import_flag, SYSDATE);
    END IF;
  ...
  *============================================*/
  ...
  BEGIN
    ...
    INSERT INTO t_supplier_info_temp
      (supplier_temp_id, company_id, user_id, err_msg_id,
       inside_supplier_code, supplier_company_name, cooperation_type,
       cooperation_model_code, cooperation_model, company_city,
       company_province, company_county, company_address, social_credit_code,
       legal_representative, company_contact_person, company_contact_phone,
       cooperation_type_code, company_city_code, company_province_code,
       company_county_code, memo)
    VALUES
      (p_supp_rec.supplier_temp_id, p_supp_rec.company_id,
       p_supp_rec.user_id, p_supp_rec.err_msg_id,
       p_supp_rec.inside_supplier_code, p_supp_rec.supplier_company_name,
       p_supp_rec.cooperation_type, p_supp_rec.cooperation_model_code,
       p_supp_rec.cooperation_model, p_supp_rec.company_city,
       p_supp_rec.company_province, p_supp_rec.company_county,
       p_supp_rec.company_address, p_supp_rec.social_credit_code,
       p_supp_rec.legal_representative, p_supp_rec.company_contact_person,
       p_supp_rec.company_contact_phone, p_supp_rec.cooperation_type_code,
       p_supp_rec.company_city_code, p_supp_rec.company_province_code,
       p_supp_rec.company_county_code, p_supp_rec.memo);
  ...
  *============================================*/
      ...
      ELSE
        ...
        ELSE
          ...
          INSERT INTO t_coop_scope
            (coop_scope_id, supplier_info_id, company_id, coop_mode,
             coop_classification, coop_product_cate, coop_subcategory,
             create_id, create_time, update_id, update_time, remarks, pause,
             sharing_type)
          VALUES
            (p_id, p_supplier_id, data_rec.company_id, data_rec.coop_mode,
             data_rec.coop_classification, data_rec.coop_product_cate,
             data_rec.coop_subcategory, data_rec.create_id, SYSDATE,
             data_rec.create_id, SYSDATE, NULL, 0, data_rec.sharing_type);
        END IF;
        ---加上共享
        IF data_rec.sharing_sup_code IS NOT NULL THEN
          FOR sup IN (SELECT regexp_substr(data_rec.sharing_sup_code,
            ...
            INSERT INTO scmdata.t_supplier_shared
              (supplier_shared_id, company_id, supplier_info_id,
               shared_supplier_code, remarks, coop_scope_id)
            VALUES
              (f_get_uuid(), data_rec.company_id, p_supplier_id,
               (SELECT a.supplier_code
                   FROM t_supplier_info a
                  WHERE a.inside_supplier_code = sup.code
                    AND a.company_id = data_rec.company_id), NULL, p_id);
          END LOOP;
        END IF;
      END IF;
    END LOOP;
  ...
  BEGIN
    ...
    END IF;
    INSERT INTO t_coop_scope
      (coop_scope_id, supplier_info_id, company_id, coop_classification,
       coop_product_cate, coop_subcategory, create_id, create_time,
       update_id, update_time, remarks, pause, sharing_type, publish_id,
       publish_date)
    VALUES
      (p_cp_data.coop_scope_id, p_cp_data.supplier_info_id,
       p_cp_data.company_id, p_cp_data.coop_classification,
       p_cp_data.coop_product_cate, p_cp_data.coop_subcategory,
       p_cp_data.create_id, p_cp_data.create_time, p_cp_data.update_id,
       p_cp_data.update_time, p_cp_data.remarks, p_cp_data.pause,
       p_cp_data.sharing_type, p_cp_data.publish_id, p_cp_data.publish_date);
  ...
  BEGIN
    INSERT INTO t_supplier_shared
      (coop_scope_id, supplier_shared_id, company_id, supplier_info_id,
       shared_supplier_code, remarks)
    VALUES
      (scope_rec.coop_scope_id, scmdata.f_get_uuid(), scope_rec.company_id,
       scope_rec.supplier_info_id, p_supplier_code, '');
