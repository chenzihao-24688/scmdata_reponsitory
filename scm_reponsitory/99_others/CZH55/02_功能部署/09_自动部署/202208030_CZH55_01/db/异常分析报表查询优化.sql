DECLARE
  v_sql CLOB;
BEGIN
  v_sql := 'SELECT DISTINCT c.company_dict_name NAME, NULL VALUE
...
SELECT v.supplier_code VALUE, v.supplier_company_name NAME
  FROM (SELECT listagg(DISTINCT tc.coop_classification, '';'') coop_classification,
         ...
         WHERE t.company_id = %default_company_id%
           AND t.status = 1
           AND t.cooperation_model <> ''OF''
         GROUP BY tc.coop_classification,
                  t.supplier_code,
                  t.supplier_company_name,
                  t.company_id) v
 WHERE :abn_fileds_type = ''02''
  ...
  UPDATE bw3.sys_param_list t
   ...
   WHERE t.param_name = 'ABN_FILEDS_TYPE_A';
END;
/
