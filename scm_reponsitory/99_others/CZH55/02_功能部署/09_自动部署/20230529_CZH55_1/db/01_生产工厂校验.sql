...
BEGIN
  --生产进度填报的前提：必须先指定工厂
  scmdata.pkg_check_data_comm.p_check_str_is_null(p_str => :factory_code_pr,p_err_msg => ''保存失败，请先指派生产工厂！'');
  --获取进度更新日期
...
BEGIN
  ...
  UPDATE scmdata.t_production_progress t
   ...
   WHERE t.product_gress_id = :product_gress_id;
END;';
UPDATE bw3.sys_item_list t SET t.update_sql = v_sql WHERE t.item_id = 'a_product_210';
END;
/
