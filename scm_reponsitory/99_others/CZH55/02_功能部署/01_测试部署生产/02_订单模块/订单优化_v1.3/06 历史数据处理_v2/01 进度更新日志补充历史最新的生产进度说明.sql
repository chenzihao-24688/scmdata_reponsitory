...
BEGIN
  FOR l_rec IN (SELECT b.product_gress_code,
                ...
                --WHERE v.product_gress_id =  'cba028d764fc1f8fe053e2281caca224'
                ORDER BY v.product_gress_id, v.rn
                ) LOOP
    --判断是否日志主表是否已存在LOG_ID
    SELECT COUNT(1)
      INTO v_flag
...
/*SELECT *
  FROM scmdata.t_plat_log t1
 INNER JOIN scmdata.t_plat_logs t2
    ON t2.log_id = t1.log_id
   AND t2.company_id = t2.company_id
 WHERE (t1.apply_pk_id, t1.company_id) IN
       (SELECT a.product_gress_id, a.company_id
          FROM scmdata.t_production_progress_log a
