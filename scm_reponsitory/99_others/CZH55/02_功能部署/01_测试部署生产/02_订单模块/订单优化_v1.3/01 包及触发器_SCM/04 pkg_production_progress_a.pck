  ...
  *============================================*/
  ...
  BEGIN
    ...
    ELSIF p_plan_date IS NULL AND p_delivery_date IS NOT NULL THEN
      ...
      --订单周期
      v_cycle := p_delivery_date - v_order_date;
      --订单剩余环节用时占比总和
      SELECT ceil(SUM(b.time_ratio) / 100 -
                  (MIN(b.time_ratio) / 100 * p_curlink_complet_prom))
        INTO v_left_link_timeprom
      ...
      vo_forecast_date := v_left_days + p_progress_update_date;
      vo_forecast_days := vo_forecast_date - p_delivery_date;
    END IF;
  ...
  BEGIN
    ...
    ELSE
      ...
      FOR pro_rec IN pro_cur LOOP
        IF p_item_id IN ('a_product_110', 'a_product_210') THEN
