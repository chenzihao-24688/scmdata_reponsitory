CREATE OR REPLACE PACKAGE pkg_production_progress IS
  ...
  FUNCTION check_uwmax_delivery_date_same(p_pro_rec       scmdata.t_production_progress%ROWTYPE,
                                          p_delivery_date DATE) RETURN NUMBER;
  --自动创建的扣款单
...
CREATE OR REPLACE PACKAGE BODY pkg_production_progress IS
  ...
  *============================================*/
  ...
  BEGIN
    ...
    FOR po_line_rec IN (SELECT *
      ...
      p_produ_rec.latest_planned_delivery_date := '';
      ...
      p_produ_rec.fabric_check                 := ''; --面料检测 还未开发此模块，暂不处理，默认为空
  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT decode(trunc(po_line_rec.delivery_date),
                  trunc(po_header_rec.create_time),
                  1,
                  ceil(trunc(po_line_rec.delivery_date) -
                       trunc(po_header_rec.create_time)))
    ...
    FOR pnode_rec IN pno_cur LOOP
      ...
      IF pnode_rec.progress_node_name = '交货' THEN
        ...
        v_target_completion_time := trunc(po_line_rec.delivery_date);
        --节点周期=生产周期*该节点用时占比
  ...
  --订单接口  订单交期，成本需同步更新生产订单计算逻辑（供orders触发器调用）
  PROCEDURE sync_orders_update_product(po_header_rec scmdata.t_ordered%ROWTYPE,
                                       po_line_rec   scmdata.t_orders%ROWTYPE,
  ...
  BEGIN
    ...
    IF v_flag > 0 THEN
      UPDATE scmdata.t_production_progress pr
         SET pr.forecast_delivery_date = po_line_rec.delivery_date
       WHERE pr.company_id = po_line_rec.company_id
         AND pr.order_id = po_line_rec.order_id
         AND pr.goo_id = po_line_rec.goo_id;
    END IF;
  ...
  *============================================*/
  ...
  BEGIN
    ...
    --2.1 如果未填时，订单结束失败，提示用户：“订单结束失败，订单实际延误天数＞2天，请填写'"延期问题分类、延期原因分类、延期原因细分、问题描述"！“
    IF v_actual_delay_day > 2 THEN
      IF v_count <> 4 THEN
        raise_application_error(-20002,
                                q'[提示：订单结束失败，订单结束失败,订单实际延误天数＞2天，请填写,延期问题分类、延期原因分类、延期原因细分、问题描述！]');
      END IF;
    END IF;
            ...
            FOR p_abn_rec IN (SELECT abn.abnormal_id,
              ...
              IF p_abn_rec.anomaly_class = 'AC_QUALITY' THEN
                ...
                sync_abn_ded_bill(p_company_id         => p_pro_rec.company_id,
                                  ...
                                  p_delay_amount       => p_abn_rec.delay_amount, --延期数量 
  ...
  *============================================*/
  ...
  BEGIN
    --1.订单交期
    SELECT trunc(ln.delivery_date), ln.order_amount
      INTO v_order_date, v_order_amount
    ...
    v_delay_day := ceil(v_delivery_date - v_order_date);
    ...
    UPDATE t_production_progress t
       SET t.actual_delivery_date = v_delivery_date,
           ...
           t.order_full_rate      = v_order_full_rate
  ...
  BEGIN
    ...
    IF p_abn_rec.anomaly_class IS NOT NULL AND
      ...
      IF p_abn_rec.delay_amount > v_order_amount THEN
        raise_application_error(-20002, '延期数量不能大于订单数量！');
      END IF;
  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT trunc(pln.delivery_date),
           decode(trunc(pln.delivery_date),
                  trunc(po.create_time),
                  1,
                  ceil(trunc(pln.delivery_date) - trunc(po.create_time)))
    ...
    IF p_produ_rec.progress_status = '00' THEN
      IF v_plan_completion_time IS NULL THEN
        p_produ_rec.forecast_delivery_date := v_delivery_date; --订单交期
      ELSE
        p_produ_rec.forecast_delivery_date := v_plan_completion_time; --计划完成日期（交货）
      END IF;
    ELSE
      --2.3 C 生产进度状态为进行中，预测交期=最新节点实际完成日期+剩余未完成节点的节点周期 .如果都为进行中，则取计划完成日期（交货）/订单交期
      SELECT MAX(trunc(actual_completion_time)), MAX(node_num)
        INTO v_actual_completion_time, v_node_num
        FROM (SELECT trunc(pn.actual_completion_time) actual_completion_time,
                     pn.node_num
                FROM scmdata.t_production_node pn
               WHERE pn.company_id = pno_rec.company_id
                 AND pn.product_gress_id = pno_rec.product_gress_id
      ...
      --节点周期=生产周期*该节点用时占比
      --生产周期=订单交期-接单日期
      FOR pn_data IN (SELECT time_ratio
                        FROM scmdata.t_production_node pn
                       WHERE pn.company_id = pno_rec.company_id
                         AND pn.product_gress_id = pno_rec.product_gress_id
      ...
      p_produ_rec.forecast_delivery_date := nvl((v_actual_completion_time +
                                                ceil(v_node_cycle)),
                                                nvl(v_plan_completion_time,
                                                    v_delivery_date));
    
    END IF;
  
    --2.4 最新计划交期  =计划完成日期（交货）
    p_produ_rec.latest_planned_delivery_date := v_plan_completion_time;
  ...
  *============================================*/
  ...
  BEGIN
    ...
    ELSE
      RETURN 0;
    END IF;
  END check_uwmax_delivery_date_same;
  ...
  BEGIN
    ...
    IF v_count > 0 THEN
      ...
      ELSE
        ...
        v_flag := check_uwmax_delivery_date_same(p_pro_rec       => p_pro_rec,
                                                 p_delivery_date => v_delivery_date);
  ...
  *============================================*/
  ...
  BEGIN
    ...
    ELSIF p_orgin = 'MA' THEN
      ...
      p_duc_rec.deduction_status := '00'; --待处理
      --按扣款方式给扣款单价赋值
      IF v_deduction_method = 'METHOD_00' THEN
        p_duc_rec.discount_unit_price := nvl(p_order_price, 0);
      ELSE
