CREATE OR REPLACE PACKAGE scmdata.pkg_supplier_info IS
  ...
  *============================================*/
  ...
  --通过所属品类 获取所在分组
  PROCEDURE get_category_config(p_company_id         VARCHAR2,
                                p_supp_id            VARCHAR2,
                                po_group_name        OUT VARCHAR2,
                                po_area_group_leader OUT VARCHAR2);
  ...
  --获取分组
  PROCEDURE get_group_name(p_company_id         VARCHAR2,
                           p_supp_id            VARCHAR2,
                           po_group_name        OUT VARCHAR2,
                           po_area_group_leader OUT VARCHAR2);

  --更新所在区域 区域组长
  PROCEDURE update_group_name(p_company_id       VARCHAR2,
                              p_supplier_info_id VARCHAR2);
  ...
  *============================================*/
  PROCEDURE submit_t_supplier_info(p_supplier_info_id   VARCHAR2,
                                   ...
                                   p_user_id            VARCHAR2);
  --关联供应商
  --准入流程自动带入关联供应商时，需匹配合作范围(分类，生产类别，子类)，对应合作范围才带入关联供应商；
  PROCEDURE relate_suppier(p_rela_sup_id VARCHAR2,
...
SELECT v.supplier_info_id,
 ...
 WHERE ((%is_company_admin% = 1) OR
       instr_priv(p_str1  => @subsql@,
                   ...
                   p_split => ';') > 0)
 ORDER BY nvl(fa.is_urgent, 0) DESC, v.create_date DESC]';
    RETURN v_query_sql;
...
SELECT v.supplier_info_id,
 ...
 WHERE a.company_id = %default_company_id%
  ...
  --所在分组
  /*  FUNCTION f_get_area_group_name(p_company_id VARCHAR2,
                                 p_supp_id    VARCHAR2,
                                 p_area       VARCHAR2) RETURN VARCHAR2 IS
  BEGIN
    --品类优先区域
    --1 是否存在品类
    SELECT COUNT(1)
     ...
     WHERE t.company_id = p_company_id
       AND t.supplier_info_id = p_supp_id;
  
    IF v_flag = 0 THEN
  
    END IF;
  
  END get_area_group_name;*/
  ...
  *============================================*/
  ...
  --p_supp_id 供应商ID
  --po_group_name 分组名称
  --po_area_group_leader 区域组长
  PROCEDURE get_category_config(p_company_id         VARCHAR2,
                                p_supp_id            VARCHAR2,
                                po_group_name        OUT VARCHAR2,
                                po_area_group_leader OUT VARCHAR2) IS
    v_group_name        VARCHAR2(32);
    v_area_group_leader VARCHAR2(32);
  BEGIN
    SELECT MAX(dd.group_name), MAX(dd.area_group_leader)
      INTO v_group_name, v_area_group_leader
      FROM (SELECT listagg(DISTINCT sa.coop_classification, ';') within GROUP(ORDER BY sa.coop_classification) coop_classification,
                   listagg(DISTINCT sa.coop_product_cate, ';') within GROUP(ORDER BY sa.coop_product_cate) coop_product_cate,
                   MAX(sp.company_province) company_province,
                   MAX(sp.company_city) company_city,
                   MAX(sp.company_id) company_id,
                   MAX(sp.supplier_info_id) supplier_info_id
              FROM scmdata.t_supplier_info sp
             INNER JOIN scmdata.t_coop_scope sa
                ON sa.company_id = sp.company_id
               AND sa.supplier_info_id = sp.supplier_info_id
             GROUP BY sp.supplier_info_id, sa.coop_classification) cc
     INNER JOIN (SELECT aa.group_name,
       ...
       AND (instr_priv(dd.province_id, cc.company_province) > 0 AND
            instr_priv(dd.city_id, cc.company_city) > 0)
     WHERE cc.supplier_info_id = p_supp_id
       AND cc.company_id = p_company_id;
  
    po_group_name        := v_group_name;
    po_area_group_leader := v_area_group_leader;
  
  END get_category_config;
  --通过所属区域  获取所在分组
  /*  FUNCTION get_area_config(p_company_id VARCHAR2, p_supp_id VARCHAR2)
    RETURN VARCHAR2 IS
    v_group_name VARCHAR2(32);
  BEGIN
    v_group_name := NULL;
    SELECT MAX(dd.group_name)
     INTO v_group_name
     FROM scmdata.t_supplier_info cc
     LEFT JOIN (SELECT aa.group_name, bb.province_id || bb.city_id AS quyu
  ...
  BEGIN
    ...
    --所在分组根据供应商的品类、区域，从所在分组配置中匹配，自动获取对应分组；
    IF v_flag > 0 THEN
      get_category_config(p_company_id         => p_company_id,
                          p_supp_id            => p_supp_id,
                          po_group_name        => po_group_name,
                          po_area_group_leader => po_area_group_leader);
    ELSE
      po_group_name        := NULL;
      po_area_group_leader := NULL;
    END IF;
  END get_group_name;

  --更新所在区域 区域组长
  PROCEDURE update_group_name(p_company_id       VARCHAR2,
                              p_supplier_info_id VARCHAR2) IS
    vo_group_name        VARCHAR2(32);
    vo_area_group_leader VARCHAR2(32);
  BEGIN
    pkg_supplier_info.get_group_name(p_company_id         => p_company_id,
                                     p_supp_id            => p_supplier_info_id,
                                     po_group_name        => vo_group_name,
                                     po_area_group_leader => vo_area_group_leader);
    UPDATE scmdata.t_supplier_info t
       SET t.group_name        = vo_group_name,
           t.area_group_leader = vo_area_group_leader
  ...
  *============================================*/
  ...
  PROCEDURE create_t_supplier_info(p_company_id     VARCHAR2,
    ...
    --验厂报告 生产能力评估明细  如果分类+生产类别重复，则需合并
  ...
  BEGIN
    ...
    /*    pkg_supplier_info.get_group_name(p_company_id         => p_company_id,
                                       ...
                                       po_group_name        => iv_supp_info.group_name,
                                       po_area_group_leader => iv_supp_info.area_group_leader);
    */
    --判断验厂方式
    --1.不验厂  来源只有验厂申请单
    --按验厂申请单 供应商信息 生成档案
    IF fask_rec.factory_ask_type = 0 THEN
      ...
      FOR fscope_rec IN fask_scope_cur(fask_rec.factory_ask_id) LOOP
        ...
        --准入流程自动带入关联供应商时，需匹配合作范围(分类，生产类别，子类)，对应合作范围才带入关联供应商；
        --合作工厂获取是否指定是‘是’/指定的显示
        relate_suppier(p_rela_sup_id => fask_rec.rela_supplier_id,
                       scope_rec     => iv_sp_scope_info);
    ...
    --更新所在区域 区域组长
    update_group_name(p_company_id       => p_company_id,
  ...
  *============================================*/
  ...
  --准入流程自动带入关联供应商时，需匹配合作范围(分类，生产类别，子类)，对应合作范围才带入关联供应商；
  PROCEDURE relate_suppier(p_rela_sup_id VARCHAR2,
                           scope_rec     scmdata.t_coop_scope%ROWTYPE) IS
    v_coop_classification VARCHAR2(2000);
    v_coop_product_cate   VARCHAR2(4000);
    v_coop_subcategory    VARCHAR2(4000);
    v_rela_supp_code      VARCHAR2(32); --关联供应商编号
  BEGIN
    IF p_rela_sup_id IS NULL THEN
      NULL;
    ELSE
      SELECT MAX(t.supplier_code)
        INTO v_rela_supp_code
        FROM scmdata.t_supplier_info t
       WHERE t.supplier_info_id = p_rela_sup_id
         AND t.status = 1;
    
      SELECT listagg(t.coop_classification, ';') coop_classification,
             listagg(t.coop_product_cate, ';') coop_product_cate,
             listagg(t.coop_subcategory, ';') coop_subcategory
        INTO v_coop_classification, v_coop_product_cate, v_coop_subcategory
        FROM scmdata.t_coop_scope t
       WHERE t.supplier_info_id = p_rela_sup_id;
    
      IF instr_priv(v_coop_classification, scope_rec.coop_classification) > 0 AND
         instr_priv(v_coop_product_cate, scope_rec.coop_product_cate) > 0 AND
         instr_priv(v_coop_subcategory, scope_rec.coop_subcategory) > 0 THEN
        insert_supplier_shared(scope_rec       => scope_rec,
  ...
  *============================================*/
  ...
  BEGIN
    ...
    --新增日志操作
    IF p_item_id = 'a_supp_161' THEN
      scmdata.pkg_supplier_info.insert_oper_log(p_sp_data.supplier_info_id,
                                                '修改档案-基本信息',
                                                '',
                                                p_sp_data.update_id,
                                                p_sp_data.company_id,
  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT MAX(status_af_oper),
           MAX(flow_node_name_af || flow_node_status_desc_af)
  ...
  BEGIN
    --获取供应商名称  接收人
    SELECT MAX(t.company_name),
           MAX((SELECT t.inner_user_id
    ...
    ELSIF vo_flow_status IN ('FA13') THEN
      --重置接收人  根据能力评估分类+生产类别 默认生产部经理 ：除了鞋子的其他分类通知：叶其林，鞋通知：康平 ，两者都有则通知：叶其林、康平
