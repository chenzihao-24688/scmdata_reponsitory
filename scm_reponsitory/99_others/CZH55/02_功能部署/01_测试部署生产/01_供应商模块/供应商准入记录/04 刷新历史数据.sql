DECLARE
  ...
  v_num   NUMBER;
BEGIN
  for i in (SELECT sp.company_id,sp.supplier_info_id,sp.inside_supplier_code FROM scmdata.t_supplier_info sp where sp.inside_supplier_code is not null) loop 

    v_num := 0;
    SELECT *
      INTO sup_rec
      FROM scmdata.t_supplier_info sp
     WHERE sp.supplier_info_id = i.supplier_info_id
       AND sp.company_id = i.company_id;
    sup_rec.inside_supplier_code := i.inside_supplier_code;
    ...
    FOR scope_rec  IN (SELECT * FROM scmdata.t_coop_scope tc
                        WHERE tc.supplier_info_id = i.supplier_info_id
                           AND tc.company_id = i.company_id) LOOP 
    ...
    FOR fac_rec  IN (SELECT * FROM scmdata.t_coop_factory tf
                       WHERE tf.supplier_info_id = i.supplier_info_id
                            AND tf.company_id = i.company_id) LOOP 
