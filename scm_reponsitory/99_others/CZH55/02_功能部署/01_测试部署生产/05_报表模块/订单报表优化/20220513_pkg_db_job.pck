    ...
    BEGIN
      IF trunc(SYSDATE, 'mm') + 2 = trunc(SYSDATE) THEN
        ...
        for i in (select t.pt_ordered_id
                    from scmdata.t_bestgoodsofmonth a
                   inner join scmdata.pt_ordered t
                      on t.goo_id_pr = a.goo_id
                     and t.year = a.ayear
                     and t.month = a.amonth
                   where t.company_id = p_company_id
                       AND to_char(t.delivery_date, 'yyyy-mm') =
                           to_char(add_months(SYSDATE, -1), 'yyyy-mm')) loop
          update scmdata.pt_ordered b
             set b.is_twenty = 1
           where b.pt_ordered_id = i.pt_ordered_id;
        end loop;
        /*        MERGE INTO (SELECT goo_id_pr, is_twenty
