...
SELECT t.product_gress_id,
 ...
 AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_110') > 0 AND
       instr_priv(%coop_class_priv%, cf.category) > 0)
...
SELECT t.product_gress_id,
 ...
 WHERE oh.company_id = %default_company_id%
   --AND ((%is_company_admin%) = 1  OR instr(%coop_class_priv%,cf.category) > 0)
    AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_116') > 0 AND
       instr_priv(%coop_class_priv%, cf.category) > 0)
...
SELECT t.product_gress_id,
 ...
 WHERE oh.company_id = %default_company_id%
  ...
  --AND ((%is_company_admin%) = 1  OR instr(%coop_class_priv%,cf.category) > 0)
   AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_118') > 0 AND
       instr_priv(%coop_class_priv%, cf.category) > 0)
...
AND t.factory_code = sp1.supplier_code*/
 ...
 WHERE oh.company_id = %default_company_id%
  --AND ((%is_company_admin%) = 1  OR instr(%coop_class_priv%,cf.category) > 0)
   AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_120_1') > 0 AND
       instr_priv(%coop_class_priv%, cf.category) > 0)
 ORDER BY a.create_time DESC]';
...
AND t.factory_code = sp1.supplier_code*/
 ...
 WHERE oh.company_id = %default_company_id%
  --AND ((%is_company_admin%) = 1  OR instr(%coop_class_priv%,cf.category) > 0)
  AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_120_2') > 0 AND
       instr_priv(%coop_class_priv%, cf.category) > 0)
...
v_str11 := q'[SELECT DISTINCT po.order_id,
 ...
 WHERE po.company_id = %default_company_id%
  ...
  AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_130_1') > 0 AND
       instr_priv(%coop_class_priv%, tc.category) > 0)
...
v_str13 := q'[SELECT DISTINCT po.order_id,
 ...
 --AND ((%is_company_admin%) = 1  OR instr(%coop_class_priv%,tc.category) > 0)
 AND (%is_company_admin% = 1) OR (instr_priv(%coop_data_class_item_id%, 'a_product_130_2') > 0 AND
       instr_priv(%coop_class_priv%, tc.category) > 0)
 ORDER BY po.approve_time DESC]';
 
v_str14 := '';

update nbw.sys_item_list t set t.select_sql = v_str1 ,t.subselect_sql = v_str2 WHERE t.item_id = 'a_product_110';
update nbw.sys_item_list t set t.select_sql = v_str3 ,t.subselect_sql = v_str4 WHERE t.item_id = 'a_product_116';

update nbw.sys_item_list t set t.select_sql = v_str5 ,t.subselect_sql = v_str6 WHERE t.item_id = 'a_product_118';
update nbw.sys_item_list t set t.select_sql = v_str7 ,t.subselect_sql = v_str8 WHERE t.item_id = 'a_product_120_1';

update nbw.sys_item_list t set t.select_sql = v_str9 ,t.subselect_sql = v_str10 WHERE t.item_id = 'a_product_120_2';
update nbw.sys_item_list t set t.select_sql = v_str11 ,t.subselect_sql = v_str12 WHERE t.item_id = 'a_product_130_1';

update nbw.sys_item_list t set t.select_sql = v_str13 ,t.subselect_sql = v_str14 WHERE t.item_id = 'a_product_130_2';
