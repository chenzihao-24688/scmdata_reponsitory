...
BEGIN
  ...
  IF  v_flag > 0 THEN
    v_tmpsql := ' AND 1=1';
  ELSE
    v_tmpsql := ' AND instr(%coop_class_priv%,cf.category) > 0';
  END IF;
...
SELECT t.product_gress_id,
 ...
 INNER JOIN supp sp2
   ...
   AND t.supplier_code = sp2.supplier_code
 WHERE oh.company_id = %default_company_id%
 --AND instr(%coop_class_priv%,cf.category)>0
 @subsql@
...
v_str11 := q'[SELECT DISTINCT po.order_id,
 ...
 WHERE po.company_id = %default_company_id%
  ...
  --AND instr(%coop_class_priv%,tc.category)>0
  @subsql@
...
BEGIN
  ...
  IF  v_flag > 0 THEN
    v_tmpsql := ' AND 1=1';
  ELSE
    v_tmpsql := ' AND instr(%coop_class_priv%,tc.category) > 0';
  END IF;
