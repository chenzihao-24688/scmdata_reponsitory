  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT t.actual_delay_day,
           ...
           t.delivery_amount
      INTO v_actual_delay_day, v_count, v_delivery_amount
  ...
  BEGIN
    ...
    SELECT t.order_id
     ...
     WHERE t.company_id = p_company_id
       AND t.product_gress_id = p_product_gress_id;
  
    --订单交期
    v_order_date := get_order_date(p_company_id => p_company_id,
                                   p_order_code => v_order_code,
                                   p_status     => 1);
    ...
    IF v_pro_status = '00' OR v_pro_status = '02' THEN
      ...
      IF v_nplnl_count = 0 AND v_natnl_count = 0 THEN
        ...
        IF vo_delay_day > 0 THEN
          vo_forecast_date := v_order_date + vo_delay_day;
        ELSE
          vo_forecast_date := v_order_date;
        END IF;
        --1.2 计划完成日期 有值(交货/最新节点)/实际完成日期 为空（所有节点）
      ELSIF v_nplnl_count > 0 AND v_natnl_count = 0 THEN
        --1.2.1 若当前日期已超过计划完成日期（交货/最新节点）
        vo_delay_day := check_is_delay(p_max_node_date => v_plnl_date);
        --1.2.2 若 计划完成日期 最新节点 = 交货节点
        IF v_plnl_num = v_count THEN
          ...
          ELSE
            vo_forecast_date := v_plnl_date;
          END IF;
          --1.2.4 若 计划完成日期 最新节点 <> 交货节点
        ELSE
          --1.2.5 预测交期 = 计划完成日期（最新节点） + 剩余节点的周期  + 超计划完成日期天数（vo_delay_day ? >0:vo_delay_day:0）
          ...
          ELSE
            vo_forecast_date := v_plnl_date + v_node_cycle;
          END IF;
        END IF;
      END IF;
      --2.生产进度进行中  特有逻辑
      IF v_pro_status = '02' THEN
        ...
        IF v_nplnl_count = 0 AND v_natnl_count > 0 THEN
          ...
          ELSE
            ...
            ELSE
              vo_forecast_date := v_atnl_date + v_node_cycle;
            END IF;
          END IF;
