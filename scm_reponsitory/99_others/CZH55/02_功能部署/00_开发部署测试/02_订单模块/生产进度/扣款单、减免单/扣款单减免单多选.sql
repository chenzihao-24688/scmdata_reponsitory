...
SELECT '合计金额' "订单单据号",
  ...
  FROM (SELECT '合计金额' "订单单据号",
               ...
               NULL "扣款原因",
               NULL "应扣比例",
               (SELECT td1.total_money FROM total_cal td1) ded_price,
               NULL "减免比例",
               (SELECT td2.total_reduction_price * -1 FROM total_cal td2) redu_price,
               (SELECT td3.total_actual_discount_price FROM total_cal td3) actuded_price,
               NULL "备注"
          FROM scmdata.t_ordered a
         WHERE a.company_id = %default_company_id%
           AND a.order_code IN (@selection))]';
