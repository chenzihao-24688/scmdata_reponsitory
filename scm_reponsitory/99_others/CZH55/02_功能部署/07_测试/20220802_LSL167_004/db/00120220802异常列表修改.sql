----修改问题分类分类sys_pick_list
begin
update bw3.sys_pick_list t
   set t.pick_sql     = '----20220728 LSL167修改、（责任1级部门、责任2级部门存编码）
SELECT ad.problem_classification problem_class_pr,
...
SELECT ad.problem_classification problem_class_pr,
 ...
 WHERE tc.company_id = :company_id
   AND tc.goo_id = :goo_id_pr
   AND ad.anomaly_classification = :anomaly_class_pr*/',
       t.other_fields = 'CAUSE_CLASS_PR,PROBLEM_CLASS_PR,CAUSE_DETAILED_PR,IS_SUP_RESPONSIBLE,RESPONSIBLE_DEPT_PR,RESPONSIBLE_DEPT_SEC_PR,FIRST_DEPT_ID,SECOND_DEPT_ID'
 where t.element_id = 'pick_a_product_118_1';
end;
/

---异常处理列表-待处理页面select——sql更新
declare
  v_sql clob := 'WITH supp AS
...
begin
  update bw3.sys_item_list t
     set t.select_sql    = v_sql,
         t.update_sql    = u_sql,
         t.noshow_fields = 'ABNORMAL_ID,ANOMALY_CLASS_PR,PRODUCT_GRESS_ID,COMPANY_ID,SUPPLIER_CODE_PR,FACTORY_CODE_PR,ORDER_ID_PR,ABNORMAL_STATUS_PR,HANDLE_OPINIONS_PR,IS_DEDUCTION_PR,CONFIRM_ID,CONFIRM_DATE,DEDUCTION_METHOD_PR,RESPONSIBLE_PARTY_PR,ABN_RANGE,PROGRESS_STATUS_PR,FIRST_DEPT_ID,SECOND_DEPT_ID'
   where t.item_id = 'a_product_120_1';
end;
/

---异常处理列表-已处理页面select——sql更新
declare
  v_sql clob := 'WITH supp AS
 (SELECT sp.company_id, sp.supplier_code, sp.supplier_company_name
    FROM scmdata.t_supplier_info sp),
...
begin
  update bw3.sys_item_list t
     set t.select_sql    = v_sql,
         t.noshow_fields = 'ANOMALY_CLASS_PR,PRODUCT_GRESS_ID,COMPANY_ID,SUPPLIER_CODE_PR,FACTORY_CODE_PR,ORDER_ID_PR,ABNORMAL_ID,ABNORMAL_STATUS_PR,HANDLE_OPINIONS_PR,IS_DEDUCTION_PR,ABN_RANGE,PROGRESS_STATUS_PR,FIRST_DEPT_ID,SECOND_DEPT_ID'
   where t.item_id = 'a_product_120_2';
end;
/

---生产进度下异常处理按钮调整页面——异常处理单挂靠loou_up
declare
  v_sql clob := 'WITH supp AS
 (SELECT sp.company_id, sp.supplier_code, sp.supplier_company_name
    FROM scmdata.t_supplier_info sp
...
begin
  update bw3.sys_item_list t
     set t.select_sql    = v_sql,
         t.update_sql    = u_sql,
         t.noshow_fields = 'PRODUCT_GRESS_ID,COMPANY_ID,SUPPLIER_CODE_PR,FACTORY_CODE_PR,ORDER_ID_PR,ABNORMAL_ID_PR,ABNORMAL_STATUS_PR,IS_DEDUCTION_PR,HANDLE_OPINIONS_PR,ANOMALY_CLASS_PR,RESPONSIBLE_PARTY_PR,DEDUCTION_METHOD_PR,ABN_RANGE,PROGRESS_STATUS_PR,FIRST_DEPT_ID,SECOND_DEPT_ID'
