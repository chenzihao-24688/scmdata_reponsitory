CREATE OR REPLACE PACKAGE pkg_supplier_info IS
  ...
  --合作范围  begin
  --select_sql
  FUNCTION f_query_sup_coop_list(p_item_id      VARCHAR2,
                                 p_sup_id       VARCHAR2,
                                 p_rest_methods VARCHAR2 DEFAULT NULL) RETURN CLOB;
  --insert sql
  FUNCTION f_insert_sup_coop_list(p_item_id VARCHAR2, p_sup_id VARCHAR2)
    RETURN CLOB;
  --update sql
  FUNCTION f_update_sup_coop_list(p_item_id VARCHAR2, p_sup_id VARCHAR2)
    RETURN CLOB;
  --delete sql
  FUNCTION f_delete_sup_coop_list(p_item_id VARCHAR2, p_sup_id VARCHAR2)
    RETURN CLOB;
  --合作范围  end
...
SELECT v.supplier_info_id,
 ...
 WHERE a.company_id = %default_company_id%
  ...
  FUNCTION f_query_sup_coop_list(p_item_id      VARCHAR2,
                                 p_sup_id       VARCHAR2,
                                 p_rest_methods VARCHAR2 DEFAULT NULL) RETURN CLOB IS
    v_query_sql CLOB;
  BEGIN
  
    v_query_sql := q'[SELECT tc.coop_scope_id,
       tc.coop_scope_id ||]' || q'['/]' || p_rest_methods ||
                   q'[' coop_scope_id_rest,
       tc.pause,
 ...
 WHERE tc.supplier_info_id = ']' || p_sup_id || q'['
  ...
  v_sup_id  varchar2(32) := ']' || p_sup_id || q'[';
BEGIN
  p_cp_data.coop_scope_id       := scmdata.f_get_uuid();
  p_cp_data.supplier_info_id    := v_sup_id;
...
BEGIN
  ...
  --更新所在分组，区域组长
  pkg_supplier_info.p_update_group_name(p_company_id  => %default_company_id%,p_supplier_info_id => v_sup_id);
END;]';
    IF p_item_id IN ('a_supp_151_1', 'a_supp_161_1', 'a_supp_171_1') THEN
      RETURN v_delete_sql;
    ELSE
  ...
  BEGIN
   ...
   SELECT f.coop_factory_id,
       f.coop_factory_id ||]' || q'['/]' ||
                   p_rest_methods || q'[' coop_factory_id_rest,
       --f.supplier_info_id,
       f.fac_sup_info_id,
       f.company_id,
 ...
 WHERE f.supplier_info_id = ']' || p_sup_id || q'['
 ...
 ORDER BY f.factory_type ASC]';
  ...
  --insert sql
  FUNCTION f_insert_coop_factory_list(p_item_id VARCHAR2,
                                      p_sup_id  VARCHAR2) RETURN CLOB IS
    v_insert_sql CLOB;
  BEGIN
    v_insert_sql := q'[DECLARE
      v_fac_rec scmdata.t_coop_factory%ROWTYPE;
      v_sup_id  varchar2(32) := ']' || p_sup_id ||
                    q'[';
    BEGIN
      ...
      v_fac_rec.memo             := :memo;
      v_fac_rec.supplier_info_id := v_sup_id;
