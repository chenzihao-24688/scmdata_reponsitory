...
BEGIN
  BEGIN
    ...
    USING (
      ...
      dilvery_info AS
       (SELECT * FROM scmdata.t_delivery_record)
      SELECT DISTINCT *
        FROM (SELECT to_number(extract(YEAR FROM po.delivery_date)) YEAR,
                     ...
                     (SELECT aa.nick_name
                       ...
                       INNER JOIN (SELECT fu.nick_name,
                                         ts.group_name,
                                         ts.group_config_id,
                                         ts.pause
                                    FROM scmdata.sys_company_user fu
                                   INNER JOIN scmdata.t_supplier_group_config ts
                                      ON fu.company_id = ts.company_id
                                     AND fu.user_id = ts.area_group_leader) aa
                          ON sp.group_name = aa.group_name
                       INNER JOIN t_supplier_group_area_config bb
                          ON aa.group_config_id = bb.group_config_id
                       WHERE aa.pause = 0
                         AND sp.company_id = po.company_id
                         AND sp.supplier_code = po.supplier_code) area_gp_leader, --区域组长
                     --'' is_twenty, --是否前20%
                     CASE
  ...
  BEGIN
    FOR pt_rec IN (SELECT pt.company_id,
                    ...
                    WHERE pt.company_id = p_company_id) LOOP
      UPDATE scmdata.pt_ordered pt
         SET pt.flw_order_manager = pkg_db_job.f_get_manager(p_company_id     => pt_rec.company_id,
                                                             p_user_id        => pt_rec.flw_order,
                                                             p_company_job_id => '1001005003005002'),
             pt.qc_manager        = pkg_db_job.f_get_manager(p_company_id     => pt_rec.company_id,
                                                             p_user_id        => pt_rec.qc,
  ...
  BEGIN
    ...
    USING (SELECT * FROM scmdata.t_bestgoodsofmonth) bg
