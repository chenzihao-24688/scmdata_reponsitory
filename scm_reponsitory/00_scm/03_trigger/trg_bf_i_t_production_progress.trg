create or replace trigger scmdata.trg_bf_i_t_production_progress
  before insert on scmdata.t_production_progress
  for each row
declare
...
begin
  ...
  --20220425只同步生产订单的待送检
  select nvl(max(a.is_product_order), 0), max(a.order_type)
    into p_is_product_order, p_order_type
  ...
  if p_i = 1 then
    ...
    if p_commodity_info.supplier_code = 'C00216' and
    ...
    if p_config_id is not null and p_is_setting is not null and
      ...
      if p_scope = '全部' then
        ...
        scmdata.pkg_fabric_evaluate.p_insert_FABRIC_EVALUATE_GOO_ID(pi_goo_id          => :new.goo_id,
                                                                    pi_company_id      => :new.company_id,
                                                                    pi_except_progress => 1);
        if p_order_type in ('PP', 'WP') then
          :NEW.FABRIC_CHECK           := 'FABRIC_EVELUATE_NO_SEND';
          :new.fabric_check_extend_01 := 'FABRIC_EVELUATE_NO_SEND';
        else
