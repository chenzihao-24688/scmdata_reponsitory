...
CREATE OR REPLACE PACKAGE BODY SCMDATA.PKG_RETURN_MANAGEMENT IS
  ...
  BEGIN
    FOR I IN (SELECT EXG_ID,
      ...
      V_QA_ID :=NULL;
      V_QA_IDS:=NULL;
      V_RM_CAUSE_DETAIL_ID:=NULL;
      V_FIRST_DEPT_ID:=NULL;
      V_SECOND_DEPT_ID:=NULL;
      ----供应商编号转为scm供应商档案编号
      ...
      /*V_MERCHER2 := F_GET_GENDAN(PI_GOO_ID     => V_GOOID,
       ...
       --退货单供应商的跟单员
        ...
        IF INSTR(';'||V_FENBU||';',';'||V_CATEGORY||';') >0 AND nvl(INSTR(V_MERCHER2,Z.GENDANYUAN ),0) <1 THEN
  ...
  BEGIN
    ...
    FOR V IN (SELECT T.RM_ID, T.EXG_ID, T.RELA_GOO_ID, T.COMPANY_ID
               ...
               WHERE TO_NUMBER(TO_CHAR(T.ACCESS_TIME, 'yyyymm')) = V_DATE_M1
