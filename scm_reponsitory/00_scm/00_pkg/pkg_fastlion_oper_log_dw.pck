...
create or replace package body scmdata.PKG_FASTLION_OPER_LOG_DW is
  ...
  begin
    ...
    scmdata.pkg_fastlion_oper_log_dw.dw_fastlion_sys_oper_log(pi_date => sysdate - 1);
  end;
  ...
  begin
    ...
    insert into scmdata.sys_group_item_dw
      ...
      select scmdata.f_get_uuid_pe(), base.*
        from (select a.item_id,
                ...
                from bw3.sys_item a
               inner join bw3.sys_tree_list tl
    ...
    loop
      ...
      for x in (select a.*,
                       ...
                       b.pause,
                       (select nvl(max(1), 0)
                          from bw3.sys_tree_list tp
                         inner join bw3.sys_item ti
                            on ti.item_id = tp.parent_id
                         where (ti.item_type = 'menu' or not exists
                                (select 1
                                   from bw3.sys_item_list c
        ...
        merge into scmdata.sys_group_item_dw a
        using (select distinct i.item_id, i.caption_sql
                 from bw3.sys_item_element_rela ier
                inner join bw3.sys_action a
                   on a.element_id = ier.element_id
                inner join bw3.sys_cond_rela cr
                   on cr.ctl_id = a.element_id
                inner join bw3.sys_cond_operate co
        ...
        using (select distinct i.item_id, i.caption_sql
                 from bw3.sys_item_element_rela ier
                inner join bw3.sys_associate a
                   on a.element_id = ier.element_id
                inner join bw3.sys_tree_list tl
    ...
    delete from scmdata.sys_group_item_dw a
     where not exists
     (select 1 from bw3.sys_item a where a.item_id = a.item_id);
  ...
  begin
    ...
    --从【BW3.SYS_OPER_LOGS】中获取数据，取数规则：
    ...
    insert into scmdata.sys_group_oper_log_dw
      ...
      select scmdata.f_get_uuid_pe(), base.*
        from (select distinct a.user_id user_account,
               ...
               where trunc(a.opt_time, 'dd') = p_day
                 and a.opt_type = '302'
                 and (a.rest_desc = '操作执行成功' or a.rest_desc is null)
                 and a.uuid is not null) base;
    ...
    update scmdata.sys_group_oper_log_dw a
     ...
     where a.when_day = p_day
       ...
       and a.item_id in (select item_id from scmdata.sys_group_item_dw);
    --对于有多个归属的菜单，往上搜索指存在
    loop
      update scmdata.sys_group_oper_log_dw a
         set a.last_menu_name =
