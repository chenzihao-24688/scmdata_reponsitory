CREATE OR REPLACE PACKAGE SCMDATA.pkg_db_job IS
  ...
  --获取封存时间
  FUNCTION f_get_seal_date(p_rtn_type INT DEFAULT 0) RETURN VARCHAR2;

  --订单数据表
  --次月1~5号,返回上个月份
  FUNCTION f_get_month(p_begin_date DATE, p_seal_day INT) RETURN VARCHAR2;
  --通过分组获取区域组长
  ...
  * ALERTER  : 
  * ALERTER_TIME  : 
  * Purpose  : 基于订单交期数据的相关报表定时执行
  * Obj_Name    : p_related_order_report_execute
  * Arg_Number  : 0
  *============================================*/
  procedure p_related_order_report_execute ;
  --3 更新回货计划表的数据 begin
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_db_job IS
  ...
  --获取封存时间
  --p_rtn_type : 0 封存天数  1:封存日期
  FUNCTION f_get_seal_date(p_rtn_type INT DEFAULT 0) RETURN VARCHAR2 IS
    v_month     VARCHAR2(2);
    v_seal_days VARCHAR2(2);
    v_seal_date VARCHAR2(32);
  BEGIN
    v_month := to_char(SYSDATE, 'mm');
  
    SELECT to_char(MAX(t.seal_days))
      INTO v_seal_days
      FROM scmdata.t_order_seal_time t
    ...
    ELSIF p_rtn_type = 1 THEN
      v_seal_date := to_char(trunc(SYSDATE, 'mm') + v_seal_days,
                             'yyyy-mm-dd');
      RETURN v_seal_date;
    ELSE
      RETURN NULL;
  ...
  *============================================*/
  ...
  FUNCTION f_get_month(p_begin_date DATE, p_seal_day INT) RETURN VARCHAR2 IS
    v_date VARCHAR2(10);
  BEGIN
  
    --1）当月月初至当月5号 同步上月订单
    v_date := CASE
                WHEN SYSDATE BETWEEN p_begin_date AND
                     p_begin_date + p_seal_day THEN
  ...
  *============================================*/
  ...
  BEGIN
    ...
    v_seal_day := to_number(f_get_seal_date(p_rtn_type => 0));
    --当月月初至当月5号 同步上月订单
    v_cur_mon := f_get_month(p_begin_date => p_begin_date,
       ...
       MATCHED THEN
        UPDATE
           SET pta.year                   = ptb.year,
               ...
               pta.flw_order             =
               (CASE
                 WHEN pta.updated_flw = 1 AND pta.updated_flw IS NOT NULL THEN
                  pta.flw_order
                 ELSE
                  ptb.flw_order
               END),
               ...
               pta.is_sup_duty          = decode(pta.updated,
                                                 0,
                                                 ptb.is_sup_responsible,
                                                 pta.is_sup_duty),
               ...
               --'' /*ptb.arrival_date*/,
               --'' /*ptb.sort_date*/,
                ptb.isfirstordered, ptb.memo, ptb.finish_time_scm, 'ADMIN', SYSDATE, 'ADMIN', SYSDATE, ptb.goo_id_pr, ptb.sho_id, ptb.group_name, ptb.location_area, ptb.area_gp_leader, ptb.latest_planned_delivery_date, ptb.is_product_order, ptb.season);
  ...
  * Arg_Number  : 0
  *============================================*/
  procedure p_related_order_report_execute is
    v_seal_day varchar2(128);
  begin
   /*1~10号执行 基于订单交期数据的相关报表执行时间*/
    if to_char(sysdate, 'dd') < '10' then
      v_seal_day := scmdata.pkg_db_job.f_get_seal_date(p_rtn_type => 1);
     /*a.每月执行*/
      if to_char(sysdate, 'yyyy-mm-dd') = v_seal_day then
        --1.月度__每月执行
        ----1)kpi指标报表——维度:分类
        pkg_kpipt_order_proc.p_t_kpiorder_month_proc(t1_type => 0, p1_type => 1);
        ----2)kpi指标报表——维度:区域组
        pkg_kpipt_order_proc.p_t_kpiorder_qu_month_proc(t1_type => 0,p1_type => 1);
        ----3)总结类报表
        pkg_indicator_reached.p_indicator_reached_month;
        ----4)多维度交期指标查询报表
        scmdata.pkg_kpipt_order.p_t_kpi_month(t_type => 0, p_type => 1);
        ----5)供应商绩效盘点报表——维度:供应商
        scmdata.pkg_supplier_performance.p_supplier_performance_month(t_type => 1);
        ----6)供应商绩效盘点报表——维度:生产工厂
        scmdata.pkg_supplier_performance.p_factory_performance_month(t_type => 1);
     /*b.1月执行季度、半年度、年度*/    
        if to_char(sysdate, 'mm') = '01' then
          --2.季度__每季度执行
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_quarter_proc(t1_type => 0, p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_quarter_proc(t1_type => 0,p1_type => 1);
          ----3)总结类报表
          pkg_indicator_reached.p_indicator_reached_quarter;
          ----4)多维度交期指标查询报表
          scmdata.pkg_kpipt_order.p_t_kpi_quarter(t_type => 0, p_type => 1);
          ----5)供应商绩效盘点报表——维度:供应商
          scmdata.pkg_supplier_performance.p_supplier_performance_quarter(t_type => 1);
          ----6)供应商绩效盘点报表——维度:生产工厂
          scmdata.pkg_supplier_performance.p_factory_performance_quarter(t_type => 1);
          ----7)每季度_绩效考核维护模块定时生成生产工厂绩效评估报表
          scmdata.pkg_performance_maintenance.p_performance_report_quarter;
          --3.半年度
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_halfyear_proc(t1_type => 0, p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_halfyear_proc(t1_type => 0, p1_type => 1);
          ----3)总结类报表
          pkg_indicator_reached.p_indicator_reached_halfyear;
          ----4)多维度交期指标查询报表
          scmdata.pkg_kpipt_order.p_t_kpi_halfyear(t_type => 0, p_type => 1);
          ----5)供应商绩效盘点报表——维度:供应商
          scmdata.pkg_supplier_performance.p_supplier_performance_halfyear(t_type => 1);
          ----6)供应商绩效盘点报表——维度:生产工厂
          scmdata.pkg_supplier_performance.p_factory_performance_halfyear(t_type => 1);
          --4.年度
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_year_proc(t1_type => 0, p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          pkg_kpipt_order_proc.p_t_kpiorder_qu_year_proc(t1_type => 0, p1_type => 1);
          ----3)总结类报表
          pkg_indicator_reached.p_indicator_reached_year;
          ----4)多维度交期指标查询报表
          scmdata.pkg_kpipt_order.p_t_kpi_year(t_type => 0, p_type => 1);
          ----5)供应商绩效盘点报表——维度:供应商
          scmdata.pkg_supplier_performance.p_supplier_performance_year(t_type => 1);
          ----6)供应商绩效盘点报表——维度:生产工厂
          scmdata.pkg_supplier_performance.p_factory_performance_year(t_type => 1);
          ----7)每年度_绩效考核维护模块定时生成生产工厂绩效评估报表
          scmdata.pkg_performance_maintenance.p_performance_report_year;
      /*c.4、10月执行季度*/          
        elsif to_char(sysdate, 'mm') in ('04', '10') then
          --2.季度__每季度执行
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_quarter_proc(t1_type => 0, p1_type => 1);
          ----2)kpi指标报表——维度:区域组
          ...
          scmdata.pkg_supplier_performance.p_factory_performance_quarter(t_type => 1);
      /*d.7月执行季度、半年度*/         
        elsif to_char(sysdate, 'mm') = '07' then
          --2.季度__每季度执行
          ----1)kpi指标报表——维度:分类
          pkg_kpipt_order_proc.p_t_kpiorder_quarter_proc(t1_type => 0, p1_type => 1);
          ----2)kpi指标报表——维度:区域组
  ...
  end p_related_order_report_execute;
  ...
  BEGIN
    ...
    USING (
      ...
      WHEN NOT MATCHED THEN
        INSERT
          (pta.t_return_plan_id, pta.company_id, pta.order_id,
           pta.delivery_date, pta.year, pta.month, pta.menswear_amount,
           pta.womenswear_amount, pta.underwear_amount,
           pta.beautymakeup_amount, pta.shoesbags_amount, pta.taopin_amount,
           pta.warehouse, pta.create_id, pta.create_time, pta.update_id,
           pta.update_time)
        VALUES
          (scmdata.f_get_uuid(), ptb.company_id, ptb.order_code,
           ptb.latest_delivery_date, ptb.year, ptb.month, ptb.menswear,
           ptb.womenswear, ptb.underwear, ptb.beautymakeup, ptb.shoesbags,
           ptb.taopin, ptb.sho_id, 'ADMIN', SYSDATE, 'ADMIN', SYSDATE);
  EXCEPTION
    WHEN OTHERS THEN
      BEGIN
