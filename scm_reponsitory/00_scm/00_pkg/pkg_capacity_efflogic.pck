CREATE OR REPLACE PACKAGE SCMDATA.pkg_capacity_efflogic IS
  ...
  /*===================================================================================
    ...
    入参:
      ...
      v_inp_is_weekplan_recalc   :  周产能是否重算
      v_inp_is_suppieces_reclac  :  供应商件数是否重算
      v_inp_company_id           :  企业Id
  
    版本:
      2022-08-25 : 队列派生-生成周产能规划重算队列
      2023-05-08 : 生成周产能规划重算队列新增周产能是否重算，供应商件数
                   是否重算列增加
  ...
  (
    ...
    v_inp_faccode             IN VARCHAR2 DEFAULT NULL,
    v_inp_cate                IN VARCHAR2,
    v_inp_product_cate        IN VARCHAR2 DEFAULT NULL,
    v_inp_subcategory         IN VARCHAR2 DEFAULT NULL,
    v_inp_operate_id          IN VARCHAR2,
    v_inp_operate_time        IN VARCHAR2,
    v_inp_is_weekplan_recalc  IN NUMBER DEFAULT 1,
    v_inp_is_suppieces_reclac IN NUMBER DEFAULT 1,
  ...
  /*===================================================================================
     ...
     版本:
       2023-06-05 : 工厂生产分类占比配置初始值刷新
      
  ===================================================================================*/
  PROCEDURE p_initialize_capc_fac_procate_rate_zero_value
  (
  ...
  /*===================================================================================
     ...
     版本:
       2023-05-08 : 工厂生产分类占比配置生效逻辑
    
  ===================================================================================*/
  PROCEDURE p_capc_fac_procate_rate_cfg_efflogic
  (
    v_inp_queue_id   IN VARCHAR2,
    v_inp_cond       IN VARCHAR2,
    v_inp_company_id IN VARCHAR2
  ...
  /*===================================================================================
     ...
     版本:
       2023-06-05 : 供应商产品子类占比配置初始值刷新
      
  ===================================================================================*/
  PROCEDURE p_initialize_capc_sup_subcate_rate_zero_value
  (
    v_inp_supplier_code IN VARCHAR2,
    v_inp_category      IN VARCHAR2,
  ...
  /*===================================================================================
     ...
     版本:
       2023-05-15 : 供应商产品子类占比配置生效逻辑
    
  ===================================================================================*/
  PROCEDURE p_capc_sup_subcate_rate_cfg_efflogic
  (
    v_inp_queue_id   IN VARCHAR2,
    v_inp_cond       IN VARCHAR2,
    v_inp_company_id IN VARCHAR2
  ...
  /*===================================================================================
     ...
     版本:
       2023-06-07 : 根据不同参数对工厂生产分类占比配置初始值刷新
      
  ===================================================================================*/
  PROCEDURE p_initialize_capc_fac_procate_rate_zero_value_by_diffparam
  (
  ...
  /*===================================================================================
     ...
     版本:
       2023-06-07 : 根据不同参数对供应商产品子类占比配置初始值刷新
      
  ===================================================================================*/
  PROCEDURE p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam
  (
    v_inp_supplier_code IN VARCHAR2 DEFAULT NULL,
    v_inp_category      IN VARCHAR2 DEFAULT NULL,
    v_inp_product_cate  IN VARCHAR2 DEFAULT NULL,
  ...
  /*=============================================================================
     ...
     版本:
       2023-05-10_zc314 : 获取供应商周产能规划(件数)基表datasql
          
  ==============================================================================*/
  FUNCTION f_get_weekplan_pieces_recalc_datasql
  (
    v_inp_supplier_code IN VARCHAR2,
    v_inp_factory_code  IN VARCHAR2 DEFAULT NULL,
    v_inp_category      IN VARCHAR2,
  ...
  /*=============================================================================
     ...
     版本:
       2023-05-11_zc314 : 供应商周产能规划(件数)重算逻辑
            
  ==============================================================================*/
  PROCEDURE p_weekplan_pieces_recalc
  (
    v_inp_supplier_code IN VARCHAR2,
    v_inp_factory_code  IN VARCHAR2 DEFAULT NULL,
    v_inp_category      IN VARCHAR2,
...
END pkg_capacity_efflogic;
/

CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_capacity_efflogic IS
 ...
 GROUP BY SUPPLIER_CODE, FACTORY_CODE, COOP_CATEGORY';
        ...
        IF v_jugnum = 0 THEN
          ...
          VALUES
            (scmdata.f_get_uuid(), v_compid, v_cwpid, n.coop_category, n.coop_productcate, n.coop_subcategory, n.supplier_code, n.factory_code, i.year, i.week, i.day, i.work_hour, i.work_people, i.appcapacity_rate, i.prod_effient, trunc(i.work_hour * 60 *
                    i.work_people *
                    i.prod_effient / 100), trunc(i.work_hour * 60 *
                    i.work_people *
                    i.prod_effient / 100 *
                    i.appcapacity_rate / 100), v_scspfcdayrate, trunc(i.work_hour * 60 *
                    i.work_people *
                    i.prod_effient / 100 *
                    v_scspfcdayrate), trunc(i.work_hour * 60 * i.work_people *
                    i.prod_effient / 100 *
  ...
  BEGIN
    ...
    SELECT nvl(MAX(1), 0)
     ...
     WHERE status = 'PR'
       AND supplier_code = v_inp_supcode
       AND category = v_inp_cate
       ...
       AND EXISTS (SELECT 1
             ...
             WHERE queue_id = tab.queue_id
               AND company_id = tab.company_id)
          --zc314_add_2305241614_begin
       AND NOT EXISTS
     (SELECT 1
              FROM scmdata.t_queue que
             INNER JOIN scmdata.t_queue_iflrows queifl
                ON que.queue_id = queifl.queue_id
               AND que.company_id = queifl.company_id
             WHERE que.queue_status = 'PR'
               AND nvl(queifl.ir_colvalue1, ' ') = nvl(v_inp_supcode, ' ')
               AND nvl(queifl.ir_colvalue2, ' ') = nvl(v_inp_faccode, ' ')
               AND nvl(queifl.ir_colvalue3, ' ') = nvl(v_inp_cate, ' ')
               AND nvl(queifl.ir_colvalue4, ' ') =
                   nvl(v_inp_product_cate, ' ')
               AND nvl(queifl.ir_colvalue5, ' ') =
                   nvl(v_inp_subcategory, ' ')
               AND nvl(queifl.company_id, ' ') = nvl(v_inp_company_id, ' '))
          --zc314_add_2305241614_end
       AND rownum = 1;
    ...
    IF v_jugnum = 0 THEN
      ...
      VALUES
        (v_queueid, v_inp_company_id, 'PR', 'SCM_CAPCWKPLAN_RECALC', v_inp_operate_id, to_date(v_inp_operate_time,
                  'YYYY-MM-DD HH24-MI-SS'));
    
      --周产能和供应商周产能件数是否重算判断
      IF v_inp_is_weekplan_recalc = 1
         AND v_inp_is_suppieces_reclac = 1 THEN
        --周产能重算队列新增
        INSERT INTO scmdata.t_capc_wkplanrecalcqueue
          (cwpn_id, company_id, status, queue_id, supplier_code, category, create_id, create_time, is_weekplan_recalc, is_suppieces_recalc)
        VALUES
          (scmdata.f_get_uuid(), v_inp_company_id, 'PR', v_queueid, v_inp_supcode, v_inp_cate, v_inp_operate_id, to_date(v_inp_operate_time,
                    'YYYY-MM-DD HH24-MI-SS'), v_inp_is_weekplan_recalc, v_inp_is_suppieces_reclac);
      
        --影响行生成
        scmdata.pkg_capacity_iflrow.p_common_queueifldata_generator(v_tab     => 'SCM_CAPC_WEEKPLAN_RELA',
                                                                    v_supcode => v_inp_supcode,
                                                                    v_cate    => v_inp_cate,
                                                                    v_queueid => v_queueid,
                                                                    v_compid  => v_inp_company_id);
      ELSIF v_inp_is_weekplan_recalc = 0
            AND v_inp_is_suppieces_reclac = 1 THEN
        --周产能重算队列新增
        INSERT INTO scmdata.t_capc_wkplanrecalcqueue
          (cwpn_id, company_id, status, queue_id, supplier_code, category, create_id, create_time, is_weekplan_recalc, is_suppieces_recalc)
        ...
        scmdata.pkg_capacity_iflrow.p_common_queueifldata_generator(v_tab     => 'SCM_CAPC_WEEKPLAN_RELA',
                                                                    v_supcode => v_inp_supcode,
                                                                    v_faccode => v_inp_faccode,
                                                                    v_cate    => v_inp_cate,
                                                                    v_procate => v_inp_product_cate,
                                                                    v_subcate => v_inp_subcategory,
                                                                    v_queueid => v_queueid,
                                                                    v_compid  => v_inp_company_id);
      END IF;
    END IF;
  ...
  (
    ...
    v_is_weekplan_recalc  NUMBER(1);
    v_is_suppieces_recalc NUMBER(1);
  BEGIN
    --周产能规划重算队列内，供应商编码，分类编码，创建人，创建时间，
    --是否需要重算周产能规划，是否需要重算供应商产能件数
    SELECT MAX(supplier_code),
           MAX(category),
           MAX(create_id),
           MAX(to_char(create_time, 'YYYY-MM-DD HH24-MI-SS')),
           MAX(is_weekplan_recalc),
           MAX(is_suppieces_recalc)
      INTO v_supcode,
           v_cate,
           v_operid,
           v_opertime,
           v_is_weekplan_recalc,
           v_is_suppieces_recalc
      FROM scmdata.t_capc_wkplanrecalcqueue
     WHERE queue_id = v_queueid
       AND company_id = v_compid;
  
    --需要重算周产能规划
    IF nvl(v_is_weekplan_recalc, 0) = 1 THEN
    ...
    --供应商周产能规划(件数)需要重算
    IF nvl(v_is_suppieces_recalc, 0) = 1 THEN
      --获取分类，生产分类，产品子类，供应商编码，生产工厂编码
      SELECT MAX(CASE
                   WHEN ir_colname3 = 'CATEGORY' THEN
                    ir_colvalue3
                 END),
             MAX(CASE
                   WHEN ir_colname4 = 'PRODUCT_CATE' THEN
                    ir_colvalue4
                 END),
             MAX(CASE
                   WHEN ir_colname5 = 'SUBCATEGORY' THEN
                    ir_colvalue5
                 END),
             MAX(CASE
                   WHEN ir_colname1 = 'SUPPLIER_CODE' THEN
                    ir_colvalue1
                 END),
             MAX(CASE
                   WHEN ir_colname2 = 'FACTORY_CODE' THEN
                    ir_colvalue2
                 END)
        INTO v_category,
             v_product_cate,
      ...
      --供应商编码，分类编码不为空
      IF v_supplier_code IS NOT NULL
         AND v_category IS NOT NULL THEN
        --重算
        p_weekplan_pieces_recalc(v_inp_supplier_code => v_supplier_code,
  ...
  BEGIN
    ...
    --工厂生产类别比例配置数据生成_zc314_2305221328_add
    scmdata.pkg_capacity_management.p_gen_capc_fac_procate_rate_cfg_data(v_inp_supplier_code  => v_supcode,
                                                                         v_inp_factory_code   => v_faccode,
                                                                         v_inp_category       => v_cate,
                                                                         v_inp_product_cate   => v_procate,
                                                                         v_inp_company_id     => v_compid,
                                                                         v_inp_operate_userid => v_operid,
                                                                         v_inp_operate_time   => to_date(v_opertime,
                                                                                                         'yyyy-mm-dd hh24-mi-ss'),
                                                                         v_inp_generate_mode  => 'I');
  
    --工厂生产分类占比配置初始值刷新_zc314_2306071441_add
    p_initialize_capc_fac_procate_rate_zero_value(v_inp_supplier_code => v_supcode,
                                                  v_inp_factory_code  => v_faccode,
                                                  v_inp_category      => v_cate,
                                                  v_inp_operate_id    => v_operid,
                                                  v_inp_operate_time  => v_opertime,
                                                  v_inp_company_id    => v_compid);
  
    --供应商产品子类比例配置数据生成_zc314_2305221328_add
    scmdata.pkg_capacity_management.p_gen_capc_sup_subcate_rate_cfg_data(v_inp_supplier_code  => v_supcode,
                                                                         v_inp_category       => v_cate,
                                                                         v_inp_product_cate   => v_procate,
                                                                         v_inp_subcategory    => v_subcate,
                                                                         v_inp_company_id     => v_compid,
                                                                         v_inp_operate_userid => v_operid,
                                                                         v_inp_operate_time   => to_date(v_opertime,
                                                                                                         'yyyy-mm-dd hh24-mi-ss'),
    ...
    --供应商产品子类占比配置初始值刷新_zc314_2306071441_add
    p_initialize_capc_sup_subcate_rate_zero_value(v_inp_supplier_code => v_supcode,
                                                  v_inp_category      => v_cate,
                                                  v_inp_product_cate  => v_procate,
                                                  v_inp_operate_id    => v_operid,
                                                  v_inp_operate_time  => v_opertime,
  ...
  BEGIN
    ...
    --周产能规划重算
    p_gen_wkplanrecalcrela(v_inp_supcode      => v_supcode,
                           v_inp_cate         => v_cate,
                           v_inp_operate_id   => v_operid,
                           v_inp_operate_time => v_opertime,
                           v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    FOR i IN (SELECT DISTINCT supplier_code,
               ...
               WHERE coop_category = v_cate
                 ...
                 AND company_id = v_compid) LOOP
      --生成周产能相关数据
      p_gen_wkplanrecalcrela(v_inp_supcode      => i.supplier_code,
                             v_inp_cate         => i.coop_category,
                             v_inp_operate_id   => v_opid,
                             v_inp_operate_time => to_char(v_optime,
                                                           'YYYY-MM-DD HH24:MI:SS'),
                             v_inp_company_id   => i.company_id);
  ...
  BEGIN
    ...
    IF v_method = 'INS' THEN
      ...
      --工厂生产类别比例配置数据生成_zc314_2305221328_add
      scmdata.pkg_capacity_management.p_gen_capc_fac_procate_rate_cfg_data(v_inp_category       => v_curcate,
                                                                           v_inp_product_cate   => v_curprocate,
                                                                           v_inp_company_id     => v_curcompid,
                                                                           v_inp_operate_userid => v_creid,
                                                                           v_inp_operate_time   => SYSDATE,
                                                                           v_inp_generate_mode  => 'I');
    
      --根据不同参数对工厂生产分类占比配置初始值刷新
      p_initialize_capc_fac_procate_rate_zero_value_by_diffparam(v_inp_category     => v_curcate,
                                                                 v_inp_company_id   => v_curcompid,
                                                                 v_inp_operate_id   => v_creid,
                                                                 v_inp_operate_time => to_char(SYSDATE,
                                                                                               'yyyy-mm-dd hh24-mi-ss'));
    
      --供应商产品子类比例配置数据生成_zc314_2305221328_add
      scmdata.pkg_capacity_management.p_gen_capc_sup_subcate_rate_cfg_data(v_inp_category       => v_curcate,
                                                                           v_inp_product_cate   => v_curprocate,
                                                                           v_inp_subcategory    => v_cursubcate,
                                                                           v_inp_company_id     => v_curcompid,
                                                                           v_inp_operate_userid => v_creid,
                                                                           v_inp_operate_time   => SYSDATE,
                                                                           v_inp_generate_mode  => 'I');
      ...
      --根据不同参数对供应商产品子类占比配置初始值刷新
      p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam(v_inp_category     => v_curcate,
                                                                 v_inp_product_cate => v_curprocate,
                                                                 v_inp_company_id   => v_curcompid,
                                                                 v_inp_operate_id   => v_creid,
                                                                 v_inp_operate_time => to_char(SYSDATE,
    ...
    ELSIF v_method = 'UPD' THEN
      ...
      --选出分类-生产分类-子类
      v_exesql := 'SELECT MAX(CATEGORY), MAX(PRODUCT_CATE), MAX(SUBCATEGORY), MAX(IN_PLANNING) FROM SCMDATA.T_CAPACITY_PLAN_CATEGORY_CFG WHERE ' ||
                  v_cond;
      EXECUTE IMMEDIATE v_exesql
      ...
      IF v_rawsubcate IS NOT NULL THEN
        ...
        IF v_inplnn = 1 THEN
          --工厂生产类别比例配置数据生成_zc314_2305221328_add
          scmdata.pkg_capacity_management.p_gen_capc_fac_procate_rate_cfg_data(v_inp_category       => v_cate,
                                                                               v_inp_product_cate   => v_procate,
                                                                               v_inp_company_id     => v_compid,
                                                                               v_inp_operate_userid => v_updid,
                                                                               v_inp_operate_time   => SYSDATE,
                                                                               v_inp_generate_mode  => 'I');
        
          --根据不同参数对工厂生产分类占比配置初始值刷新
          p_initialize_capc_fac_procate_rate_zero_value_by_diffparam(v_inp_category     => v_cate,
                                                                     v_inp_company_id   => v_compid,
                                                                     v_inp_operate_id   => v_updid,
                                                                     v_inp_operate_time => to_char(SYSDATE,
                                                                                                   'yyyy-mm-dd hh24-mi-ss'));
        
          --供应商产品子类比例配置数据生成_zc314_2305221328_add
          scmdata.pkg_capacity_management.p_gen_capc_sup_subcate_rate_cfg_data(v_inp_category       => v_cate,
                                                                               v_inp_product_cate   => v_procate,
                                                                               v_inp_subcategory    => v_subcate,
                                                                               v_inp_company_id     => v_compid,
                                                                               v_inp_operate_userid => v_updid,
          ...
          --根据不同参数对供应商产品子类占比配置初始值刷新
          p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam(v_inp_category     => v_cate,
                                                                     v_inp_product_cate => v_procate,
                                                                     v_inp_company_id   => v_compid,
                                                                     v_inp_operate_id   => v_updid,
                                                                     v_inp_operate_time => to_char(SYSDATE,
  ...
  BEGIN
    ...
    IF v_pause = 0 THEN
      FOR i IN (SELECT csc.supplier_code,
        ...
        --工厂生产类别比例配置数据生成
        scmdata.pkg_capacity_management.p_gen_capc_fac_procate_rate_cfg_data(v_inp_supplier_code  => i.supplier_code,
                                                                             v_inp_factory_code   => i.factory_code,
                                                                             v_inp_category       => i.coop_category,
                                                                             v_inp_product_cate   => i.coop_productcate,
                                                                             v_inp_company_id     => i.company_id,
                                                                             v_inp_operate_userid => i.create_id,
                                                                             v_inp_operate_time   => SYSDATE,
                                                                             v_inp_generate_mode  => 'I');
      
        --根据不同参数对工厂生产分类占比配置初始值刷新
        p_initialize_capc_fac_procate_rate_zero_value_by_diffparam(v_inp_supplier_code => i.supplier_code,
                                                                   v_inp_factory_code  => i.factory_code,
                                                                   v_inp_category      => i.coop_category,
                                                                   v_inp_company_id    => i.company_id,
                                                                   v_inp_operate_id    => i.create_id,
                                                                   v_inp_operate_time  => to_char(SYSDATE,
                                                                                                  'yyyy-mm-dd hh24-mi-ss'));
      
        --供应商产品子类比例配置数据生成
        scmdata.pkg_capacity_management.p_gen_capc_sup_subcate_rate_cfg_data(v_inp_supplier_code  => i.supplier_code,
                                                                             v_inp_category       => i.coop_category,
                                                                             v_inp_product_cate   => i.coop_productcate,
                                                                             v_inp_subcategory    => i.coop_subcategory,
                                                                             v_inp_company_id     => i.company_id,
                                                                             v_inp_operate_userid => i.create_id,
                                                                             v_inp_operate_time   => SYSDATE,
                                                                             v_inp_generate_mode  => 'I');
        ...
        --根据不同参数对供应商产品子类占比配置初始值刷新
        p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam(v_inp_supplier_code => i.supplier_code,
                                                                   v_inp_category      => i.coop_category,
                                                                   v_inp_product_cate  => i.coop_productcate,
                                                                   v_inp_company_id    => i.company_id,
                                                                   v_inp_operate_id    => i.create_id,
  ...
  BEGIN
    ...
    IF v_pausestr IS NOT NULL THEN
      ...
      IF v_pause = 1 THEN
        FOR x IN (SELECT csc.supplier_code,
                         cfc.factory_code,
                         csc.coop_category,
                         csc.company_id
                    FROM scmdata.t_coopcate_supplier_cfg csc
        ...
        END LOOP;
      ELSE
        FOR y IN (SELECT csc.supplier_code,
                         cfc.factory_code,
                         csc.coop_category,
                         csc.coop_productcate,
                         csc.coop_subcategory,
                   ...
                   WHERE csc.csc_id = v_cscid
                     AND csc.company_id = v_compid) LOOP
          --工厂生产类别比例配置数据生成
          scmdata.pkg_capacity_management.p_gen_capc_fac_procate_rate_cfg_data(v_inp_supplier_code  => y.supplier_code,
                                                                               v_inp_factory_code   => y.factory_code,
                                                                               v_inp_category       => y.coop_category,
                                                                               v_inp_product_cate   => y.coop_productcate,
                                                                               v_inp_company_id     => y.company_id,
                                                                               v_inp_operate_userid => v_updateid,
                                                                               v_inp_operate_time   => SYSDATE,
                                                                               v_inp_generate_mode  => 'I');
        
          --根据不同参数对工厂生产分类占比配置初始值刷新
          p_initialize_capc_fac_procate_rate_zero_value_by_diffparam(v_inp_supplier_code => y.supplier_code,
                                                                     v_inp_factory_code  => y.factory_code,
                                                                     v_inp_category      => y.coop_category,
                                                                     v_inp_company_id    => y.company_id,
                                                                     v_inp_operate_id    => v_updateid,
                                                                     v_inp_operate_time  => to_char(SYSDATE,
                                                                                                    'yyyy-mm-dd hh24-mi-ss'));
        
          --供应商产品子类比例配置数据生成
          scmdata.pkg_capacity_management.p_gen_capc_sup_subcate_rate_cfg_data(v_inp_supplier_code  => y.supplier_code,
                                                                               v_inp_category       => y.coop_category,
                                                                               v_inp_product_cate   => y.coop_productcate,
                                                                               v_inp_subcategory    => y.coop_subcategory,
                                                                               v_inp_company_id     => y.company_id,
                                                                               v_inp_operate_userid => v_updateid,
                                                                               v_inp_operate_time   => SYSDATE,
                                                                               v_inp_generate_mode  => 'I');
          ...
          --根据不同参数对供应商产品子类占比配置初始值刷新
          p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam(v_inp_supplier_code => y.supplier_code,
                                                                     v_inp_category      => y.coop_category,
                                                                     v_inp_product_cate  => y.coop_productcate,
                                                                     v_inp_company_id    => y.company_id,
                                                                     v_inp_operate_id    => v_updateid,
      ...
      IF v_tmpsupcode IS NOT NULL
        ...
        p_gen_wkplanrecalcrela(v_inp_supcode      => v_tmpsupcode,
                               v_inp_cate         => v_tmpcate,
                               v_inp_operate_id   => v_updateid,
                               v_inp_operate_time => v_updatetime,
                               v_inp_company_id   => v_compid);
      END IF;
    END IF;
  ...
  BEGIN
    ...
    IF v_pausestr IS NOT NULL THEN
      ...
      p_gen_wkplanrecalcrela(v_inp_supcode      => v_rcsupcode,
                             v_inp_cate         => v_rccate,
                             v_inp_operate_id   => v_updateid,
                             v_inp_operate_time => v_updatetime,
                             v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    FOR y IN (SELECT DISTINCT supplier_code,
               ...
               WHERE coop_category = v_cate
                 AND company_id = v_compid) LOOP
      p_gen_wkplanrecalcrela(v_inp_supcode      => y.supplier_code,
                             v_inp_cate         => y.coop_category,
                             v_inp_operate_id   => v_operid,
                             v_inp_operate_time => v_opertime,
                             v_inp_company_id   => y.company_id);
    ...
    LOOP
      ...
      IF v_rcsupcode IS NOT NULL THEN
        ...
        IF nvl(v_tmpsupcode, ' ') <> v_rcsupcode
          ...
          p_gen_wkplanrecalcrela(v_inp_supcode      => v_rcsupcode,
                                 v_inp_cate         => v_rccate,
                                 v_inp_operate_id   => nvl(v_updateid,
                                                           v_createid),
                                 v_inp_operate_time => nvl(v_updatetime,
                                                           v_createtime),
                                 v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    FOR i IN (SELECT supplier_code,
                     ...
                     company_id,
                     CASE
                       WHEN allocate_percentage <> 0 THEN
                        allocate_percentage
                       WHEN allocate_percentage = 0
                            AND rn <> cnt THEN
                        round((100 - allo_sum) / cnt, 2)
                       WHEN allocate_percentage = 0
                            AND rn = cnt THEN
                        round(100 - allo_sum -
                              ((100 - allo_sum) / cnt * (cnt - 1)),
                              2)
                ...
                FROM (SELECT supplier_code,
                             ...
                             company_id,
                             allocate_percentage,
                             SUM(allocate_percentage) over(PARTITION BY supplier_code, factory_code, category, company_id) allo_sum,
                             SUM(CASE
                                   WHEN allocate_percentage = 0 THEN
                                    1
                                   ELSE
                                    0
                                 END) over(PARTITION BY supplier_code, factory_code, category, company_id) cnt,
                             CASE
                               WHEN allocate_percentage = 0 THEN
                                1
                               ELSE
                                0
                             END is_zero_percentage,
                             rownum rn
                       ...
                       WHERE supplier_code = v_inp_supplier_code
                         ...
                         AND company_id = v_inp_company_id)
               WHERE is_zero_percentage = 1) LOOP
      --更新分配比例
      UPDATE scmdata.t_capc_fac_procate_rate_cfg
         SET allocate_percentage = i.allocate_percentage
       WHERE supplier_code = i.supplier_code
         AND factory_code = i.factory_code
         AND category = i.category
         AND product_cate = i.product_cate
      ...
      --新增进入供应商周产能规划(件数)重算
      p_gen_wkplanrecalcrela(v_inp_supcode             => i.supplier_code,
                             v_inp_faccode             => i.factory_code,
                             v_inp_cate                => i.category,
                             v_inp_product_cate        => i.product_cate,
                             v_inp_operate_id          => v_inp_operate_id,
                             v_inp_operate_time        => v_inp_operate_time,
                             v_inp_is_weekplan_recalc  => 0,
                             v_inp_is_suppieces_reclac => 1,
                             v_inp_company_id          => i.company_id);
    END LOOP;
  END p_initialize_capc_fac_procate_rate_zero_value;
  ...
  (
    ...
    v_category            VARCHAR2(2);
    v_product_cate        VARCHAR2(4);
    v_allocate_percentage NUMBER(5, 2);
    v_update_id           VARCHAR2(32);
    v_update_time         DATE;
    v_sql                 CLOB;
    v_self_description    VARCHAR2(1024) := 'scmdata.pkg_capacity_efflogic.p_capc_fac_procate_rate_cfg_efflogic';
  BEGIN
    --获取队列中变化的分配占比，修改人，修改时间
    SELECT MAX(CASE
                 WHEN vc_col = 'ALLOCATE_PERCENTAGE' THEN
                  vc_curval
               END),
           MAX(CASE
                 WHEN vc_col = 'UPDATE_ID' THEN
    ...
    --获取配置Id，企业Id，供应商编码，生产工厂编码，分类编码，生产分类编码
    v_sql := 'SELECT MAX(cfprc_id), MAX(company_id), MAX(supplier_code), MAX(factory_code), MAX(category), MAX(product_cate) FROM ' ||
             'scmdata.t_capc_fac_procate_rate_cfg WHERE ' || v_inp_cond;
  
    --执行 Sql
    EXECUTE IMMEDIATE v_sql
      INTO v_cfprc_id, v_company_id, v_supplier_code, v_factory_code, v_category, v_product_cate;
  
    IF v_update_id IS NULL THEN
      --获取udpate_id
      v_sql := 'SELECT MAX(operate_id) FROM scmdata.t_capc_fac_procate_rate_cfg_view WHERE ' ||
               v_inp_cond;
    ...
    --更新逻辑
    scmdata.pkg_capacity_management.p_tabapi_upd_capc_fac_procate_rate_cfg(v_inp_cfprc_id            => v_cfprc_id,
                                                                           v_inp_allocate_percentage => v_allocate_percentage,
                                                                           v_inp_update_id           => v_update_id,
                                                                           v_inp_update_time         => v_update_time,
                                                                           v_inp_company_id          => v_company_id,
                                                                           v_inp_invoke_object       => v_self_description);
  
    --数据刷新
    p_initialize_capc_fac_procate_rate_zero_value(v_inp_supplier_code => v_supplier_code,
                                                  v_inp_factory_code  => v_factory_code,
                                                  v_inp_category      => v_category,
                                                  v_inp_operate_id    => v_update_id,
                                                  v_inp_operate_time  => to_char(v_update_time,
                                                                                 'yyyy-mm-dd hh24-mi-ss'),
                                                  v_inp_company_id    => v_company_id);
  
    --新增进入供应商周产能规划(件数)重算
    p_gen_wkplanrecalcrela(v_inp_supcode             => v_supplier_code,
                           v_inp_faccode             => v_factory_code,
                           v_inp_cate                => v_category,
                           v_inp_product_cate        => v_product_cate,
                           v_inp_operate_id          => v_update_id,
                           v_inp_operate_time        => to_char(v_update_time,
                                                                'yyyy-mm-dd hh24-mi-ss'),
                           ...
                           v_inp_is_suppieces_reclac => 1,
                           v_inp_company_id          => v_company_id);
  END p_capc_fac_procate_rate_cfg_efflogic;
  ...
  BEGIN
    ...
    FOR i IN (SELECT supplier_code,
                ...
                FROM (SELECT supplier_code,
                             ...
                             company_id,
                             allocate_percentage,
                             SUM(allocate_percentage) over(PARTITION BY supplier_code, category, product_cate, company_id) allo_sum,
                             SUM(CASE
                                   WHEN allocate_percentage = 0 THEN
                                    1
                                   ELSE
                                    0
                                 END) over(PARTITION BY supplier_code, category, product_cate, company_id) cnt,
                             CASE
                               WHEN allocate_percentage = 0 THEN
                                1
                               ELSE
               ...
               WHERE is_zero_percentage = 1) LOOP
      --数据更新
      UPDATE scmdata.t_capc_sup_subcate_rate_cfg
         SET allocate_percentage = i.allocate_percentage
       WHERE supplier_code = i.supplier_code
         AND category = i.category
         AND product_cate = i.product_cate
      ...
      p_gen_wkplanrecalcrela(v_inp_supcode             => i.supplier_code,
                             v_inp_cate                => i.category,
                             v_inp_product_cate        => i.product_cate,
                             v_inp_subcategory         => i.subcategory,
                             v_inp_operate_id          => v_inp_operate_id,
                             v_inp_operate_time        => v_inp_operate_time,
                             v_inp_is_weekplan_recalc  => 0,
                             v_inp_is_suppieces_reclac => 1,
                             v_inp_company_id          => i.company_id);
    END LOOP;
  END p_initialize_capc_sup_subcate_rate_zero_value;
  ...
  (
    ...
    v_update_time         DATE;
    v_sql                 CLOB;
    v_self_description    VARCHAR2(1024) := 'scmdata.pkg_capacity_efflogic.p_capc_sup_subcate_rate_cfg_efflogic';
  BEGIN
    --获取队列中变化的分配占比，修改人，修改时间
    SELECT MAX(CASE
                 WHEN vc_col = 'ALLOCATE_PERCENTAGE' THEN
    ...
    --获取配置Id，企业Id，供应商编码，生产工厂编码，分类编码，生产分类编码
    v_sql := 'SELECT MAX(cssrc_id), MAX(company_id), MAX(supplier_code), MAX(category), MAX(product_cate), MAX(subcategory) FROM ' ||
             'scmdata.t_capc_sup_subcate_rate_cfg WHERE ' || v_inp_cond;
  
    --执行 Sql
    EXECUTE IMMEDIATE v_sql
      INTO v_cssrc_id, v_company_id, v_supplier_code, v_category, v_product_cate, v_subcategory;
  
    IF v_update_id IS NULL THEN
      --获取udpate_id
      v_sql := 'SELECT MAX(operate_id) FROM scmdata.t_capc_sup_subcate_rate_cfg_view WHERE ' ||
               v_inp_cond;
    ...
    --更新逻辑
    scmdata.pkg_capacity_management.p_tabapi_upd_capc_sup_subcate_rate_cfg(v_inp_cssrc_id            => v_cssrc_id,
                                                                           v_inp_company_id          => v_company_id,
                                                                           v_inp_allocate_percentage => v_allocate_percentage,
                                                                           v_inp_update_id           => v_update_id,
                                                                           v_inp_update_time         => v_update_time,
    ...
    --数据刷新
    p_initialize_capc_sup_subcate_rate_zero_value(v_inp_supplier_code => v_supplier_code,
                                                  v_inp_category      => v_category,
                                                  v_inp_product_cate  => v_product_cate,
                                                  v_inp_operate_id    => v_update_id,
                                                  v_inp_operate_time  => to_char(v_update_time,
    ...
    p_gen_wkplanrecalcrela(v_inp_supcode             => v_supplier_code,
                           ...
                           v_inp_is_suppieces_reclac => 1,
                           v_inp_company_id          => v_company_id);
  END p_capc_sup_subcate_rate_cfg_efflogic;
  ...
  BEGIN
    ...
    --条件不为空
    IF v_conds IS NOT NULL THEN
      v_conds := ' WHERE ' || v_conds;
    END IF;
  
    --构建 Sql
    v_sql := 'SELECT supplier_code, factory_code, category, company_id FROM scmdata.t_capc_fac_procate_rate_cfg ' ||
             v_conds ||
             ' GROUP BY supplier_code, factory_code, category, company_id';
    ...
    LOOP
      ...
      --游标展开数据不为空
      IF v_supplier_code IS NOT NULL
         AND v_factory_code IS NOT NULL
    ...
    --游标关闭
    CLOSE cur;
  END p_initialize_capc_fac_procate_rate_zero_value_by_diffparam;
  ...
  BEGIN
    ...
    --构建 Sql 
    v_sql := 'SELECT supplier_code, category, product_cate, company_id FROM scmdata.t_capc_sup_subcate_rate_cfg ' ||
             v_conds ||
             ' GROUP BY supplier_code, category, product_cate, company_id';
    ...
    LOOP
      ...
      IF v_supplier_code IS NOT NULL
         AND v_category IS NOT NULL
         AND v_product_cate IS NOT NULL
         AND v_company_id IS NOT NULL THEN
        --供应商产品子类占比配置初始值刷新
        p_initialize_capc_sup_subcate_rate_zero_value(v_inp_supplier_code => v_supplier_code,
                                                      v_inp_category      => v_category,
    ...
    --游标关闭
    CLOSE cur;
  END p_initialize_capc_sup_subcate_rate_zero_value_by_diffparam;
  ...
  BEGIN
    ...
    IF v_vcnotworkdayr IS NOT NULL
      ...
      FOR z IN (SELECT DISTINCT supplier_code,
                 ...
                 WHERE supplier_code = v_supcode
                   AND company_id = v_compid) LOOP
        p_gen_wkplanrecalcrela(v_inp_supcode      => z.supplier_code,
                               v_inp_cate         => z.coop_category,
                               v_inp_operate_id   => v_vcupdid,
                               v_inp_operate_time => v_vcupdtime,
                               v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    IF v_method = 'INS' THEN
      ...
      IF nvl(v_wkcapcrealc, 1) = 0 THEN
        p_gen_wkplanrecalcrela(v_inp_supcode      => v_vcsupcode,
                               v_inp_cate         => v_vccate,
                               v_inp_operate_id   => v_vccreid,
                               v_inp_operate_time => v_vccretime,
                               v_inp_company_id   => v_vccompid);
    ...
    ELSIF v_method = 'UPD' THEN
      ...
      IF nvl(v_wkcapcrealc, 1) = 0 THEN
        ...
        IF v_rawsupcode IS NOT NULL THEN
          p_gen_wkplanrecalcrela(v_inp_supcode      => v_rawsupcode,
                                 v_inp_cate         => v_rawcate,
                                 v_inp_operate_id   => nvl(v_vcupdid,
                                                           v_vccreid),
                                 v_inp_operate_time => nvl(v_vcupdtime,
                                                           v_vccretime),
                                 v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    IF v_cursupcode IS NOT NULL THEN
      ...
      p_gen_wkplanrecalcrela(v_inp_supcode      => v_cursupcode,
                             v_inp_cate         => v_cate,
                             v_inp_operate_id   => v_operid,
                             v_inp_operate_time => v_opertime,
                             v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    p_gen_wkplanrecalcrela(v_inp_supcode      => v_supcode,
                           v_inp_cate         => v_cate,
                           v_inp_operate_id   => v_vcoperid,
                           v_inp_operate_time => v_vcopertime,
                           v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    FOR x IN (SELECT csc_id,
      ...
      IF v_supcode IS NOT NULL
        ...
        --供应商产能预约
        scmdata.pkg_capacity_efflogic.p_idu_supcapcappdata_by_diffparam(v_supcode  => v_supcode,
                                                                        v_faccode  => v_faccode,
                                                                        v_cate     => v_curcate,
                                                                        v_operid   => v_updid,
                                                                        v_opertime => v_updtime,
        ...
        --下单规划
        scmdata.pkg_capacity_efflogic.p_idu_alorder_data_by_diffparam(v_supcode => v_supcode,
                                                                      v_faccode => v_faccode,
                                                                      v_cate    => v_curcate,
                                                                      v_procate => v_curprocate,
        ...
        --周产能规划数据重算
        scmdata.pkg_capacity_efflogic.p_delete_wkplandata(v_supcode => v_supcode,
                                                          v_faccode => v_faccode,
                                                          v_cate    => v_curcate,
                                                          v_procate => v_curprocate,
                                                          v_subcate => v_sgsubcate,
        ...
        p_gen_wkplanrecalcrela(v_inp_supcode      => v_supcode,
                               v_inp_cate         => v_curcate,
                               v_inp_operate_id   => v_updid,
                               v_inp_operate_time => v_updtime,
                               v_inp_company_id   => v_compid);
  ...
  BEGIN
    ...
    FOR m IN (SELECT DISTINCT a.supplier_code,
               ...
               WHERE a.supplier_code = v_supcode
                 ...
                 AND instr(v_subcate, a.coop_subcategory) > 0
                 AND a.company_id = v_compid) LOOP
      p_gen_wkplanrecalcrela(v_inp_supcode      => m.supplier_code,
                             v_inp_cate         => m.coop_category,
                             v_inp_operate_id   => v_updid,
                             v_inp_operate_time => v_updtime,
                             v_inp_company_id   => m.company_id);
  ...
  BEGIN
    ...
    p_gen_wkplanrecalcrela(v_inp_supcode      => v_supcode,
                           v_inp_cate         => nvl(v_curcate, v_cate),
                           v_inp_operate_id   => v_updid,
                           v_inp_operate_time => v_updtime,
                           v_inp_company_id   => v_compid);
  ...
  BEGIN
    --条件赋值
    v_conds := ' tab.supplier_code = ''' || v_inp_supplier_code ||
               ''' and tab.company_id = ''' || v_inp_company_id || ''' ';
  
    --生产工厂不为空
    IF v_inp_factory_code IS NOT NULL THEN
      v_conds := scmdata.f_sentence_append_rc(v_sentence   => v_conds,
                                              v_appendstr  => ' and tab.factory_code = ''' ||
                                                              v_inp_factory_code ||
                                                              ''' ',
                                              v_middliestr => ' ');
    END IF;
    ...
    IF v_inp_product_cate IS NOT NULL THEN
      v_conds := scmdata.f_sentence_append_rc(v_sentence   => v_conds,
                                              v_appendstr  => ' and tab.product_cate = ''' ||
                                                              v_inp_product_cate ||
                                                              ''' ',
                                              v_middliestr => ' ');
    END IF;
    ...
    IF v_inp_subcategory IS NOT NULL THEN
      v_conds := scmdata.f_sentence_append_rc(v_sentence   => v_conds,
                                              v_appendstr  => ' and tab.subcategory = ''' ||
                                                              v_inp_subcategory ||
                                                              ''' ',
                                              v_middliestr => ' ');
    END IF;
    ...
    --DataSql 构建
    v_sql := 'SELECT tab.supplier_code,
       tab.factory_code,
       tab.category,
       tab.product_cate,
       tab.subcategory,
       tab.day,
       tab.company_id,
       tab.sfcpallocate_percentage,
       tab.scpsallocate_percentage,
       tab.capacity_ceiling_t * (tab.sfcpallocate_percentage / 100) *
             (tab.scpsallocate_percentage / 100) sfcps_allocapc_ceiling_t,
       tab.capacity_appointment_t *
             (tab.sfcpallocate_percentage / 100) *
             (tab.scpsallocate_percentage / 100) sfcps_allocapcp_appointment_t,
       tab.capacity_ceiling_t * (tab.sfcpallocate_percentage / 100) *
             (tab.scpsallocate_percentage / 100) / swmc.standard_worktime sfcps_allocapcp_ceiling_p,
       tab.capacity_appointment_t *
             (tab.sfcpallocate_percentage / 100) *
             (tab.scpsallocate_percentage / 100) / swmc.standard_worktime sfcps_allocapcp_appointment_p
  FROM (SELECT ssd.supplier_code,
               ssd.factory_code,
               ssd.category,
               cfprc.product_cate,
               cssrc.subcategory,
               ssd.day,
               ssd.company_id,
               trunc(ssd.capacity_ceiling) capacity_ceiling_t,
               trunc(ssd.app_capacity) capacity_appointment_t,
               cfprc.allocate_percentage sfcpallocate_percentage,
               cssrc.allocate_percentage scpsallocate_percentage
          FROM scmdata.t_supcapcweekplan_sup_detail ssd
          LEFT JOIN scmdata.t_capc_fac_procate_rate_cfg cfprc
            ON ssd.supplier_code = cfprc.supplier_code
           AND ssd.factory_code = cfprc.factory_code
           AND ssd.category = cfprc.category
           AND ssd.company_id = cfprc.company_id
          LEFT JOIN scmdata.t_capc_sup_subcate_rate_cfg cssrc
            ON cfprc.supplier_code = cssrc.supplier_code
           AND cfprc.category = cssrc.category
           AND cfprc.product_cate = cssrc.product_cate
           AND cfprc.company_id = cssrc.company_id) tab
  LEFT JOIN scmdata.t_standard_work_minte_cfg swmc
    ON tab.category = swmc.category
   AND tab.product_cate = swmc.product_cate
   AND tab.subcategory = swmc.subcategory
   AND tab.company_id = swmc.company_id
 WHERE ' || v_conds;
  
    --返回DataSql 
    RETURN v_sql;
  END f_get_weekplan_pieces_recalc_datasql;
  ...
  (
    ...
    v_day                           DATE;
    v_company_id                    VARCHAR2(32);
    v_sfcpallocate_percentage       NUMBER(5, 2);
    v_scpsallocate_percentage       NUMBER(5, 2);
    v_sfcps_allocapcp_ceiling_t     NUMBER;
    v_sfcps_allocapcp_appointment_t NUMBER;
    v_sfcps_allocapcp_ceiling_p     NUMBER;
    v_sfcps_allocapcp_appointment_p NUMBER;
    v_sql                           CLOB;
  BEGIN
    --数据获取Sql
    v_sql := f_get_weekplan_pieces_recalc_datasql(v_inp_supplier_code => v_inp_supplier_code,
    ...
    LOOP
      ...
      FETCH cur
        INTO v_supplier_code,
             ...
             v_day,
             v_company_id,
             v_sfcpallocate_percentage,
             v_scpsallocate_percentage,
             v_sfcps_allocapcp_ceiling_t,
             v_sfcps_allocapcp_appointment_t,
             v_sfcps_allocapcp_ceiling_p,
             v_sfcps_allocapcp_appointment_p;
      ...
      BEGIN
        ...
        IF v_supplier_code IS NOT NULL
           ...
           AND v_category IS NOT NULL
           AND v_product_cate IS NOT NULL
           AND v_subcategory IS NOT NULL THEN
          --供应商周产能规划(件数)增改逻辑
          scmdata.pkg_capacity_management.p_tabapi_iou_capacity_weekplan_numpieces_base(v_inp_cwpnb_id                      => scmdata.f_get_uuid(),
                                                                                        v_inp_company_id                    => v_company_id,
                                                                                        ...
                                                                                        v_inp_product_cate                  => v_product_cate,
                                                                                        v_inp_subcategory                   => v_subcategory,
                                                                                        v_inp_calculate_day                 => v_day,
                                                                                        v_inp_sfcpallocate_percentage       => v_sfcpallocate_percentage,
                                                                                        v_inp_scpsallocate_percentage       => v_scpsallocate_percentage,
                                                                                        v_inp_sfcps_allocapcp_ceiling_t     => v_sfcps_allocapcp_ceiling_t,
                                                                                        v_inp_sfcps_allocapcp_appointment_t => v_sfcps_allocapcp_appointment_t,
                                                                                        v_inp_sfcps_allocapcp_ceiling_p     => v_sfcps_allocapcp_ceiling_p,
                                                                                        v_inp_sfcps_allocapcp_appointment_p => v_sfcps_allocapcp_appointment_p);
        END IF;
      EXCEPTION
        WHEN OTHERS THEN
          --继续
  ...
  BEGIN
    ...
    ELSIF v_vctab = 'SCMDATA.T_CAPC_FAC_PROCATE_RATE_CFG' THEN
      --工厂生产分类占比配置
      p_capc_fac_procate_rate_cfg_efflogic(v_inp_queue_id   => v_queueid,
                                           v_inp_cond       => v_vccond,
                                           v_inp_company_id => v_compid);
    
    ELSIF v_vctab = 'SCMDATA.T_CAPC_SUP_SUBCATE_RATE_CFG' THEN
      --供应商产品子类占比配置
      p_capc_sup_subcate_rate_cfg_efflogic(v_inp_queue_id   => v_queueid,
                                           v_inp_cond       => v_vccond,
                                           v_inp_company_id => v_compid);
