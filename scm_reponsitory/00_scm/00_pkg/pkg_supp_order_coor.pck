CREATE OR REPLACE PACKAGE SCMDATA.pkg_supp_order_coor IS
  ...
  --订单商品明细
  FUNCTION f_query_ordergoods_list RETURN CLOB;
  --查询操作记录
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_supp_order_coor IS
  ...
  BEGIN
    ...
    SELECT z.order_code,
           ...
           cd_a.company_dict_name samll_category,
           CASE
             ...
             WHEN z.send_by_sup = 1 THEN
              '是'
           END send_by_sup,
           z.rationame rationame_00000, --zc314 add
 ...
 ORDER BY a.barcode  --zc314 add
]';
    RETURN v_sql;
  END f_query_ordernums_list;
  --订单商品明细
  FUNCTION f_query_ordergoods_list RETURN CLOB IS
    v_sql CLOB;
  BEGIN
    v_sql := q'[SELECT od.ordersitem_id,
       od.barcode,
       tcs.colorname,
       tcs.sizename,
       od.order_amount,
 ...
 WHERE od.company_id = :company_id
   AND od.order_id = :order_code
   AND od.goo_id = :goo_id
 ORDER BY od.barcode --zc314 add]';
    RETURN v_sql;
  END f_query_ordergoods_list;
  --查询操作记录
