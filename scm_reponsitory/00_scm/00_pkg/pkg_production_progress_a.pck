CREATE OR REPLACE PACKAGE SCMDATA.pkg_production_progress_a IS
  ...
  --获取生产节点模板
  FUNCTION f_get_progress_config_module(p_company_id VARCHAR2,
                                        p_cate       VARCHAR2,
                                        p_pcate      VARCHAR2,
                                        p_scate      VARCHAR2)
    RETURN VARCHAR2;
  ...
  --lookup  
  --数据来源
  FUNCTION f_get_data_source_look_sql RETURN VARCHAR2;

  --异常来源
  FUNCTION f_get_abn_source_look_sql(p_company_id         VARCHAR2,
                                     p_check_link         VARCHAR2 DEFAULT NULL,
                                     p_check_gd_link_desc VARCHAR2 DEFAULT NULL,
                                     p_check_num          VARCHAR2 DEFAULT NULL)
    RETURN CLOB;

  --查货人
  FUNCTION f_get_checker_look_sql(p_data_source VARCHAR2,
                                  p_company_id  VARCHAR2) RETURN VARCHAR2;

  --获取查货人
  /*  FUNCTION f_get_checker_by_abn_orgin_ma(p_abn_source VARCHAR2,
                                       p_company_id VARCHAR2,
  ...
  --异常处理列表
  --保存校验
  PROCEDURE p_check_abnormal_by_update(p_abn_rec scmdata.t_abnormal%ROWTYPE);
  --删除校验
  PROCEDURE p_check_abnormal_by_delete(p_company_id  VARCHAR2,
                                       p_user_id     VARCHAR2,
                                       p_abnormal_id VARCHAR2);

  --删除异常单
  PROCEDURE p_delete_abormal(p_company_id VARCHAR2, p_abnormal_id VARCHAR2);

END pkg_production_progress_a;
/

CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_production_progress_a IS
  ...
  BEGIN
    ...
    FOR order_rec IN (SELECT po.company_id,
      ...
      IF v_count = 0 THEN
        ...
        IF scmdata.pkg_plat_comm.f_is_check_fields_eq(p_old_field => v_abn_status,
                                                      p_new_field => v_abn_status_tol) = 1 THEN
          ...
          --如果交货记录 存在到仓日期为空则不处理 
          IF scmdata.pkg_plat_comm.f_is_check_fields_eq(p_old_field => v_end_time_c,
                                                        p_new_field => v_total_c) = 1 THEN
      ...
      ELSE
        IF p_item_id IN ('a_product_110', 'a_product_150', 'a_product_210') THEN
          ...
          ELSE
            ...
            IF p_item_id NOT IN ('a_product_210') THEN
              SELECT t.dept_name
                INTO v_dept_name
  ...
  *============================================*/
  ...
  BEGIN
    INSERT INTO t_progress_log
      (log_id, product_gress_id, company_id, log_type, log_msg, operater,
       operate_company_id, create_id, create_time, update_id, update_time,
       memo)
    VALUES
      (scmdata.f_get_uuid(), p_product_gress_id, p_company_id, p_log_type,
       p_log_msg, p_operater, p_operate_company_id, p_create_id, SYSDATE,
       p_update_id, SYSDATE, p_memo);
  ...
  *============================================*/
  ...
  FUNCTION f_get_progress_config_module(p_company_id VARCHAR2,
                                        ...
                                        p_scate      VARCHAR2)
    RETURN VARCHAR2 IS
    v_progress_config_id VARCHAR2(32);
  BEGIN
    SELECT MAX(pc.progress_config_id)
      INTO v_progress_config_id
     ...
     INNER JOIN scmdata.t_progress_node_config a
       ...
       AND a.pause = 0
       AND t.pause = 0
     INNER JOIN scmdata.t_progress_config pc
        ON pc.progress_config_id = a.progress_config_id
       AND pc.company_id = a.company_id
     WHERE t.industry_classification = p_cate
       AND t.production_category = p_pcate
       AND instr(';' || t.product_subclass || ';', ';' || p_scate || ';') > 0
       AND t.company_id = p_company_id;
    RETURN v_progress_config_id;
  END f_get_progress_config_module;
  ...
  *============================================*/
    ...
    CURSOR pro_cur IS
      SELECT p.goo_id,
       ...
       INNER JOIN scmdata.t_orders od
          ON od.goo_id = p.goo_id
         AND od.order_id = p.order_id
         AND od.company_id = p.company_id
       INNER JOIN scmdata.t_ordered oh
          ON oh.order_code = od.order_id
  ...
  BEGIN
    ...
    ELSE
      --2022-09-05  zxp clj 需求变更  801 【生产进度】：[批量复制进度]功能修改
      --该逻辑舍弃
      /*    --判断需复制进度的生产订单 货号是否一致
      SELECT COUNT(DISTINCT p.goo_id)
        INTO v_goo_cnt
      ...
      FOR pro_rec IN pro_cur LOOP
        --获取预测交期
        scmdata.pkg_production_progress_a.p_forecast_delivery_date(p_progress_id          => pro_rec.product_gress_id,
                                                                   p_company_id           => v_company_id,
                                                                   p_progress_status      => be_pro_rec.progress_status,
                                                                   p_progress_update_date => be_pro_rec.progress_update_date,
                                                                   p_plan_date            => be_pro_rec.plan_delivery_date,
                                                                   p_delivery_date        => pro_rec.delivery_date,
                                                                   p_curlink_complet_prom => round(be_pro_rec.curlink_complet_ratio,
                                                                                                   2),
                                                                   po_forecast_date       => vo_forecast_date,
                                                                   po_forecast_days       => vo_forecast_days);
  ...
  --lookup  
  --数据来源
  FUNCTION f_get_data_source_look_sql RETURN VARCHAR2 IS
    v_sql VARCHAR2(512);
  BEGIN
    v_sql := q'[
    SELECT 'MA' ORIGIN,'手动新增' ORIGIN_DESC FROM dual 
    UNION ALL
    SELECT 'SC' ORIGIN,'系统创建' ORIGIN_DESC FROM dual
    ]';
    RETURN v_sql;
  END f_get_data_source_look_sql;
  ...
  BEGIN
    v_sql := q'[
 SELECT abnormal_orgin,abn_orgin_desc,']' || p_check_link ||
             q'[' CHECK_LINK,']' || p_check_gd_link_desc ||
             q'[' CHECK_GD_LINK_DESC,']' || p_check_num ||
             q'[' CHECK_NUM FROM (SELECT c.company_dept_id abnormal_orgin, c.dept_name abn_orgin_desc
  FROM scmdata.sys_company_dept c
 WHERE c.company_id = ']' || p_company_id || q'['
   AND LEVEL = 3
 START WITH c.company_dept_id = 'b550778b4ef236b4e0533c281caca074' --'b550778b4ef236b4e0533c281caca074'
CONNECT BY PRIOR c.company_dept_id = c.parent_id
UNION ALL
SELECT 'ed7ff3c7135a236ae0533c281caccd8d' abnormal_orgin,
       '供应商' abn_orgin_desc
  FROM dual) --供应商        
     ]';
    RETURN v_sql;
  END f_get_abn_source_look_sql;
  ...
  FUNCTION f_get_checker_look_sql(p_data_source VARCHAR2,
    ...
    v_sql VARCHAR2(512);
  BEGIN
    IF p_data_source = 'SC' THEN
      v_sql := q'[
      SELECT a.user_id checker, a.company_user_name checker_desc
 ...
 INNER JOIN scmdata.sys_company_job b
   ...
   AND b.company_id = a.company_id
 WHERE b.company_job_id IN ('1001005003005001001', '1001005003005001')
   AND b.company_id = ']' || p_company_id || q'['
     ]';
    END IF;
    RETURN v_sql;
  END f_get_checker_look_sql;
  ...
  /*  FUNCTION f_get_checker_by_abn_orgin_ma(p_abn_source VARCHAR2,
    ...
    RETURN VARCHAR2 IS
  BEGIN
    \*   IF p_abn_source IN ('ed7ff3c7135a236ae0533c281caccd8d','14') THEN
     NULL;
    END IF;*\
    RETURN NULL;
  END f_get_checker_by_abn_orgin_ma;*/

  --异常处理列表
  --保存校验
  PROCEDURE p_check_abnormal_by_update(p_abn_rec scmdata.t_abnormal%ROWTYPE) IS
  BEGIN
    --异常类型=质量异常
    IF p_abn_rec.anomaly_class = 'AC_QUALITY' THEN
      --异常来源=生产管理部
      IF p_abn_rec.abnormal_orgin = 'b550778b4f2d36b4e0533c281caca074' THEN
        IF p_abn_rec.checker IS NULL OR p_abn_rec.check_link IS NULL OR
           p_abn_rec.check_num IS NULL THEN
          raise_application_error('-20002',
                                  '当异常类型为“质量异常”且异常来源为“生产管理部”时，【查货人】、【查货环节】、【查货次数】必填！');
        END IF;
        --异常来源=生产技术部
      ELSIF p_abn_rec.abnormal_orgin = 'b550778b4f3f36b4e0533c281caca074' THEN
        IF p_abn_rec.check_link IS NULL OR p_abn_rec.check_num IS NULL THEN
          raise_application_error('-20002',
                                  '当异常类型为“质量异常”且异常来源为“生产技术部”时，【查货环节】、【查货次数】必填！');
        END IF;
      ELSE
        NULL;
      END IF;
    ELSE
      NULL;
    END IF;
  END p_check_abnormal_by_update;
  ...
  BEGIN
    SELECT MAX(t.create_id), MAX(t.origin)
      INTO v_user_id, v_origin
      FROM scmdata.t_abnormal t
     WHERE t.abnormal_id = p_abnormal_id
       AND t.company_id = p_company_id;
  
    IF v_origin = 'SC' THEN
      raise_application_error(-20002, '系统创建的异常单，不可删除！');
    ELSIF v_origin = 'MA' THEN
      IF scmdata.pkg_plat_log.f_is_check_fields_eq(p_old_field => v_user_id,
                                                   p_new_field => p_user_id) = 0 THEN
        raise_application_error(-20002,
                                '手动新增的异常单，只允许创建人删除！');
      END IF;
    ELSE
      NULL;
    END IF;
  END p_check_abnormal_by_delete;

  --删除异常单
  PROCEDURE p_delete_abormal(p_company_id VARCHAR2, p_abnormal_id VARCHAR2) IS
  BEGIN
    DELETE FROM scmdata.t_abnormal abn
     WHERE abn.company_id = p_company_id
       AND abn.abnormal_id = p_abnormal_id;
  END p_delete_abormal;
END pkg_production_progress_a;
/
