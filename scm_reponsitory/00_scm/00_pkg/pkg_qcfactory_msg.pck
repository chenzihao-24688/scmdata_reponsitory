...
create or replace package body scmdata.pkg_qcfactory_msg is
  ...
  begin
    ...
    for x in (select count(*) p_count,
                     qc_group_id,
                     max(cu.inner_user_id) p_manage_inner_id,
                     listagg(si.supplier_company_name, ';') p_factory
      ...
      if x.p_count > 3 then
        p_factory := regexp_substr(x.p_factory, '[^;]+;[^;]+;[^;]+');
      else
        p_factory := x.p_factory;
      end if;
    
      scmdata.pkg_send_wx_msg.p_send_com_person_wx_msg(p_company_id => p_company_id,
                                                       p_user_id    => x.p_manage_inner_id,
                                                       p_content    => p_factory || '等' ||
                                                                       x.p_count ||
                                                                       '家工厂没有分配到查货QC，请前往SCM系统【QC品控-QC查货工厂分配】中进行维护。',
                                                       p_sender     => 'scm');
  ...
  begin
    ...
    for x in (select a.qc_group_id,
      ...
      select count(*), listagg(cu.company_user_name, ';')
      ...
      if p_count >= 1 then
        if p_count > 3 then
          p_user_name := regexp_substr(p_user_name, '[^;]+;[^;]+;[^;]+');
        end if;
        scmdata.pkg_send_wx_msg.p_send_com_person_wx_msg(p_company_id => p_company_id,
                                                         p_user_id    => x.inner_user_id,
                                                         p_content    => p_user_name || '等' ||
                                                                         p_count ||
                                                                         '个组员没有分配到查货工厂，请前往SCM系统【QC品控-QC查货工厂分配】中进行维护。',
                                                         p_sender     => 'scm');
      end if;
    end loop;
