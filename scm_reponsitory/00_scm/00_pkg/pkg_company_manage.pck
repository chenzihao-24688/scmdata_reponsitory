create or replace package scmdata.PKG_COMPANY_MANAGE is
  ...
  */                                               
  ...
  /* PI_USER_ID   ：购买人员工号
     ...
     PO_RESULT    ：输出标识(数字)
     PO_MESSAGE   : 输出错误描述
     PI_COMPANY_ROLE: 企业在平台的类别，need:采购方    supp:成品供应商   mrp:面料供应商
   purpose：供应商批量自动购买应用
   by: hx 2021-11-20
  */  
...
BEGIN
  FOR R_PRIV IN C_PRIV LOOP
    ...
    SELECT COUNT(*) INTO V_COUNT  FROM bw3.SYS_COND_LIST    WHERE COND_ID like V_COND_ID||'%';
    ...
    IF V_COUNT = 0 THEN
     ...
     update sys_app_privilege a set a.cond_id=v_cond_id where a.priv_id=R_PRIV.Priv_Id;
      ...
      select count(*) into v_i from bw3.sys_cond_rela a
      ...
      if   R_PRIV.Item_Id is not null or v_i =0 THEN
        ...
        */
        ...
        SELECT nvl(MAX(tmpnum) + 1, 0)
          INTO v_appendnum
          FROM (SELECT to_number(tmpnum) tmpnum
                  FROM (SELECT substr(a.cond_id, instr(a.cond_id, '_auto_') + 6, 99) tmpnum
                          FROM bw3.sys_cond_rela a
                         WHERE a.obj_type = r_priv.obj_type
                           AND a.ctl_id = r_priv.ctl_id
                           AND instr(a.cond_id, 'cond_' || r_priv.ctl_id || '_' || r_priv.obj_type || '_auto_') > 0)
                 WHERE regexp_instr(lower(tmpnum), '[a-z \s \S]') = 0
                   AND length(tmpnum) <= 8);
        
        V_COND_ID := 'cond_' || r_priv.ctl_id || '_' || r_priv.obj_type || '_auto_' || v_appendnum;
        --zc314_add_end
