...
CREATE OR REPLACE PACKAGE BODY SCMDATA.PKG_ARCHIVE_COMMON IS
    ...
    IF ITEMS <> ' ' THEN
      WHILE NEW_ITEMS <> OLD_ITEMS LOOP
        ...
        SELECT LISTAGG(DISTINCT COL, ',')
          INTO NEW_ITEMS
          FROM (SELECT REGEXP_SUBSTR(ITEMS, '[^,]+', 1, LEVEL) COL
                  FROM DUAL
  ...
  BEGIN
    EXE_SQL := 'SELECT NVL(LISTAGG(ITEM_ID,'',''),'' '')
                 ...
                 WHERE PAUSE = 0
                   AND NODE_ID IN
                         (SELECT SHORT_PATH
                            FROM BW3.SYS_SHORTCUT
                           WHERE SHORT_ID IN
                                   (SELECT SHORT_ID
                                      FROM BW3.SYS_SHORTCUT_NODE_RELA
                                     WHERE PAUSE = 0
                                       AND NODE_ID IN
                                            (SELECT NODE_ID
                                               FROM BW3.SYS_TREE_LIST
    ...
    EXE_SQL := 'SELECT NVL(LISTAGG(CITEM,'',''),'' '')
                  FROM (SELECT CONNECT_BY_ISCYCLE ISCYCLED, CITEM
                          FROM (SELECT A.ITEM_ID AITEM, C.ITEM_ID CITEM
                                  ...
                                  INNER JOIN BW3.SYS_TREE_LIST C
  ...
  FUNCTION F_GET_COND_OPERATE_RELA(V_ITEMS   IN CLOB) RETURN CLOB IS
    ...
    V_RETCLOB    CLOB;
  BEGIN
    FOR I IN (SELECT ELEMENT_ID FROM BW3.SYS_ITEM_ELEMENT_RELA
    ...
    SELECT NVL(LISTAGG(DISTINCT B.TO_CONFIRM_ITEM_ID||','||B.TO_CANCEL_ITEM_ID, ','), ' ')
      ...
      FROM (SELECT COND_ID
             ...
             WHERE INSTR(V_IES,','||CTL_ID||',')>0
               AND PAUSE = 0) A
     INNER JOIN BW3.SYS_COND_OPERATE B
      ...
      WHILE REGEXP_COUNT(TMP_SQL, TMP_PAT) > 0 LOOP
        ...
        IF REGEXP_COUNT(TMP_TAB,'T_|SYS_') > 0 THEN
          IF REGEXP_COUNT(TABS||',',TMP_TAB||',')=0 THEN
            IF INSTR(TMP_TAB,'SCMDATA.') = 0 AND INSTR(TMP_TAB,'BW3.') = 0 THEN
  ...
  FUNCTION F_GET_ACTIONS_TABS(INS_ITEMS  IN VARCHAR2,
    ...
    ACTCUR    ACTYPE;
    ACTROW    BW3.SYS_ACTION%ROWTYPE;
    TABS      CLOB;
  BEGIN
    TABS := INS_TABS;
    EXE_SQL := 'SELECT A.*
                  FROM BW3.SYS_ACTION A
  ...
  BEGIN
    ...
    IF PK_VALS IS NOT NULL THEN
      BEGIN
        ...
        IF SCMDATA.PKG_VARIABLE.F_GET_VARCHAR_WITH_CHAID (V_OBJID   => 'SYS',
                                                          V_COMPID  => 'b6cc680ad0f599cde0531164a8c0337f',
                                                          V_VARNAME => 'DELETE_STAGE_JUDGE',
                                                          V_CHAID   => CF_ID) <> 'PS' THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          SCMDATA.PKG_VARIABLE.P_INS_OR_UPD_VARIABLE_WITH_CHAID_AT(V_OBJID   => 'SYS',
                                                                   V_COMPID  => 'b6cc680ad0f599cde0531164a8c0337f',
                                                                   V_VARNAME => 'DELETE_STAGE_JUDGE',
                                                                   V_VARTYPE => 'VARCHAR',
                                                                   V_VARCHAR => 'ER',
        ...
        FOR Y IN (SELECT DISTINCT MASTER_TAB,SLAVE_TAB,COND_STR
                    FROM (SELECT MASTER_ITEM,MASTER_TAB,SLAVE_ITEM,
                            ...
                            FROM (SELECT A.ITEM_ID MASTER_ITEM,SCMDATA.PKG_ARCHIVE_COMMON.F_GET_MAJORTAB(B.SELECT_SQL) MASTER_TAB,
                                    ...
                                    FROM BW3.SYS_ITEM_RELA A
                                    LEFT JOIN BW3.SYS_ITEM_LIST B ON A.ITEM_ID = B.ITEM_ID
                                    LEFT JOIN BW3.SYS_ITEM_LIST C ON A.RELATE_ID = C.ITEM_ID
