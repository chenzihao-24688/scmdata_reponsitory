 ...
 INNER JOIN SCMDATA.SYS_COMPANY_DICT D2 ON B.SAMLL_CATEGORY=D2.COMPANY_DICT_VALUE AND D2.COMPANY_ID=A.COMPANY_ID]';
      ...
      ELSIF V_SELECT1 = '05' AND V_SELECT2 IS NOT NULL THEN
        ...
        ELSE 
          ...
          V_VALUE := '全部仓库';
       END IF;
       ELSIF v_select1 IS NULL OR (V_SELECT1 IS NOT NULL AND V_SELECT2 IS NULL) THEN 
         V_S_SQL:=V_S_SQL||', SUM(A.EXAMOUNT*B.PRICE) RMMONEY ';   
      END IF;
...
/*INNER JOIN SCMDATA.SYS_COMPANY_DICT J ON J.COMPANY_DICT_VALUE = B.SAMLL_CATEGORY AND J.COMPANY_ID = B.COMPANY_ID */]';
  ...
  BEGIN
    ...
    IF V_ORIGIN = 'M' THEN
      ...
      V_S_SQL := q'[SELECT E.GROUP_NAME SUP_GROUP_NAME2, C.PROBLEM_CLASSIFICATION, C.CAUSE_CLASSIFICATION,
                         D.DEPT_NAME, (CASE WHEN C.IS_SUP_EXEMPTION = 0 THEN  '否' ELSE '是' END) IS_SUP_EXEMPTION,
                         SUM(A.EXAMOUNT2 * B.PRICE) RMMONEY
                    FROM (SELECT Z.*,(CASE WHEN INSTR(z.sup_group_name,',') >0 THEN  z.examount/(REGEXP_COUNT(z.SUP_GROUP_NAME, '','') + 1)
             ELSE z.examount END ) EXAMOUNT2,
      ...
      ON A.SUP_GROUP_NAME2 = E.GROUP_CONFIG_ID
     AND A.COMPANY_ID = E.COMPANY_ID   ]';
      ...
      V_GR_SQL := q'[   GROUP BY E.GROUP_NAME,
          C.PROBLEM_CLASSIFICATION,A.FIRST_DEPT_ID,
      ...
      --区域的条件
      IF V_GROUP <> '全部' THEN
        V_S1_SQL:=q'[ SELECT E.GROUP_NAME SUP_GROUP_NAME2,
         C.PROBLEM_CLASSIFICATION,
         C.CAUSE_CLASSIFICATION,
         D.DEPT_NAME,
         (CASE   WHEN C.IS_SUP_EXEMPTION = 0 THEN '否'  ELSE  '是'  END) IS_SUP_EXEMPTION,
         SUM((A.EXAMOUNT123 * B.PRICE) / (REGEXP_COUNT(A.SUP_GROUP_NAME, ',') + 1)) RMMONEY  ]';
         
        /*V_WH_SQL := V_WH_SQL || '  AND instr ('',''||E.GROUP_NAME||'','','',''|| '''|| V_GROUP ||
                    '''||'','') >0 AND A.COMPANY_ID = ''' || V_COMPID || '''';*/
         V_WH_SQL := V_WH_SQL || '  AND  A.SUP_GROUP_NAME2 = '''||V_GROUP|| ''' AND A.COMPANY_ID = ''' || V_COMPID || '''';           
      
       ELSIF V_GROUP IS NULL THEN V_WH_SQL:=V_WH_SQL||' AND A.COMPANY_ID = '''||V_COMPID||'''';
         V_S1_SQL:=q'[ SELECT A.SUP_GROUP_NAME2,
         C.PROBLEM_CLASSIFICATION,
         C.CAUSE_CLASSIFICATION,
         D.DEPT_NAME,
         (CASE   WHEN C.IS_SUP_EXEMPTION = 0 THEN '否'  ELSE  '是'  END) IS_SUP_EXEMPTION,
        ...
        V_WH2_SQL := q'[  SELECT F.*,  F.EXAMOUNT / (REGEXP_COUNT(F.SHO_ID, ',') + 1) EXAMOUNT123
          ...
          CONNECT BY PRIOR RM_ID = RM_ID
                 AND LEVEL <=
                     LENGTH(Z.SUP_GROUP_NAME) -
                     LENGTH(REGEXP_REPLACE(Z.SUP_GROUP_NAME, ',', '')) + 1
                 AND PRIOR DBMS_RANDOM.VALUE IS NOT NULL) F
           WHERE ]' ||
                    '  (instr('',''||F.sho_id ||'','','',''||''' ||
...
LEFT JOIN SCMDATA.T_SUPPLIER_GROUP_CONFIG G ON E.GROUP_NAME=G.GROUP_CONFIG_ID AND G.COMPANY_ID = E.COMPANY_ID
