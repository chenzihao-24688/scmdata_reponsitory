CREATE OR REPLACE PACKAGE SCMDATA.pkg_production_progress IS
  --全局变量
  --预测交期二维数组
  --（列）
  ...
  PROCEDURE check_deduction_config(p_company_id        VARCHAR2,
                                   p_goo_id            VARCHAR2,
                                   p_status            NUMBER,
                                   p_order_create_time DATE DEFAULT NULL,
                                   po_count            OUT NUMBER,
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_production_progress IS
  --校验是否已经同步生产进度
  FUNCTION check_is_sync_production_progress(po_header_rec scmdata.t_ordered%ROWTYPE)
    RETURN NUMBER IS
    v_flag NUMBER;
  ...
  BEGIN
    ...
    ELSE
      ...
      FOR po_line_rec IN (SELECT *
        ...
        ---20230612 lsl add 领料单生成
        mrp.pkg_bulk_cargo_bom.p_pick_list(v_product_gress_code => v_product_gress_code,
                                           v_company_id         => 'b6cc680ad0f599cde0531164a8c0337f');
        --end 

      --5.判断生产进度配置表是否有适用范围(同时校验是否停用)，有则生成节点进度，无则不用生成。
      --20220617 订单优化1.3版舍弃该逻辑
  ...
  *============================================*/
  ...
  BEGIN
    INSERT INTO t_production_progress
      (product_gress_id, company_id, product_gress_code, order_id,
       progress_status, goo_id, supplier_code, factory_code,
       forecast_delivery_date, forecast_delay_day, actual_delivery_date,
       actual_delay_day, latest_planned_delivery_date, order_amount,
       delivery_amount, approve_edition, fabric_check, qc_quality_check,
       exception_handle_status, handle_opinions, create_id, create_time,
       origin, memo, qc_check, qa_check, order_rise_status)
    VALUES
      (p_produ_rec.product_gress_id, p_produ_rec.company_id,
       p_produ_rec.product_gress_code, p_produ_rec.order_id,
       p_produ_rec.progress_status, p_produ_rec.goo_id,
       p_produ_rec.supplier_code, p_produ_rec.factory_code,
       ...
       p_produ_rec.order_amount, p_produ_rec.delivery_amount,
       p_produ_rec.approve_edition, p_produ_rec.fabric_check,
       p_produ_rec.qc_quality_check, p_produ_rec.exception_handle_status,
       p_produ_rec.handle_opinions, p_produ_rec.create_id,
       p_produ_rec.create_time, p_produ_rec.origin, p_produ_rec.memo,
       p_produ_rec.qc_check, p_produ_rec.qa_check,
  ...
  *============================================*/
  ...
  PROCEDURE check_production_progress(p_product_gress_id VARCHAR2) IS
    ...
    v_delivery_amount_all NUMBER;
    v_order_amount_sku    NUMBER;
    v_cnt                 INT;
  BEGIN
    SELECT MAX(pr.company_id),
           MAX(pr.order_id),
           MAX(pr.goo_id),
           MAX(pr.delivery_amount)
      INTO v_company_id, v_order_id, v_goo_id, v_delivery_amount_all
      FROM scmdata.t_production_progress pr
    ...
    --20221019 clj 提出需求：生产订单进度表/非生产订单跟进表】主表的”交货数量“与从表【订单明细】的sku交货量之和是否相等
    --如不等则提示”交货记录未更新完，不可结束订单！”
    SELECT SUM(od.got_amount), COUNT(od.ordersitem_id)
      INTO v_order_amount_sku, v_cnt
      FROM scmdata.t_ordersitem od
     WHERE od.company_id = v_company_id
       AND od.order_id = v_order_id
       AND od.goo_id = v_goo_id;
    IF v_cnt > 0 THEN
      IF nvl(v_delivery_amount_all, 0) <> nvl(v_order_amount_sku, 0) THEN
        raise_application_error(-20002,
                                '订单结束失败！交货记录未更新完，不可结束');
      END IF;
    END IF;
  ...
  *============================================*/
  PROCEDURE finish_production_progress(p_product_gress_id VARCHAR2,
    ...
    v_deduction_proportion NUMBER; --扣款比例
    ...
    IF p_pro_rec.progress_status <> p_status THEN
      ...
      IF v_suc_count = v_pr_tol THEN
        ...
        SELECT MAX(province || city || county) location_area,
               MAX(sg.group_config_id) group_name,
               MAX(sg.area_group_leader) area_group_leader,
               MAX(company_vill) company_vill
        ...
        --新增季节字段，获取商品档案季节字段存于订单表
        SELECT MAX(tc.season)
          INTO v_season
          FROM scmdata.t_commodity_info tc
         WHERE tc.goo_id = p_pro_rec.goo_id
           AND tc.company_id = p_pro_rec.company_id;
      
        UPDATE scmdata.t_ordered po
           SET po.order_status        = 'OS02',
               po.area_group_id       = v_group_name,
               po.area_group_leaderid = v_area_group_leader,
               po.area_locatioin      = v_location_area,
               po.season              = v_season,
               po.company_vill        = v_company_vill,
        ...
        ----20230112 zc314 begin 
        ----需求说明: 订单新增 Qc/Qc主管字段，结束订单时赋值到 scmdata.t_ordered 
        scmdata.pkg_order_management.p_ordqcrelainfo_record(v_inp_ordid  => p_pro_rec.order_id, 
                                                            v_inp_compid => p_pro_rec.company_id);
        ----20230112 zc314 end
      
        ----20220722 lsl167 需求优化 禅道815  生产/非生产订单“单价”=0的订单：不再生成扣款单；
        SELECT MAX(t.order_price)
          INTO v_price
          FROM scmdata.t_orders t
         WHERE t.company_id = p_pro_rec.company_id
           AND t.order_id = p_pro_rec.order_id;
        IF v_price <> 0 THEN
          --20220722 lsl167 新增判断v_price单价为0 不生成扣款单
          ---end
                ...
                IF p_abn_rec.is_deduction = 1 THEN
                  ...
                  IF p_abn_rec.anomaly_class = 'AC_QUALITY' THEN
                    ...
                    IF p_abn_rec.deduction_method = 'METHOD_00' THEN
                      ...
                      --扣款单价
                      v_order_price := p_abn_rec.deduction_unit_price;
                      --扣款金额
                      v_discount_price := v_order_price * v_abn_amount;
                    ...
                    ELSIF p_abn_rec.deduction_method = 'METHOD_02' THEN
                      ...
                      --扣款比例
                      v_deduction_proportion := p_abn_rec.deduction_unit_price;
                      --20220506 更改需求，数量取值需根据异常处理单的“异常范围”进行统计
                      --扣款金额
                      v_discount_price := (v_abn_amount * v_order_price *
                                          v_deduction_proportion) / 100;
                    ELSE
                      NULL;
                    END IF;
                    --生成扣款单
                    sync_abn_ded_bill(p_company_id           => p_pro_rec.company_id,
                                      ...
                                      p_deduction_type       => p_abn_rec.deduction_method,
                                      ...
                                      p_order_price          => v_order_price, --扣款单价
                                      p_deduction_proportion => v_deduction_proportion, --扣款比例
  ...
  PROCEDURE check_deductions(p_company_id VARCHAR2, p_order_id VARCHAR2) IS
    v_deds_flag     NUMBER;
    v_product_order NUMBER; ---20220725 lsl167 新增
  BEGIN
    SELECT COUNT(1)
      INTO v_deds_flag
    ...
    ----20220725 lsl167 需求优化 非生产订单生成扣款数据后，直接流转到到【扣款处理列表-已处理】列表，并且“是否免扣”标记为：是
    SELECT MAX(t.is_product_order)
      INTO v_product_order
      FROM scmdata.t_ordered t
     WHERE t.company_id = p_company_id
       AND t.order_code = p_order_id
       AND t.is_product_order = '0';
  
    IF v_deds_flag > 0 THEN
      IF v_product_order = 0 THEN
        UPDATE scmdata.t_ordered po
           SET po.approve_status    = '01',
               po.is_free_deduction = '1',
               po.approve_time      = SYSDATE,
               po.approve_id        = 'ADMIN'
         WHERE po.company_id = p_company_id
           AND po.order_code = p_order_id;
      ELSE
      ...
      IF p_abn_rec.anomaly_class IS NOT NULL AND
        ...
        IF p_abn_rec.anomaly_class = 'AC_QUALITY' THEN
          ...
          IF p_abn_rec.detailed_reasons = ' ' OR
             ...
             p_abn_rec.cause_detailed = ' ' OR
             p_abn_rec.problem_class = ' ' OR p_abn_rec.handle_opinions IS NULL THEN
  ...
  *============================================*/
  ...
  BEGIN
    ...
    INSERT INTO t_abnormal
      (abnormal_id, company_id, abnormal_code, order_id, progress_status,
       goo_id, anomaly_class, problem_class, cause_class, cause_detailed,
       detailed_reasons, is_sup_responsible, delay_date, delay_amount,
       responsible_party, responsible_dept, responsible_dept_sec,
       handle_opinions, quality_deduction, is_deduction, deduction_method,
       deduction_unit_price, file_id, applicant_id, applicant_date,
       confirm_id, confirm_company_id, confirm_date, create_id, create_time,
       update_id, update_time, origin, origin_id, memo, abnormal_range,
       orgin_type, abnormal_orgin, checker, check_link, check_num)
    VALUES
      (p_abn_rec.abnormal_id, p_abn_rec.company_id, p_abn_rec.abnormal_code,
       p_abn_rec.order_id, p_abn_rec.progress_status, p_abn_rec.goo_id,
       p_abn_rec.anomaly_class, p_abn_rec.problem_class,
       p_abn_rec.cause_class, p_abn_rec.cause_detailed,
       p_abn_rec.detailed_reasons, p_abn_rec.is_sup_responsible,
       p_abn_rec.delay_date, p_abn_rec.delay_amount,
       p_abn_rec.responsible_party, p_abn_rec.responsible_dept,
       p_abn_rec.responsible_dept_sec, p_abn_rec.handle_opinions,
       p_abn_rec.quality_deduction, p_abn_rec.is_deduction,
       p_abn_rec.deduction_method, p_abn_rec.deduction_unit_price,
       p_abn_rec.file_id, p_abn_rec.applicant_id, p_abn_rec.applicant_date,
       p_abn_rec.confirm_id, p_abn_rec.confirm_company_id,
       to_date(to_char(p_abn_rec.confirm_date, 'YYYY-MM-DD'), 'YYYY-MM-DD'),
       p_abn_rec.create_id, p_abn_rec.create_time, p_abn_rec.update_id,
       p_abn_rec.update_time, p_abn_rec.origin, p_abn_rec.origin_id,
       p_abn_rec.memo, p_abn_rec.abnormal_range, p_abn_rec.orgin_type,
       p_abn_rec.abnormal_orgin, p_abn_rec.checker, p_abn_rec.check_link,
       p_abn_rec.check_num);
  ...
  *============================================*/
  ...
  BEGIN
    --校验异常单
    check_abnormal(p_abn_rec => p_abn_rec, p_is_check => p_is_check);
    IF p_abn_rec.anomaly_class = 'AC_DATE' THEN
      UPDATE t_abnormal t
         SET t.delay_date           = p_abn_rec.delay_date,
             ...
             t.abnormal_orgin       = p_abn_rec.abnormal_orgin,
             t.checker              = p_abn_rec.checker,
             t.check_link           = p_abn_rec.check_link,
             t.check_num            = p_abn_rec.check_num
  ...
  *============================================*/
  ...
  BEGIN
    ...
    --20220613 clj、zxp 新增需求：批量复制异常单，保存时增加校验异常范围  暂停开发
    --操作复制的订单，复制后异常范围为 全部 ，则需直接赋值异常数量为订单数量
    --操作复制的订单，复制后异常范围为 订单颜色 ，则需直接赋值异常数量为订单颜色的总和
    --操作复制的订单，复制后异常范围为 指定数量 ，则异常数量为空；
    --操作提交，需校验存在订单扣款方式为“指定数量”，但异常数量为空则不可操作提交，提示：提交失败！存在订单“指定数量”，但异常数量为空，请补充后再提交。
    /*  IF ' ' || v_abn_rec_cp.abn_range || ' ' = ' 00 ' THEN
      SELECT nvl(MAX(t.order_amount), 0)
        INTO v_order_amount
        FROM scmdata.t_production_progress t
       WHERE t.goo_id = v_abn_rec.goo_id
         AND t.order_id = v_abn_rec.order_id
         AND t.company_id = p_company_id;
    ELSIF ' ' || v_abn_rec_cp.abn_range || ' ' = ' 01 ' THEN
      IF ' ' || v_abn_rec.abn_range || ' ' = ' 01 ' THEN
        p_abn_rec.delay_amount := v_abn_rec.delay_amount;
      ELSE
        p_abn_rec.delay_amount := NULL;
      END IF;
    ELSE
      SELECT SUM(CASE
                   WHEN instr(' ' || v_abn_rec_cp.abn_range || ' ',
    ...
    v_abn_rec_cp.abnormal_range := v_abn_rec.abnormal_range;
  
    v_abn_rec_cp.update_id   := p_user_id;
    v_abn_rec_cp.update_time := SYSDATE;
  
    update_abnormal(p_abn_rec => v_abn_rec_cp, p_is_check => 0);
  ...
  *============================================*/
  ...
  BEGIN
    INSERT INTO t_production_node
      (product_node_id, company_id, product_gress_id, product_node_code,
       node_num, node_name, time_ratio, target_completion_time,
       plan_completion_time, actual_completion_time, complete_amount,
       progress_status, progress_say, update_id, update_date, create_id,
       create_time, memo)
    VALUES
      (pno_rec.product_node_id, pno_rec.company_id,
       pno_rec.product_gress_id, pno_rec.product_node_code, pno_rec.node_num,
       pno_rec.node_name, pno_rec.time_ratio,
       trunc(pno_rec.target_completion_time),
       trunc(pno_rec.plan_completion_time),
       trunc(pno_rec.actual_completion_time), pno_rec.complete_amount,
       pno_rec.progress_status, pno_rec.progress_say, pno_rec.update_id,
       pno_rec.update_date, pno_rec.create_id, pno_rec.create_time,
       pno_rec.memo);
  ...
  *============================================*/
  ...
  BEGIN
    ...
    ----20220810 lsl167 禅道829
    /*扣款处理列表主表的”责任部门（1级）“只包含列举的部门：物流部/男装品类部/女装品类部/鞋包品类部/淘品品类部/美妆品类部/内衣事业部，并且”实际应扣金额“=0的扣款数据。
    
    操作[确认]成功后，数据流转到【扣款处理列表-已处理】“是否免扣”需自动标识为：是 */
  
    SELECT COUNT(1)
      INTO v_count
      FROM scmdata.t_ordered po
     WHERE EXISTS (SELECT 1
              FROM (SELECT DISTINCT abn.responsible_dept
                      FROM scmdata.t_deduction td
                     INNER JOIN scmdata.t_abnormal abn
                        ON po.company_id = abn.company_id
                       AND po.order_code = abn.order_id
                       AND td.abnormal_id = abn.abnormal_id
                       AND abn.responsible_dept IS NOT NULL) tr
             WHERE tr.responsible_dept IN
                   ('b550778b4f2236b4e0533c281caca074',
                    'b550778b4efc36b4e0533c281caca074',
                    'b550778b4f1736b4e0533c281caca074',
                    'b550778b4f4936b4e0533c281caca074',
                    'b550778b4ef436b4e0533c281caca074',
                    'b550778b4f0536b4e0533c281caca074',
                    'b550778b4f1036b4e0533c281caca074'))
       AND EXISTS (SELECT 1
              FROM (SELECT SUM(td.actual_discount_price) sum_price
                      FROM t_deduction td
                     WHERE td.company_id = po.company_id
                       AND td.order_id = po.order_code) ta
             WHERE sum_price = 0)
       AND po.company_id = po_rec.company_id
       AND po.order_id = po_rec.order_id;
    IF po_rec.approve_status = '00' THEN
      IF v_count = 1 THEN
        UPDATE scmdata.t_ordered po
  ...
  *============================================*/
  ...
  BEGIN
    ...
    SELECT MAX(dc.deduction_type),
     ...
     WHERE tc.company_id = p_company_id
       ...
       AND trunc(p_order_create_time) BETWEEN td.validity_begin_date AND
           td.validity_end_date;
  ...
  BEGIN
    FOR p_date_rec IN p_date_cur(p_delivery_date) LOOP
      ...
      get_deduction(p_company_id        => p_pro_rec.company_id,
                    ...
                    p_order_create_time => p_order_create_time,
                    po_deduction_type   => v_deduction_type,
                    po_deduction_money  => v_deduction_money);
    ...
    FOR p_date_rec IN p_date_cur(p_delivery_date) LOOP
      ...
      ELSE
        ...
        sync_abn_ded_bill(p_company_id     => p_pro_rec.company_id,
                          ...
                          p_deduction_type => v_deduction_type,
                          p_delay_day      => v_delay_day, --延期天数
                          /* 原代码     p_delay_amount       => p_date_rec.delivery_amount_sum, --延期数量*/
                          ----20220822 lsl167 需求优化 禅道805
                          --订单分类为“内衣”，且异常分类为“交期异常”的扣款明细，则“数量”取对应订单【生产进度-已完成订单-主表】的“交货数量” 其他分部订单按原有规则取数
                          p_delay_amount => p_pro_rec.delivery_amount, ---交货数量
                          ----end
                          p_is_sup               => p_pro_rec.is_sup_responsible, --供应商是否免责
                          p_order_price          => v_order_price, --扣款单价
                          p_deduction_proportion => v_deduction_proportion, --扣款比例
  ...
  BEGIN
    ...
    ELSE
      ...
      sync_abn_ded_bill(p_company_id     => p_pro_rec.company_id,
                        ...
                        p_deduction_type => v_deduction_type,
                        p_delay_day      => v_delay_day, --延期天数
                        /*原代码     p_delay_amount       => v_delivery_amount, --延期数量*/
                        ----20220822 lsl167 需求优化 禅道805
                        --订单分类为“内衣”，且异常分类为“交期异常”的扣款明细，则“数量”取对应订单【生产进度-已完成订单-主表】的“交货数量” 其他分部订单按原有规则取数
                        p_delay_amount => p_pro_rec.delivery_amount, ---交货数量
                        ----end
  ...
  PROCEDURE tranf_deduction_config(p_delivery_rec scmdata.t_delivery_record%ROWTYPE) IS
    ...
    v_order_create_time      DATE;
  BEGIN
    --20220823 zxp 871
    --增加根据订单的“订单创建时间”匹配【配置中心-生产管理配置延期扣款规则配置】是否有对应生效时间的配置模型
    --如果没有则无需生成扣款，如果有则根据扣款原有生成逻辑生成扣款。
    SELECT MAX(t.create_time)
      INTO v_order_create_time
      FROM scmdata.t_ordered t
     WHERE t.order_code = p_delivery_rec.order_code
       AND t.company_id = p_delivery_rec.company_id;
    ...
    check_deduction_config(p_company_id        => p_delivery_rec.company_id,
                           ...
                           p_status            => 1,
                           p_order_create_time => v_order_create_time,
                           po_count            => vo_count,
  ...
  *============================================*/
  ...
  BEGIN
    --订单交期
    SELECT trunc(po.delivery_date), trunc(po.create_time)
      INTO v_delivery_date, v_order_create_time
      FROM scmdata.t_ordered po
     WHERE po.company_id = p_pro_rec.company_id
       AND po.order_code = p_pro_rec.order_id;
  ...
  *============================================*/
  PROCEDURE sync_abn_ded_bill(p_company_id           VARCHAR2,
    ...
    v_abnormal_id VARCHAR2(32);
    --v_deduction_method VARCHAR2(32);
    ----20220721 lsl167 新增
    v_problem_class    VARCHAR2(100);
    v_cause_class      VARCHAR2(100);
    v_cause_detailed   VARCHAR2(100);
    v_responsible_dept VARCHAR2(100);
    ----lsl167 end
  BEGIN
  
    ----20220721 lsl167 禅道814 新增交期异常系统自动创建时增加延期问题分类、延期原因分类、延期原因细分、责任部门1级
    SELECT t.delay_problem_class, ---延期问题分类
           t.delay_cause_class, --延期原因分类
           t.delay_cause_detailed, --延期原因细分
           t.responsible_dept --责任部门1级
      INTO v_problem_class,
           v_cause_class,
           v_cause_detailed,
           v_responsible_dept
    ...
    IF p_orgin = 'SC' THEN
      ...
      */
      ----20220721 lsl167 禅道814 新增交期异常系统自动创建时增加延期问题分类、延期原因分类、延期原因细分、责任部门1级
      p_abn_rec.problem_class    := v_problem_class; --问题分类
      p_abn_rec.cause_class      := v_cause_class; --原因分类
      p_abn_rec.cause_detailed   := v_cause_detailed; --原因细分
      p_abn_rec.responsible_dept := v_responsible_dept; ---责任部门1级
      ----lsl167 end
      ...
      p_abn_rec.deduction_unit_price := nvl((CASE
                                              WHEN p_deduction_type = 'METHOD_00' THEN
                                               p_order_price
                                              WHEN p_deduction_type = 'METHOD_01' THEN
                                               p_discount_price
                                              WHEN p_deduction_type = 'METHOD_02' THEN
                                               p_deduction_proportion * 100
                                            END),
                                            0);
      ...
      p_abn_rec.is_sup_responsible   := p_is_sup;
      --20220708 781 异常处理相关优化
      p_abn_rec.origin         := p_orgin;
      p_abn_rec.origin_id      := p_duc_rec.deduction_id; --来源ID
      p_abn_rec.orgin_type     := 'TD'; --来源类型
      p_abn_rec.abnormal_orgin := NULL; --异常来源
      p_abn_rec.checker        := NULL; --查货人
      p_abn_rec.check_link     := NULL; --查货环节
    
      --生成异常单
      handle_abnormal(p_abn_rec => p_abn_rec);
      ...
      p_duc_rec.deduction_amount    := p_delay_amount; --数量
      p_duc_rec.discount_type       := p_deduction_type; --扣款类型
      p_duc_rec.discount_unit_price := nvl(p_order_price, 0); --扣款单价
      p_duc_rec.discount_proportion := nvl(p_deduction_proportion * 100, 0); --扣款比例
      p_duc_rec.discount_price      := nvl(p_discount_price, 0); --扣款金额
      /*原代码
            p_duc_rec.adjust_type           := '';
      */
      ----20220721 LSL167 增加需求 系统自动调整的明细：增加自动赋值“调整类型”逻辑
    
      p_duc_rec.adjust_type := (CASE
                                 WHEN p_is_sup = 1 THEN
                                  (CASE
                                    WHEN v_responsible_dept =
                                         'b550778b4efc36b4e0533c281caca074' /*女装品类部*/
                                         OR v_responsible_dept =
                                         'b550778b4f1736b4e0533c281caca074' /*淘品品类部*/
                                         OR v_responsible_dept =
                                         'b550778b4ef436b4e0533c281caca074' /*男装品类部*/
                                         OR v_responsible_dept =
                                         'b550778b4f0536b4e0533c281caca074' /*美妆品类部*/
                                         OR v_responsible_dept =
                                         'b550778b4f1036b4e0533c281caca074' /*鞋包品类部*/
                                         OR v_responsible_dept =
                                         'b550778b4f2236b4e0533c281caca074' /*内衣事业部*/
                                     THEN
                                     '00'
                                    WHEN v_responsible_dept =
                                         'b550778b4f4936b4e0533c281caca074' THEN
                                     '01' /*物流部*/
                                    WHEN v_responsible_dept =
                                         'b550778b4f2b36b4e0533c281caca074' THEN
                                     '02' /*供应链管理部部*/
                                  --测试端
                                  -- WHEN v_responsible_dept = 'DP2203235732410750' THEN '03' /*无*/
                                  --生产端
                                    WHEN v_responsible_dept = 'DP2203084052507152' THEN
                                     '03' /*无*/
                                    WHEN v_responsible_dept =
                                         'b550778b4f5936b4e0533c281caca074' THEN
                                     '06' /*商品中心财务部*/
                                  END)
                                 ELSE
                                  ''
      ...
      p_duc_rec.adjust_reason := CASE
                                   ...
                                   ELSE
                                    ''
                                 END;
      p_duc_rec.actual_discount_price := nvl(round(p_act_discount_price, 2),
                                             0); --实际扣款金额
      ...
      insert_deduction(p_duc_rec => p_duc_rec);
      --2.手动创建
    ELSIF p_orgin = 'MA' THEN
      ...
      p_duc_rec.discount_type         := p_deduction_type; --扣款类型
      p_duc_rec.discount_unit_price   := nvl(p_order_price, 0); --扣款单价
      p_duc_rec.discount_proportion   := nvl(p_deduction_proportion, 0); --扣款比例
      p_duc_rec.discount_price        := nvl(p_discount_price, 0); --扣款金额
      p_duc_rec.adjust_type           := '';
      p_duc_rec.adjust_proportion     := '';
  ...
  BEGIN
    ...
    SELECT MAX(CASE
                 WHEN va.dept_name = '跟单部' THEN
                  'ed7ff3c7135a236ae0533c281caccd8d' --供应商
                 ELSE
                  va.company_dept_id
               END)
  ...
  *============================================*/
  ...
  BEGIN
    INSERT INTO t_deduction
      (deduction_id, company_id, order_company_id, order_id, abnormal_id,
       deduction_status, discount_unit_price, deduction_amount,
       discount_type, discount_proportion, discount_price, adjust_type,
       adjust_proportion, adjust_price, adjust_reason, actual_discount_price,
       create_id, create_time, update_id, update_time, memo, orgin,
       arrival_date)
    VALUES
      (p_duc_rec.deduction_id, p_duc_rec.company_id,
       p_duc_rec.order_company_id, p_duc_rec.order_id, p_duc_rec.abnormal_id,
       p_duc_rec.deduction_status, p_duc_rec.discount_unit_price,
       p_duc_rec.deduction_amount, p_duc_rec.discount_type,
       p_duc_rec.discount_proportion, p_duc_rec.discount_price,
       p_duc_rec.adjust_type, p_duc_rec.adjust_proportion,
       p_duc_rec.adjust_price, p_duc_rec.adjust_reason,
       p_duc_rec.actual_discount_price, p_duc_rec.create_id,
       p_duc_rec.create_time, p_duc_rec.update_id, p_duc_rec.update_time,
       p_duc_rec.memo, p_duc_rec.orgin, p_duc_rec.arrival_date);
