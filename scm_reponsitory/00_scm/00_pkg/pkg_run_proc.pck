CREATE OR REPLACE PACKAGE SCMDATA.pkg_run_proc IS
  ...
  PROCEDURE insert_log(p_record_id VARCHAR2,
                       ...
                       p_log_flag  CHAR,
                       p_msg       CLOB);
  --把scmdata.t_day_proc存在子任务，就把子任务记录到 scmdata.t_day_proc_zi
  PROCEDURE merge_day_proc_zi(p_seqno     NUMBER, --主任务号
                              p_seqno_zi  NUMBER, --子任务号
                              p_sql       CLOB, --执行sql / plsql块
                              p_pause     NUMBER,
                              p_starttime DATE,
                              p_interval  VARCHAR2,
  ...
  PROCEDURE run_proc_sql_l(p_seqno NUMBER);
END pkg_run_proc;
/
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_run_proc IS
  ...
  --执行 scmdata.t_day_proc 中主任务及其子任务的proc_sql
  /*  PROCEDURE run_proc_sql_seq(p_seqno NUMBER) IS
    vo_po_flag BOOLEAN;
    vo_msg     CLOB;
    proc_rec   scmdata.t_day_proc%ROWTYPE;
  BEGIN
    ...
    SELECT a.*
     ...
     WHERE a.seqno = p_seqno
       AND a.pause = 1;
    IF proc_rec.subtasks IS NOT NULL THEN
      SELECT COUNT(1)
        INTO v_flag
        FROM scmdata.t_day_proc b
       WHERE b.pause = 1
         AND b.is_running = 1
         AND instr(proc_rec.seqno, b.seqno) > 0;
  ...
  BEGIN
    ...
    IF p_log_flag = 'F' THEN
      ...
      v_wx_rec.company_id            := 'a972dd1ffe3b3a10e0533c281cac8fd7';
      v_wx_rec.status                := 2;
  ...
  PROCEDURE merge_day_proc_zi(p_seqno     NUMBER, --主任务号
                              ...
                              p_sql       CLOB, --执行sql / plsql块
                              p_pause     NUMBER,
                              p_starttime DATE, --下一次开始执行时间
                              p_interval  VARCHAR2,
                              p_proc_name VARCHAR2, --任务名称 
                              p_remarks   VARCHAR2) --备注 
   IS
  BEGIN
    MERGE INTO scmdata.t_day_proc_zi a
    USING (SELECT p_seqno     parent_seqno, --主任务号
                  p_seqno_zi  seqno, --子任务号
                  p_sql       proc_sql, --执行sql / plsql块
                  p_pause     pause, --是否禁用
                  p_starttime start_time, --下一次开始执行时间
                  p_interval  INTERVAL,
                  p_proc_name proc_name, --任务名称 
                  p_remarks   remarks --备注 
             FROM dual) b
    ON (a.seqno = b.parent_seqno AND a.seqno = p_seqno_zi)
    WHEN MATCHED THEN
      UPDATE
         SET a.proc_sql   = b.proc_sql,
             a.pause      = b.pause,
    ...
    WHEN NOT MATCHED THEN
      ...
      VALUES
        (scmdata.f_get_uuid(),
         ...
         b.proc_name,
         b.remarks);
  END merge_day_proc_zi;

  PROCEDURE run_proc_sql_l(p_seqno NUMBER) IS
    v_num      NUMBER;
    vo_po_flag BOOLEAN;
    vo_msg     CLOB;
    v_sql      VARCHAR2(1000);
  ...
  BEGIN
    SELECT COUNT(*)
      INTO v_num
      FROM scmdata.t_day_proc_zi t
     WHERE t.parent_seqno = p_seqno
       AND t.pause = 1;
    IF v_num > 0 THEN
      ...
      FOR i IN (SELECT *
                 ...
                 WHERE t.parent_seqno = p_seqno
                   AND t.pause = 1) LOOP
        IF trunc(i.start_time) = trunc(SYSDATE) THEN
        
          EXECUTE IMMEDIATE 'BEGIN  pkg_run_proc.proc_sql(:p_seqno,:p_sql,:po_flag,:p_out_msg); END;'
            USING IN i.seqno, IN i.proc_sql, OUT vo_po_flag, OUT vo_msg;
        
          v_sql := q'[select trunc(sysdate) +]' || i.interval ||
                   q'[ from dual]';
          EXECUTE IMMEDIATE v_sql
            INTO v_time;
        
          EXECUTE IMMEDIATE 'update scmdata.t_day_proc_zi t set t.start_time = :start_time where t.seqno = :seqno'
            USING v_time, i.seqno;
          COMMIT;
        END IF;
