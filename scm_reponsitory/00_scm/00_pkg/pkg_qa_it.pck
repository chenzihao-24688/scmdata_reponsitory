...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_qa_it IS
  ...
  BEGIN
    ...
    ELSE
      ...
      IF v_hournum <= 12 THEN
        v_intervalname := '上午';
      ELSIF v_hournum > 12 AND v_hournum <= 18 THEN
        v_intervalname := '下午';
      ELSIF v_hournum > 18 AND v_hournum <= 24 THEN
        v_intervalname := '晚上';
      END IF;
    END IF;
 ...
 WHERE orditem.order_id = :a
  ...
  BEGIN
    ...
    v_sql := 'SELECT asn_id,company_id,
       ...
       NVL(SUM(got_amount),0) got_amount,
 ...
 GROUP BY asn_id,company_id';
  ...
  VALUES
    ...
    EXECUTE IMMEDIATE v_sql
      USING v_inp_asnid, v_inp_compid, v_inp_compid, v_inp_gooid, v_inp_orderamount, v_inp_pcomeamount, v_inp_asngotamount, v_inp_gotamount, v_inp_retamount, v_inp_picktime, v_inp_shimenttime, v_inp_receivetime, v_inp_sortingtime, v_inp_warehousepos, v_inp_isflcout, v_inp_qcrequired, v_inp_memo, v_inp_createid, v_inp_createtime;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      v_errinfo := 'Format_Error_Backtrace:' ||
                   ...
                   v_inp_asngotamount || CHR(10) || 'got_amount: ' ||
                   v_inp_gotamount || CHR(10) || 'ret_amount: ' ||
...
VALUES
  (:v_inp_asnid,
   ...
   :v_inp_createtime)';
    ...
    EXECUTE IMMEDIATE v_sql
      USING v_inp_asnid, v_inp_compid, v_inp_compid, v_inp_gooid, v_inp_barcode, v_inp_orderamount, v_inp_pcomeamount, v_inp_asngotamount, v_inp_gotamount, v_inp_retamount, v_inp_picktime, v_inp_shimenttime, v_inp_receivetime, v_inp_warehousepos, v_inp_memo, v_inp_createid, v_inp_createtime;
  ...
  BEGIN
    ...
    IF v_pcomedate IS NOT NULL AND v_scantime IS NOT NULL AND
       v_suppliercode IS NOT NULL THEN
  ...
  BEGIN
    ...
    IF v_orderamount IS NOT NULL AND v_gotamount IS NOT NULL AND
       v_deliverydate IS NOT NULL AND v_jugnum = 0 THEN
      --上架时间，分拣时间赋值
  ...
  BEGIN
    ...
    --参数非空控制
    IF v_asnid IS NOT NULL AND v_compid IS NOT NULL AND v_gooid IS NOT NULL AND
       v_jugnum = 0 THEN
      --新增进入 Asnorders
  ...
  BEGIN
    ...
    LOOP
      --动态游标赋值
      FETCH asnitemcursor
        INTO v_gooid, v_barcode, v_orderamount, v_gotamount, v_deliverydate;
      --动态游标无值退出
  ...
  BEGIN
    ...
    ELSE
      ...
      IF v_ordsjugnum = 1 AND v_ordsitemjugnum = 0 THEN
      ...
      ELSIF v_ordsjugnum = 1 AND v_ordsitemjugnum = 1 THEN
