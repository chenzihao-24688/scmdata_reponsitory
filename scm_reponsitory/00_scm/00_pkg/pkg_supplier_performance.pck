...
create or replace package body scmdata.pkg_supplier_performance is
  ...
  /*-------------------------------------------------------------  
     ...
     2022-05-19 因需求变更，（1）统计维度由供应商修改为供应商+分类，
                            （2）修改了订单满足率、补货平均交期、延货金额、延货数量取值范围，
                            （3）新增异常金额、异常数量
     2022-07-19 新增门店退货金额、总定价金额算门店退货率
     2023-2-28 由于Qa重构上线薇稳定，QA指标暂不更新
  ---------------------------------------------------------------*/
  ...
  begin
    v_z1 := q'[ merge into scmdata.t_supplier_performance_month tka
      using (select pt.company_id, to_char(pt.order_create_date, 'yyyy') year, to_char(pt.order_create_date, 'mm') month,
       pt.category, pt.category_name, t.supplier_code, t.supplier_company_name  supplier_name,
    ...
    v_z2 := q'[ group by pt.company_id,to_char(pt.order_create_date, 'yyyy'),to_char(pt.order_create_date, 'mm'), 
                        pt.category,pt.category_name,t.supplier_code,t.supplier_company_name) tkb
      ...
      using (select k.company_id,k.supplier_code,k.supplier_name,k.year,k.month,k.category,k.category_name,
               ...
               from (select oh.company_id, sp2.supplier_code,sp2.supplier_company_name supplier_name,to_char(tdr.delivery_date, 'yyyy') year,
                      ...
                      inner join scmdata.pt_ordered pt
                        ...
                        and pt.supplier_code  = sp2.inside_supplier_code ) k ]';
      ...
      v_z1 := q'[ merge into scmdata.t_supplier_performance_month tka
        using (select t1.company_id,t1.supplier_company_name supplier_name,t1.supplier_code,t1.category,t1.category_name,
            t1.year,lpad(t1.month, 2, 0) month, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name,t.supplier_code,pt.category,
                            pt.category_name, pt.year,pt.month,sum(pt.order_money) order_money
                       from scmdata.pt_ordered pt
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.inside_supplier_code = pt.supplier_code 
                      group by pt.company_id, t.supplier_company_name, pt.year, pt.month,t.supplier_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.supplier_company_name,tp.supplier_code,tp.category,tp.category_name,tp.year,tp.month, sum(tp.sum_money) sho_order_money
                           from (select t3.company_id,t.supplier_company_name,t.supplier_code,t3.category,t3.category_name,t3.year,t3.month,t3.satisfy_money sum_money
                                   from scmdata.pt_ordered t3
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3.company_id
                                    and t.inside_supplier_code = t3.supplier_code 
                                   union all
                                 select t3a.company_id, t.supplier_company_name,t.supplier_code, t3a.category,t3a.category_name,t3a.year,t3a.month,
    ...
    begin
      ...
      using (select t1.company_id, t1.year, t1.month, t1.supplier_name,t1.supplier_code,
       t1.category,t1.category_name, sum(t1.sum1_money) delivery_money,sum(t1.sum1_date*t1.sum1_money) delivery_order_money
              from (select t5.company_id,t.supplier_company_name supplier_name,tor.supplier_code,
               ...
               inner join scmdata.t_supplier_info t
                  on t.company_id = t5.company_id
                 and t.inside_supplier_code = t5.supplier_code
      ...
      using (select t2.company_id,t2.supplier_code,t2.supplier_company_name supplier_name, t2.year, t2.month,t2.category,t2.category_name,
               ...
               from (select oh.company_id,sp2.supplier_code,sp2.supplier_company_name, to_char(od.delivery_date, 'yyyy') year, pt.category,pt.category_name,
                      ...
                      inner join scmdata.pt_ordered pt
                         on pt.product_gress_code = tdr.order_code
                        and pt.company_id = tdr.company_id
                        and pt.supplier_code  = sp2.inside_supplier_code 
                      inner join scmdata.sys_company_dept a
...
/*
  ...
  from (select to_char(a.confirm_date, 'yyyy') year,
         ...
         where oh.company_id = 'b6cc680ad0f599cde0531164a8c0337f'
  ...
  /*只更新或者总定价金额、门店退货金额字段*/
    begin
      v_z1 := q'[ merge into scmdata.t_supplier_performance_month tka
      using ( select z.company_id, z.year, z.month, z.category, z.supplier_code,t.supplier_company_name, w.group_dict_name category_name, y.order_amount, y.order_money,z.return_amount, z.return_money
  from (select a.company_id, a.year, a.quarter, a.month, b.category, a.supplier_code,  
               sum(a.examount) return_amount, sum(a.examount * b.price) return_money
         ...
         group by a.company_id, a.year, a.quarter, a.month, a.supplier_code, b.category) z
  left join (select a.company_id, a.supplier_code, extract(year from a.create_time) year,
                    extract(month from a.create_time) month, c.category,
              sum(b.amount) order_amount, sum(b.amount * c.price) order_money
              ...
              inner join scmdata.t_commodity_info c
                ...
                and a.company_id = c.company_id
              where b.amount > 0
              group by a.company_id, a.supplier_code, extract(year from a.create_time), extract(month from a.create_time), c.category) y
    on z.company_id = y.company_id
   and z.year = y.year
   and z.month = y.month
   and z.category = y.category
   and y.supplier_code = z.supplier_code
 inner join scmdata.sys_group_dict w
    on w.group_dict_value = z.category
   and w.group_dict_type = 'PRODUCT_TYPE'
 inner join scmdata.t_supplier_info t
    on t.company_id = z.company_id
   and t.supplier_code = z.supplier_code ]';
      v_w1 := q'[  where (z.year || z.month) < to_char(sysdate,'yyyymm') ) tkb ]';
      v_w2 := q'[  where (z.year || z.month) = to_char(add_months(sysdate,-1),'yyyymm') ) tkb ]';
      v_z2 := q'[ on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.month = tkb.month 
              and tka.supplier_code = tkb.supplier_code and tka.category = tkb.category)
      when matched then
        update
           set tka.total_pricing_money = tkb.order_money,
               tka.store_return_money  = tkb.return_money,
               tka.supplier_name       = tkb.supplier_company_name,
               tka.update_id           = 'ADMIN',
               tka.update_time         = sysdate
      when not matched then
        insert
          (tka.t_sp_month_id,
           ...
           tka.category,
           tka.category_name,
           tka.total_pricing_money,
           tka.store_return_money,
           tka.create_id,
           tka.create_time)
        values
          (scmdata.f_get_uuid(),
  ...
  begin
    ...
    using (select pt.company_id,to_char(pt.order_create_date, 'yyyy') year,to_char(pt.order_create_date, 'Q') quarter,
                  ...
                  t.supplier_company_name supplier_name, sum(pt.order_amount) order_amount, sum(pt.order_money) order_money
             from scmdata.pt_ordered pt 
            inner join scmdata.t_supplier_info t
               on t.company_id = pt.company_id
              and t.inside_supplier_code = pt.supplier_code ]';
            ...
            t1.year,t1.quarter, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name,t.supplier_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.inside_supplier_code = pt.supplier_code 
                      group by pt.company_id, t.supplier_company_name, pt.year, pt.quarter,t.supplier_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.supplier_company_name,tp.supplier_code,tp.category,tp.category_name,tp.year,tp.quarter, sum(tp.sum_money) sho_order_money
                           from (select t3.company_id,t.supplier_company_name,t.supplier_code,t3.category,t3.category_name,t3.year,t3.quarter,t3.satisfy_money sum_money
                                   from scmdata.pt_ordered t3
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3.company_id
                                    and t.inside_supplier_code = t3.supplier_code 
                                   union all
                                 select t3a.company_id, t.supplier_company_name,t.supplier_code, t3a.category,t3a.category_name,t3a.year,t3a.quarter,
                                        (t3a.order_money  - t3a.satisfy_money ) sum_money
                                   from scmdata.pt_ordered t3a
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3a.company_id
 ...
 /*   \*只更新或者新增QA查货字段*\
  ...
  /*只更新或者总定价金额、门店退货金额字段*/
    begin
      v_z1 := q'[ merge into scmdata.t_supplier_performance_quarter tka
      using ( select z.company_id, z.year, z.quarter, z.category, z.supplier_code,t.supplier_company_name, w.group_dict_name category_name,
                      y.order_amount, y.order_money,z.return_amount, z.return_money
  from (select a.company_id, a.year, a.quarter, b.category, a.supplier_code,  
               sum(a.examount) return_amount, sum(a.examount * b.price) return_money
          from scmdata.t_return_management a
         inner join scmdata.t_commodity_info b
            on a.goo_id = b.goo_id
         ...
         where a.audit_time is not null
           and c.dept_name = '供应链管理部'
         group by a.company_id, a.year, a.quarter, a.supplier_code, b.category) z
  left join (select a.company_id, a.supplier_code, extract(year from a.create_time) year,
                    to_char(a.create_time,'q') quarter, c.category,
              sum(b.amount) order_amount, sum(b.amount * c.price) order_money
               from scmdata.t_ingood a
              inner join scmdata.t_ingoods b
                 on a.ing_id = b.ing_id
              ...
              inner join scmdata.t_commodity_info c
                ...
                and a.company_id = c.company_id
              where b.amount > 0
              group by a.company_id, a.supplier_code, extract(year from a.create_time), to_char(a.create_time,'q'), c.category) y
    on z.company_id = y.company_id
   and z.year = y.year
 ...
 inner join scmdata.t_supplier_info t
    on t.company_id = z.company_id
   and t.supplier_code = z.supplier_code ]';
      v_w1 := q'[  where (z.year || z.quarter) <= (to_char(add_months(sysdate,-3),'yyyy')||to_char(add_months(sysdate,-3),'q'))  ) tkb ]';
      v_w2 := q'[  where (z.year || z.quarter) = (to_char(add_months(sysdate,-3),'yyyy')||to_char(add_months(sysdate,-3),'q')) ) tkb ]';
      v_z2 := q'[ on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.quarter = tkb.quarter 
              and tka.supplier_code = tkb.supplier_code and tka.category = tkb.category)
      when matched then
        update
  ...
  begin
    ...
    using (select pt.company_id,to_char(pt.order_create_date, 'yyyy') year,decode(to_char(pt.order_create_date, 'Q'),1,1,2,1,3,2,4,2) halfyear, 
                  t.supplier_company_name supplier_name,pt.category,pt.category_name,t.supplier_code,
               sum(pt.order_amount) order_amount,sum(pt.order_money) order_money
          from scmdata.pt_ordered pt 
          inner join scmdata.t_supplier_info t
            on t.company_id = pt.company_id
           and t.inside_supplier_code = pt.supplier_code]';
    /*更新全部历史数据*/
    v_w1 := q'[ where (to_char(pt.order_create_date, 'yyyy') || decode(to_char(pt.order_create_date, 'Q'),1,1,2,1,3,2,4,2)) <= scmdata.pkg_kpipt_order.f_yearmonth ]';
    /*只更新上一个半年度的数据*/
    v_w2 := q'[ where (to_char(pt.order_create_date, 'yyyy') || decode(to_char(pt.order_create_date, 'Q'),1,1,2,1,3,2,4,2)) = scmdata.pkg_kpipt_order.f_yearmonth ]';
      ...
      using (select k.company_id,k.supplier_code, k.supplier_name, k.year,decode(k.quarter,1,1,2,1,3,2,4,2) halfyear,
               ...
               from (select oh.company_id, sp2.supplier_code,sp2.supplier_company_name supplier_name,pt.category,pt.category_name,
                      ...
                      inner join scmdata.pt_ordered pt
                        ...
                        and pt.company_id = tdr.company_id
                        and pt.supplier_code  = sp2.inside_supplier_code ) k ]';
      v_w1 := q'[  where (k.year || decode(k.quarter,1,1,2,1,3,2,4,2)) <= scmdata.pkg_kpipt_order.f_yearmonth ]';
      v_w2 := q'[  where (k.year || decode(k.quarter,1,1,2,1,3,2,4,2)) = scmdata.pkg_kpipt_order.f_yearmonth ]';
    ...
    begin
            ...
            t1.year,t1.halfyear, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name,t.supplier_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.inside_supplier_code = pt.supplier_code 
                      group by pt.company_id, t.supplier_company_name, pt.year, decode(pt.quarter,1,1,2,1,3,2,4,2) ,t.supplier_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.supplier_company_name,tp.supplier_code,tp.category,tp.category_name,
                           ...
                           from (select t3.company_id,t.supplier_company_name,t.supplier_code,t3.category,t3.category_name,t3.year,
                                  ...
                                  inner join scmdata.t_supplier_info t
                                    ...
                                    and t.inside_supplier_code = t3.supplier_code 
                                   union all
                                 select t3a.company_id, t.supplier_company_name,t.supplier_code, t3a.category,
      ...
      v_w1 := q'[ where (t1.year || t1.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth ) tkb ]';
      v_w2 := q'[ where (t1.year || t1.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth ) tkb ]';
      ...
      using (select t1.company_id, t1.year,t1.halfyear, t1.supplier_name,t1.supplier_code,
       t1.category,t1.category_name, sum(t1.sum1_money) delivery_money,sum(t1.sum1_date*t1.sum1_money) delivery_order_money
              from (select t5.company_id,t.supplier_company_name supplier_name,tor.supplier_code,
               ...
               where t5.is_first_order = 0
                 and tor.is_product_order = 1)t1 ]';
      v_w1 := q'[  where (t1.year || t1.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth ]';
      v_w2 := q'[  where (t1.year || t1.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth ]';
    ...
    begin
      ...
      using (select t2.company_id, t2.supplier_code,t2.supplier_company_name supplier_name,t2.year,decode(t2.quarter,1,1,2,1,3,2,4,2)halfyear,
               ...
               from (select oh.company_id, sp2.supplier_code, sp2.supplier_company_name,pt.category,pt.category_name,
                      ...
                      where tdr.delivery_date > od.delivery_date
                        and a.dept_name = '供应链管理部') t2]';
      v_w1 := q'[   where (t2.year || decode(t2.quarter,1,1,2,1,3,2,4,2)) <= scmdata.pkg_kpipt_order.f_yearmonth ]';
      v_w2 := q'[   where (t2.year || decode(t2.quarter,1,1,2,1,3,2,4,2)) = scmdata.pkg_kpipt_order.f_yearmonth ]';
 ...
 inner join scmdata.sys_group_dict gd
    on gd.group_dict_type = 'PRODUCT_TYPE'
   and gd.group_dict_value = m1.category ]';
      v_w1 := q'[  where (m1.year || m1.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth) tkb]';
      v_w2 := q'[  where (m1.year || m1.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth) tkb]';
      v_z2 := q'[  on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.halfyear = tkb.halfyear
              and tka.supplier_code = tkb.supplier_code and tka.category = tkb.category)
      when matched then
        update
               ...
               from (select k.order_id,k.company_id, k.supplier,k.supplier_code,k.category,k.category_name, min(k.finish_time) finish_time, k.qc_check_node, k.last_qc_result,k.order_money 
                       from (select o.order_id,o.company_id, si.supplier_company_name supplier, qc.finish_time, g.qc_check_node,pt.order_money ,oe.supplier_code,pt.category,pt.category_name,
                              ...
                              where g.qc_check_node = 'QC_FIRST_CHECK'
                                and qc.finish_time is not null) k
                      group by k.order_id, k.supplier,k.supplier_code,k.category,k.category_name, k.company_id,k.qc_check_node, k.last_qc_result,k.order_money) base ]';
      v_w1 := q'[  where (to_char(base.finish_time, 'yyyy') || decode(to_char(base.finish_time, 'Q'),1,1,2,1,3,2,4,2)) <= scmdata.pkg_kpipt_order.f_yearmonth]';
      v_w2 := q'[  where (to_char(base.finish_time, 'yyyy') || decode(to_char(base.finish_time, 'Q'),1,1,2,1,3,2,4,2)) = scmdata.pkg_kpipt_order.f_yearmonth]';
      ...
      using (select k.company_id, k.year,k.halfyear,k.qasupplier supplier_name,k.supplier_code,k.category,k.category_name,
           sum(k.pcomesum_amount) pcomesum_amount, sum(k2.pcomesum_amount) np_amount
               ...
               left join (select distinct to_char(a.check_date, 'yyyy') year, decode( to_char(a.check_date, 'Q'),1,1,2,1,3,2,4,2)halfyear, a.qa_report_id,
                ...
                and k2.supplier_code = k.supplier_code
                and k2.category = k.category  ]';
      v_w1 := q'[ where (k.year || k.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth]';
      v_w2 := q'[ where (k.year || k.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth]';
...
begin
  v_z1 := q'[ merge into scmdata.t_supplier_performance_halfyear tka
      using ( select z.company_id, z.year, z.halfyear, z.category, z.supplier_code,t.supplier_company_name,
                     w.group_dict_name category_name, y.order_amount, y.order_money,z.return_amount, z.return_money
  from (select a.company_id, a.year, decode(a.quarter,1,1,2,1,3,2,4,2) halfyear, b.category, a.supplier_code,  
               sum(a.examount) return_amount, sum(a.examount * b.price) return_money
          from scmdata.t_return_management a
         inner join scmdata.t_commodity_info b
            on a.goo_id = b.goo_id
         ...
         where a.audit_time is not null
           and c.dept_name = '供应链管理部'
         group by a.company_id, a.year, decode(a.quarter,1,1,2,1,3,2,4,2),  a.supplier_code, b.category) z
  left join (select a.company_id, a.supplier_code, extract(year from a.create_time) year,
                    decode(to_char(a.create_time, 'Q'),1,1,2,1,3,2,4,2) halfyear,  c.category,
              sum(b.amount) order_amount, sum(b.amount * c.price) order_money
               from scmdata.t_ingood a
              inner join scmdata.t_ingoods b
                 on a.ing_id = b.ing_id
              ...
              inner join scmdata.t_commodity_info c
                ...
                and a.company_id = c.company_id
              where b.amount > 0
              group by a.company_id, a.supplier_code, extract(year from a.create_time), decode(to_char(a.create_time, 'Q'),1,1,2,1,3,2,4,2), c.category) y
    on z.company_id = y.company_id
   and z.year = y.year
 ...
 inner join scmdata.t_supplier_info t
    on t.company_id = z.company_id
   and t.supplier_code = z.supplier_code ]';
  v_w1 := q'[  where (z.year || z.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth   ) tkb ]';
  v_w2 := q'[  where (z.year || z.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth ) tkb ]';
  v_z2 := q'[ on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.halfyear = tkb.halfyear 
              and tka.supplier_code = tkb.supplier_code and tka.category = tkb.category)
      when matched then
        update
  ...
  begin
    ...
    using (select pt.company_id,to_char(pt.order_create_date, 'yyyy') year, t.supplier_company_name supplier_name,
               pt.category,pt.category_name,t.supplier_code, 
               sum(pt.order_amount) order_amount,sum(pt.order_money) order_money
          from scmdata.pt_ordered pt 
inner join scmdata.t_supplier_info t
            ...
            t1.year, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name,t.supplier_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.inside_supplier_code = pt.supplier_code 
                      group by pt.company_id, t.supplier_company_name, pt.year,t.supplier_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.supplier_company_name,tp.supplier_code,tp.category,tp.category_name,
...
begin
  v_z1 := q'[ merge into scmdata.t_supplier_performance_year tka
      using ( select z.company_id, z.year,  z.category, z.supplier_code,t.supplier_company_name,
                     w.group_dict_name category_name, y.order_amount, y.order_money,z.return_amount, z.return_money
  from (select a.company_id, a.year, b.category, a.supplier_code,  
               sum(a.examount) return_amount, sum(a.examount * b.price) return_money
          from scmdata.t_return_management a
         inner join scmdata.t_commodity_info b
            on a.goo_id = b.goo_id
         ...
         where a.audit_time is not null
           and c.dept_name = '供应链管理部'
         group by a.company_id, a.year,   a.supplier_code, b.category) z
  left join (select a.company_id, a.supplier_code, extract(year from a.create_time) year,  c.category,
              sum(b.amount) order_amount, sum(b.amount * c.price) order_money
               from scmdata.t_ingood a
              inner join scmdata.t_ingoods b
                 on a.ing_id = b.ing_id
              ...
              inner join scmdata.t_commodity_info c
                ...
                and a.company_id = c.company_id
              where b.amount > 0
              group by a.company_id, a.supplier_code, extract(year from a.create_time), c.category) y
    on z.company_id = y.company_id
   and z.year = y.year
   and z.category = y.category
   and y.supplier_code = z.supplier_code
 ...
 inner join scmdata.t_supplier_info t
    on t.company_id = z.company_id
   and t.supplier_code = z.supplier_code ]';
  v_w1 := q'[  where z.year < to_char(sysdate,'yyyy')   ) tkb ]';
  v_w2 := q'[  where z.year = to_char(sysdate,'yyyy')-1 ) tkb ]';
  v_z2 := q'[ on (tka.company_id = tkb.company_id and tka.year = tkb.year 
              and tka.supplier_code = tkb.supplier_code and tka.category = tkb.category)
      when matched then
        update
  ...
  begin
    ...
    using (select pt.company_id,to_char(pt.order_create_date, 'yyyy') year,to_char(pt.order_create_date, 'mm') month,
                  ...
                  t.supplier_company_name  factory_name,sum(pt.order_amount) order_amount,sum(pt.order_money) order_money
  from scmdata.pt_ordered pt
  inner join scmdata.t_supplier_info t 
     on t.company_id = pt.company_id 
    ...
    v_w2 := q'[  where (to_char(pt.order_create_date, 'yyyy') || to_char(pt.order_create_date, 'mm')) = to_char(add_months(sysdate,-1),'yyyymm') ]';
    v_z2 := q'[  group by pt.company_id, to_char(pt.order_create_date, 'yyyy'),to_char(pt.order_create_date, 'mm'), 
                          pt.category,pt.category_name,pt.factory_code, t.supplier_company_name ) tkb
    on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.month = tkb.month
  ...
  from (select oh.company_id, sp2.supplier_company_name, to_char(tdr.delivery_date, 'yyyy') year,to_char(tdr.delivery_date, 'mm') month,
         ...
         inner join scmdata.pt_ordered pt
            on pt.product_gress_code = tdr.order_code
           and pt.company_id = tdr.company_id
           and sp2.supplier_code = pt.factory_code ) k ]';
      v_w1 := q'[  where ( k.year || k.month) < to_char(sysdate,'yyyymm')  ]';
      v_w2 := q'[  where ( k.year || k.month) = to_char(add_months(sysdate,-1),'yyyymm')  ]';
    ...
    begin
      ...
      using ( select t1.company_id,t1.factory_company_name factory_name,t1.factory_code,t1.category,t1.category_name,
            t1.year,lpad(t1.month, 2, 0) month, t1.order_money,t2.sho_order_money
               from (select pt.company_id,t.supplier_company_name factory_company_name,pt.factory_code,pt.category,
                            pt.category_name, pt.year,pt.month,sum(pt.order_money) order_money
                       from scmdata.pt_ordered pt
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.supplier_code = pt.factory_code
                      group by pt.company_id, t.supplier_company_name , pt.year, pt.month,pt.factory_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.factory_company_name,tp.factory_code,tp.category,tp.category_name,tp.year,tp.month, sum(tp.sum_money) sho_order_money
                           from (select t3.company_id,t.supplier_company_name factory_company_name,t3.factory_code,t3.category,t3.category_name,t3.year,t3.month,t3.satisfy_money sum_money
                                   from scmdata.pt_ordered t3
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3.company_id
                                    and t.inside_supplier_code = t3.factory_code  
                                   union all
                                 select t3a.company_id, t.supplier_company_name factory_company_name,t3a.factory_code, t3a.category,t3a.category_name,t3a.year,t3a.month,
                                        (t3a.order_money  - t3a.satisfy_money ) sum_money
                                   from scmdata.pt_ordered t3a
                                  inner join scmdata.t_supplier_info t 
                                     on t.company_id = t3a.company_id 
                                    and t.supplier_code = t3a.factory_code
                                  inner join scmdata.sys_company_dept a
                                     on a.company_id = t3a.company_id
                                    and a.company_dept_id = t3a.responsible_dept
                                    and a.pause = 0
    ...
    begin
      ...
      using (select t1.company_id, t1.year, t1.month, t1.factory_name,t1.factory_code,
       t1.category,t1.category_name, sum(t1.sum1_money) delivery_money,sum(t1.sum1_date*t1.sum1_money) delivery_order_money
              from (select t5.company_id,t.supplier_company_name factory_name,t5.factory_code,
                          to_char(tba.delivery_origin_time,'yyyy')year, to_char(tba.delivery_origin_time,'mm')month,t5.category, t5.category_name,
                     (to_date(to_char(tba.delivery_origin_time , 'yyyy/mm/dd'), 'yyyy/mm/dd') -
                     to_date(to_char(t5.order_create_date, 'yyyy/mm/dd'), 'yyyy/mm/dd')) sum1_date,
                     (t5.fixed_price * tba.delivery_amount) sum1_money
                ...
                inner join scmdata.t_supplier_info t 
                   on t.company_id = t5.company_id 
                  and t.supplier_code = t5.factory_code 
                inner join scmdata.t_delivery_record tba
                  on t5.product_gress_code = tba.order_code
                 and t5.company_id = tba.company_id
                inner join scmdata.t_ordered tor
  ...
  from (select oh.company_id,sp2.supplier_company_name,t.factory_code,
         ...
         inner join scmdata.t_delivery_record tdr
          ...
          inner join scmdata.pt_ordered pt
            on pt.product_gress_code = tdr.order_code
           and pt.company_id = tdr.company_id
           and pt.factory_code = sp2.inside_supplier_code  
         inner join scmdata.sys_company_dept a
            on a.company_id = t.company_id
           and a.company_dept_id =t.responsible_dept
         where tdr.delivery_date > od.delivery_date
...
*/
  ...
  begin
    ...
    using (select pt.company_id,to_char(pt.order_create_date, 'yyyy') year,to_char(pt.order_create_date, 'Q') quarter,
                  pt.category,pt.category_name,pt.factory_code,t.supplier_company_name  factory_name,
                  sum(pt.order_amount) order_amount,sum(pt.order_money) order_money
  from scmdata.pt_ordered pt 
  inner join scmdata.t_supplier_info t 
     on t.company_id = pt.company_id 
            ...
            t1.year,t1.quarter, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name factory_company_name,pt.factory_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.supplier_code = pt.factory_code
                      group by pt.company_id, t.supplier_company_name, pt.year, pt.quarter,pt.factory_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.factory_company_name,tp.factory_code,tp.category,tp.category_name,tp.year,tp.quarter, sum(tp.sum_money) sho_order_money
                           from (select t3.company_id,t.supplier_company_name factory_company_name,t3.factory_code,t3.category,t3.category_name,t3.year,t3.quarter,t3.satisfy_money sum_money
                                   from scmdata.pt_ordered t3
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3.company_id
                                    and t.inside_supplier_code = t3.factory_code  
                                   union all
                                 select t3a.company_id, t.supplier_company_name factory_company_name,t3a.factory_code, t3a.category,t3a.category_name,t3a.year,t3a.quarter,
                                        (t3a.order_money  - t3a.satisfy_money ) sum_money
                                   from scmdata.pt_ordered t3a
                                  inner join scmdata.t_supplier_info t
                                     on t.company_id = t3a.company_id
  ...
  from (select oh.company_id,sp2.supplier_company_name,t.factory_code,
         ...
         inner join scmdata.t_delivery_record tdr
          ...
          inner join scmdata.pt_ordered pt
            on pt.product_gress_code = tdr.order_code
           and pt.company_id = tdr.company_id
           and sp2.supplier_code = pt.factory_code 
         inner join scmdata.sys_company_dept a
            on a.company_id = t.company_id
           and a.company_dept_id =t.responsible_dept
         where tdr.delivery_date > od.delivery_date
 ...
 /*   \*只更新或者新增QA查货字段*\
  ...
  inner join scmdata.t_supplier_info t 
    ...
    v_w1 := q'[ where (to_char(pt.order_create_date, 'yyyy') || decode(to_char(pt.order_create_date, 'Q'),1,1,2,1,3,2,4,2)) 
                      <= scmdata.pkg_kpipt_order.f_yearmonth ]';
    /*只更新上一个半年度数据*/
            ...
            t1.year,t1.halfyear, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name factory_company_name,pt.factory_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.supplier_code = pt.factory_code
                      group by pt.company_id, t.supplier_company_name, pt.year, decode(pt.quarter,1,1,2,1,3,2,4,2),pt.factory_code,pt.category,pt.category_name) t1
               left join (select tp.company_id, tp.factory_company_name,tp.factory_code,tp.category,tp.category_name,tp.year,tp.halfyear, sum(tp.sum_money) sho_order_money
                           from (select t3.company_id, t.supplier_company_name factory_company_name,t3.factory_code,t3.category,t3.category_name,
                                  ...
                                  inner join scmdata.t_supplier_info t
                                    ...
                                    and t.inside_supplier_code = t3.factory_code  
                                   union all
                                 select t3a.company_id, t.supplier_company_name factory_company_name,t3a.factory_code, t3a.category,t3a.category_name,
      ...
      using (select t1.company_id, t1.year, t1.halfyear, t1.factory_name,t1.factory_code,
       t1.category,t1.category_name, sum(t1.sum1_money) delivery_money,sum(t1.sum1_date*t1.sum1_money) delivery_order_money
              from (select t5.company_id,t.supplier_company_name factory_name,t5.factory_code,
               ...
               where t5.is_first_order = 0
                 and tor.is_product_order = 1)t1 ]';
      v_w1 := q'[ where (t1.year ||halfyear ) <= scmdata.pkg_kpipt_order.f_yearmonth  ]';
      v_w2 := q'[ where (t1.year ||halfyear ) = scmdata.pkg_kpipt_order.f_yearmonth  ]';
  ...
  from (select oh.company_id,sp2.supplier_company_name,t.factory_code,
         ...
         inner join scmdata.t_delivery_record tdr
          ...
          inner join scmdata.pt_ordered pt
            on pt.product_gress_code = tdr.order_code
           and pt.company_id = tdr.company_id
           and pt.factory_code  = sp2.supplier_code
         inner join scmdata.sys_company_dept a
            on a.company_id = t.company_id
           and a.company_dept_id =t.responsible_dept
         where tdr.delivery_date > od.delivery_date
           and a.dept_name = '供应链管理部') t2  ]';
      v_w1 := q'[ where (t2.year ||halfyear ) <= scmdata.pkg_kpipt_order.f_yearmonth ]';
      v_w2 := q'[ where (t2.year ||halfyear ) = scmdata.pkg_kpipt_order.f_yearmonth ]';
 ...
 /*   \*只更新或者新增QA查货字段*\
  ...
  left join (select k2.year,k2.halfyear, k2.qafactory,k2.factory_code,k2.category,k2.category_name,k2.company_id,sum(k2.pcome_amount) np_amount
   ...
   and kt.category = k3.category
   and kt.company_id = k3.company_id  ]';
      v_w1 := q'[ where (kt.year || kt.halfyear) <= scmdata.pkg_kpipt_order.f_yearmonth ) tkb ]';
      v_w2 := q'[ where (kt.year || kt.halfyear) = scmdata.pkg_kpipt_order.f_yearmonth ) tkb ]';
      v_z2 := q'[ on (tka.company_id = tkb.company_id and tka.year = tkb.year and tka.halfyear = tkb.halfyear 
           and tka.factory_code = tkb.factory_code  and tka.category = tkb.category )
      when matched then
        update
  ...
  from (select oh.company_id, sp2.supplier_company_name, to_char(tdr.delivery_date, 'yyyy') year,
         ...
         inner join scmdata.pt_ordered pt
            on pt.product_gress_code = tdr.order_code
           and pt.company_id = tdr.company_id
           and pt.factory_code = sp2.supplier_code) k ]';
      v_w1 := q'[ where k.year < to_char(sysdate,'yyyy')]';
      v_w2 := q'[ where k.year = to_char(sysdate,'yyyy')-1]';
            ...
            t1.year, t1.order_money,t2.sho_order_money
               from (select pt.company_id, t.supplier_company_name factory_company_name,pt.factory_code,pt.category,
                      ...
                      inner join scmdata.t_supplier_info t
                         on t.company_id = pt.company_id
                        and t.supplier_code = pt.factory_code
                      group by pt.company_id, t.supplier_company_name, pt.year,pt.factory_code,pt.category,pt.category_name) t1
