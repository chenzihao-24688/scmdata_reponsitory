  ...
  BEGIN
    SELECT listagg(aitem, ',') || ',' || listagg(titem, ',')
      ...
      FROM (SELECT a.item_id aitem, c.item_id titem
              FROM bw3.sys_item_element_rela a
             INNER JOIN bw3.sys_associate b
                ON a.element_id = b.element_id
              LEFT JOIN bw3.sys_tree_list c
    ...
    IF rets IS NOT NULL THEN
      SELECT listagg(DISTINCT col, ',')
        INTO items
        FROM (SELECT regexp_substr(items || ',' || rets, '[^,]+', 1, LEVEL) col
                FROM dual
              CONNECT BY LEVEL <=
    ...
    SELECT listagg(aitem, ',') || ',' || listagg(tcoitem, ',') || ',' ||
      ...
      FROM (SELECT a.item_id            aitem,
              ...
              FROM bw3.sys_item_element_rela a
             INNER JOIN bw3.sys_action b
                ON a.element_id = b.element_id
             INNER JOIN bw3.sys_cond_rela c
                ON b.element_id = c.ctl_id
             INNER JOIN bw3.sys_cond_operate d
    ...
    IF ret IS NOT NULL THEN
      SELECT listagg(DISTINCT col, ',')
        INTO items
        FROM (SELECT regexp_substr(items || ',' || ret, '[^,]+', 1, LEVEL) col
                FROM dual
              CONNECT BY LEVEL <=
...
/';
  ...
  BEGIN
    ...
    scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                          ...
                                                          unq_col   => 'ITEM_ID',
                                                          unq_vals  => itmvals_c,
                                                          rela_tabs => 'BW3.SYS_ITEM,BW3.SYS_ITEM_LIST');
  
    --获取 SYS_TREE_LIST 数据
    exesql_c := 'SELECT LISTAGG(COL1,'','') WITHIN GROUP(ORDER BY RN)||''^^^^^^''||LISTAGG(COL2,'','') WITHIN GROUP(ORDER BY RN)
                   FROM (SELECT ITEM_ID COL1,NODE_ID COL2,ROWNUM RN
                           FROM BW3.SYS_TREE_LIST
                          WHERE REGEXP_COUNT(''' ||
                itmvals_c || ''',ITEM_ID) > 0)';
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_vals  => tmpvals_c,
                                                            rela_tabs => 'BW3.SYS_DETAIL_GROUP');
    END IF;
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_vals  => tmpvals_c,
                                                            rela_tabs => 'BW3.SYS_WEB_UNION');
    END IF;
  
    --SYS_ITEM_RELA
    exesql_c := 'SELECT LISTAGG(COL1,'','') WITHIN GROUP(ORDER BY RN)||''^^^^^^''||LISTAGG(COL2,'','') WITHIN GROUP(ORDER BY RN)
                   FROM (SELECT ITEM_ID COL1,RELATE_ID COL2,ROWNUM RN
                           FROM BW3.SYS_ITEM_RELA
                          WHERE REGEXP_COUNT(''' ||
                itmvals_c || ''',ITEM_ID) > 0)';
    EXECUTE IMMEDIATE exesql_c
      INTO tmpvals_c;
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      ...
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_col   => 'ELEMENT_ID',
                                                            unq_vals  => unqvals_c,
                                                            rela_tabs => 'BW3.SYS_ELEMENT,BW3.SYS_ACTION');
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            optime    => oper_time,
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      ...
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_col   => 'ELEMENT_ID',
                                                            unq_vals  => unqvals_c,
                                                            rela_tabs => 'BW3.SYS_ELEMENT,BW3.SYS_ASSOCIATE');
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            optime    => oper_time,
                                                            unq_col   => 'ELEMENT_ID,ITEM_ID',
                                                            unq_vals  => tmpvals_c,
      ...
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_col   => 'ELEMENT_ID,ITEM_ID',
                                                            unq_vals  => tmpvals_c,
                                                            rela_tabs => 'BW3.SYS_ELEMENT_HINT');
    END IF;
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      ...
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_col   => 'ELEMENT_ID',
                                                            unq_vals  => unqvals_c,
                                                            rela_tabs => 'BW3.SYS_ELEMENT,BW3.SYS_PICK_LIST');
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            optime    => oper_time,
                                                            unq_col   => 'ELEMENT_ID,ITEM_ID',
                                                            unq_vals  => tmpvals_c,
    ...
    IF tmpvals_c <> '^^^^^^' THEN
      ...
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            ...
                                                            unq_col   => 'ELEMENT_ID',
                                                            unq_vals  => unqvals_c,
                                                            rela_tabs => 'BW3.SYS_ELEMENT,BW3.SYS_ASSIGN');
      scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                            optime    => oper_time,
                                                            unq_col   => 'ELEMENT_ID,ITEM_ID',
                                                            unq_vals  => tmpvals_c,
    ...
    WHILE (tmpvals_c <> '^^^^^^') LOOP
      ...
      IF tmpvals_c <> '^^^^^^' THEN
        ...
        scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                              ...
                                                              unq_col   => 'ELEMENT_ID',
                                                              unq_vals  => unqvals_c,
                                                              rela_tabs => 'BW3.SYS_ELEMENT,BW3.SYS_LOOK_UP');
        scmdata.pkg_export_item_and_element.p_processing_info(opuser    => operator,
                                                              optime    => oper_time,
                                                              unq_col   => 'ELEMENT_ID,ITEM_ID',
                                                              unq_vals  => tmpvals_c,
