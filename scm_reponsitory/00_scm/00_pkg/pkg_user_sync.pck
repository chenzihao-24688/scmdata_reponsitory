...
--常量：三福企业
v_company_id constant varchar2(32) :='a972dd1ffe3b3a10e0533c281cac8fd7';
...
BEGIN
  ...
  if v_user_count<>1 then
    po_result:=-1;
    po_msg:='该手机号注册平台用户失败，可能存在多个相同账户';
    return;
  else 
    update sys_user a 
       set a.nick_name=PI_USER_NAME,
           a.username=PI_USER_NAME,
           a.sex=PI_SEX,
           a.birthday=PI_BIRTH
...
is
  ...
  v_msg           varchar2(300);
begin
  for item in (select * from cmx_sys_users_itf a where a.process_time is null order by a.batch_id asc) loop
    --初始化置空
     v_flag:=null;
    --1.查找是否存在员工记录
    ...
    ELSE
       update cmx_sys_users_itf t
         set t.process_time=sysdate,
             t.process_status=DECODE(V_MSG,NULL,'D','F'),
             T.error_msg =v_msg
       WHERE T.BATCH_ID=ITEM.BATCH_ID AND T.USERID=ITEM.USERID;
    END IF;
