...
CREATE OR REPLACE PACKAGE BODY SCMDATA.PKG_ASN_INTERFACE IS
  ...
  /*=================================================================================
    ...
    用途:
      在更新 SCMDATA.T_ASNORDERSITEM 时使用本过程同时更新 scmdata.t_qa_scope_backup，
      SCMDATA.T_ASNINFO_PRETRANS_TO_WMS 对应的数据和状态
    ...
    版本:
      2021-11-01 : 在更新 SCMDATA.T_ASNORDERSITEM 时使用本过程同时更新 scmdata.t_qa_scope_backup，
                   SCMDATA.T_ASNINFO_PRETRANS_TO_WMS 对应的数据和状态
  ...
  PROCEDURE P_SYNC_QA_MISSION IS
    ...
    v_supcode VARCHAR2(32); --zc314_20221219 add
  BEGIN
    FOR I IN (SELECT *
  ...
  PROCEDURE P_ASNORDERED_ITF_REINFO_SYNC IS
    ...
    V_JUGNUM NUMBER(1);
  BEGIN
    FOR I IN (SELECT ASN_ID,COMPANY_ID, SCAN_TIME, CHANGETIMES, PCOME_DATE, PCOME_INTERVAL, order_id, memo
                FROM (SELECT ASN_ID,COMPANY_ID, SCAN_TIME, CHANGETIMES,
                             PCOME_DATE, PCOME_INTERVAL, order_id, memo
                        FROM SCMDATA.T_ASNORDERED_ITF
      ...
      BEGIN
        ...
        ELSE
          ...
          INSERT INTO scmdata.t_asnordered
            (asn_id, company_id, dc_company_id, status, order_id, supplier_code,
             pcome_date, changetimes, scan_time, memo, create_id, create_time,
             pcome_interval)
          VALUES
            (i.asn_id, i.company_id, i.company_id, 'NC', i.order_id, v_supcode,
             i.pcome_date, i.changetimes, i.scan_time, i.memo, 'admin', SYSDATE,
             i.pcome_interval);
          --zc314_20221220 add end

          /* zc314_20221220 comment
          UPDATE SCMDATA.T_ASNORDERED
             SET SCAN_TIME = I.SCAN_TIME,
                 CHANGETIMES = I.CHANGETIMES,
                 PCOME_DATE = I.PCOME_DATE,
  ...
  BEGIN
    FOR I IN (SELECT ASN_ID, COMPANY_ID, GOO_ID, BARCODE, PCOME_AMOUNT,ASNGOT_AMOUNT, GOT_AMOUNT, ORDER_AMOUNT,
      ...
      BEGIN
        ...
        --更新 scmdata.t_qa_scope_backup, scmdata.t_qa_report_backup 数量相关数据
 ...
 /*=================================================================================
  ...
  BEGIN
    ...
    --新增报告
    INSERT INTO scmdata.t_qa_report_backup
  ...
  BEGIN
    ...
    --更新 SCMDATA.T_QA_WMSRESULT_ITF 数据进入 scmdata.t_qa_scope_backup
    FOR M IN (SELECT A1.ASN_ID, B1.GOO_ID, A1.BARCODES,
               ...
               WHERE A1.ASN_ID = V_ASNID
                 ...
                 AND EXISTS (SELECT 1 FROM scmdata.t_qa_scope_backup
  ...
  BEGIN
    FOR I IN (SELECT A.ASN_ID,
               ...
               INNER JOIN SCMDATA.T_COMMODITY_INFO B
                  ON A.GOO_ID = B.RELA_GOO_ID
                 AND A.COMPANY_ID = B.COMPANY_ID
               INNER JOIN scmdata.t_qa_scope_backup C
                  ON A.ASN_ID = C.ASN_ID
      ...
      SELECT MAX(QAREPID)
        ...
        FROM (SELECT FIRST_VALUE(QA_REPORT_ID) OVER(PARTITION BY ASN_ID, GOO_ID, BARCODE, COMPANY_ID ORDER BY SKUCOMFIRM_TIME DESC) QAREPID
                FROM scmdata.t_qa_scope_backup TMPSCP
               WHERE ASN_ID = I.ASN_ID
                 AND GOO_ID = I.GOO_ID
  ...
  BEGIN
    ...
    IF V_STR IS NOT NULL THEN
      V_EXESQL := 'UPDATE scmdata.t_qa_report_backup T SET T.ORIGIN = '''||V_FINORIGIN||
                  ''''||V_STR||' WHERE T.QA_REPORT_ID = '''||V_QAREPID||''' AND T.COMPANY_ID = '''||V_COMPID||'''';
    ELSE
      V_EXESQL := 'UPDATE scmdata.t_qa_report_backup SET ORIGIN = '''||V_FINORIGIN||
  ...
  BEGIN
    ...
    V_EXESQL := 'SELECT COUNT(1) FROM scmdata.t_qa_scope_backup Z '||
        ...
        FROM (SELECT ASN_ID,
                ...
                FROM (SELECT ASN_ID,
                        ...
                        FROM (SELECT ASN_ID,
                               ...
                               WHERE ASN_ID = I.ASN_ID
                                 AND COMPANY_ID = I.COMPANY_ID
                                 AND NOT EXISTS (SELECT 1 FROM scmdata.t_qa_report_backup
                                                  WHERE QA_REPORT_ID = TMPSCP.QA_REPORT_ID
                                                    AND COMPANY_ID = TMPSCP.COMPANY_ID
  ...
  BEGIN
    FOR I IN (SELECT ASN_ID,COMPANY_ID,QA_REPORT_ID
      ...
      SELECT JSON_ARRAYAGG(JSON_OBJECT(GOO_ID,
        ...
        FROM (SELECT X.RELA_GOO_ID GOO_ID,
                ...
                FROM (SELECT QA_SCOPE_ID,
                             ...
                             COMPANY_ID,
                             SKUCOMFIRM_RESULT
                        FROM scmdata.t_qa_scope_backup TSCP
                       WHERE QA_REPORT_ID = I.QA_REPORT_ID
                         AND ASN_ID = I.ASN_ID
                         AND COMPANY_ID = I.COMPANY_ID
                         AND ORIGIN = 'SCM'
      ...
      dbms_output.put_line('{ "portBody" : [' || LTRIM(V_RETCLOB, ',') || ']}'); 
