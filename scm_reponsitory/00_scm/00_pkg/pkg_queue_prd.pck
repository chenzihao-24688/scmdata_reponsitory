CREATE OR REPLACE PACKAGE SCMDATA.pkg_queue_prd IS
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_queue_prd IS
  ...
  BEGIN
    ...
    IF v_jugnum > 0 THEN
      scmdata.pkg_queue_prd.p_queue_executor_core(v_queueexeid => v_queueexeid,
                                                  v_compid     => v_compid);
    END IF;
  ...
  BEGIN
    ...
    v_status := scmdata.pkg_queue_prd.f_get_basicqueue_executor_status;
    ...
    IF v_status = 'P' THEN
      ...
      scmdata.pkg_queue_prd.p_exe_basic_queue_one_by_one(v_id       => v_tmpid,
                                                         v_logic    => v_tmplogic,
                                                         v_qestatus => v_status);
