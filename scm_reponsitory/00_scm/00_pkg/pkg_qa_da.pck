CREATE OR REPLACE PACKAGE SCMDATA.pkg_qa_da IS
  ...
  ==============================================================================*/
  FUNCTION f_get_shoid(v_inp_qarepid IN VARCHAR2,
                       v_inp_compid  IN VARCHAR2) RETURN VARCHAR2;
  ...
  /*=============================================================================
     ...
     版本:
       2023-02-03_ZC314 : 根据 Asn单号获取预回传类型
  
  ==============================================================================*/
  FUNCTION f_qapretrans_get_transtype_by_asn(v_inp_asnid  IN VARCHAR2,
                                             v_inp_compid IN VARCHAR2)
    RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY SCMDATA.pkg_qa_da IS
  ...
  FUNCTION f_is_asnpackinfo_complete(v_inp_asnid  IN VARCHAR2,
    ...
    v_spcomeamount     NUMBER(8);
    v_itmpcomeamount   NUMBER(8);
    v_pcomeamount      NUMBER(8);
  ...
  BEGIN
    ...
    ELSE
      ...
      SELECT nvl(SUM(asns.pcome_amount), 0)
        INTO v_spcomeamount
        FROM scmdata.t_asnorders asns
       WHERE asns.asn_id = v_inp_asnid
         AND asns.company_id = v_inp_compid;
    
      SELECT nvl(SUM(itmitf.pcome_amount), 0), MAX(itmitf.barcode)
        INTO v_itmpcomeamount, v_barcode
        FROM scmdata.t_asnordersitem itmitf
       WHERE itmitf.asn_id = v_inp_asnid
         AND itmitf.company_id = v_inp_compid;
    
      IF nvl(v_itmpcomeamount, 0) = 0 THEN
        v_pcomeamount := v_spcomeamount;
      ELSE
        v_pcomeamount := v_itmpcomeamount;
      END IF;
    
      IF instr(v_barcode, '*') > 0 THEN
        v_asnpackamount := trunc(v_asnpackamount / 6);
      END IF;
  ...
  BEGIN
    ...
    IF v_jugnum = 0 THEN
      ...
      --判断接口数据是否同步正确
      IF v_jugnum = 0
         AND nvl(v_inp_status, ' ') <> 'CE'
         AND
         instr(nvl(v_inp_errinfo, ' '), 'Asn/Asn sku接口数据存在部分不成功') = 0 THEN
        --不正确，状态赋值
        v_inp_status  := 'CE';
      ...
      ELSE
        --获取asn状态
        v_status := f_get_asnstatus(v_inp_asnid  => v_inp_asnid,
                                    v_inp_compid => v_inp_compid);
      
        --免检判断
        IF v_status = 'AE' THEN
          SELECT sign(asns_flag + asnitm_flag)
            INTO v_jugnum
            FROM (SELECT MAX(asns_flag) asns_flag,
                         MAX(asnitm_flag) asnitm_flag
                    FROM (SELECT asns.asn_id,
                                 asns.company_id,
                                 CASE
                                   WHEN asns.receive_time IS NOT NULL THEN
                                    1
                                   ELSE
                                    0
                                 END AS asns_flag,
                                 CASE
                                   WHEN asnitm.receive_time IS NOT NULL THEN
                                    1
                                   ELSE
                                    0
                                 END AS asnitm_flag
        ...
        ELSE
          ...
          --状态，错误信息赋值
          IF v_jugnum <> 0
             AND instr(nvl(v_inp_errinfo, ' '), 'Asn/Asn sku未完成收货') = 0 THEN
            v_inp_status  := 'CE';
            v_inp_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_inp_errinfo,
                                                          v_appendstr  => 'Asn/Asn sku未完成收货',
                                                          v_middliestr => chr(10));
    ...
    IF v_jugnum = 0
       AND instr(nvl(v_inp_errinfo, ' '), 'Asnorderpacks未能同步完整') = 0 THEN
      v_inp_status  := 'CE';
      v_inp_errinfo := scmdata.f_sentence_append_rc(v_sentence   => v_inp_errinfo,
  ...
  BEGIN
    SELECT SUM(CASE
                 WHEN skucheck_result = 'NP'
                      AND skuprocessing_result = 'AWR' THEN
                  1
                 ELSE
                  0
               END) cnt
      ...
      FROM (SELECT asn_id,
                   ...
                   goo_id,
                   barcode,
                   first_value(skucheck_result) over(PARTITION BY asn_id, company_id, goo_id, barcode ORDER BY review_time DESC) skucheck_result,
                   first_value(skuprocessing_result) over(PARTITION BY asn_id, company_id, goo_id, barcode ORDER BY review_time DESC) skuprocessing_result
  ...
  BEGIN
    ...
    SELECT COUNT(DISTINCT qa_report_id)
      INTO v_repcnt
      FROM scmdata.t_qa_report_skudim
     WHERE asn_id = v_inp_asnid
       AND company_id = v_inp_compid;
    ...
    SELECT COUNT(DISTINCT barcode),
      ...
      INTO v_repbccnt, v_awrcnt, v_ftcnt
      FROM (SELECT barcode,
                   CASE
                     WHEN nvl(skuprocessing_result, ' ') = 'AWR' THEN
                      1
                     ELSE
                      0
                   END awrcnt,
              ...
              FROM (SELECT asn_id,
                           goo_id,
                           barcode,
                           first_value(skuprocessing_result) over(PARTITION BY asn_id, goo_id, barcode ORDER BY nvl(qualfinish_time, commit_time) DESC) skuprocessing_result,
                           first_value(nvl(qualfinish_time, DATE '2000-01-01')) over(PARTITION BY asn_id, goo_id, barcode ORDER BY nvl(qualfinish_time, commit_time) DESC) qualfinish_time
                      FROM (SELECT sku.asn_id,
                                   sku.goo_id,
                                   sku.barcode,
                                   sku.skuprocessing_result,
                                   sku.qualfinish_time,
                                   rep.commit_time
                              FROM scmdata.t_qa_report rep
                             INNER JOIN scmdata.t_qa_report_skudim sku
                                ON rep.qa_report_id = sku.qa_report_id
                               AND rep.company_id = sku.company_id
    ...
    IF v_asnstatus = 'AE' THEN
      IF v_wmsins > 0
         AND v_repbccnt = v_wmsins THEN
        v_transtype := 'DEL';
      ELSIF v_wmsins > 0
            AND v_repbccnt <> v_wmsins THEN
        v_transtype := 'SKU';
      ELSIF v_wmsins = 0 THEN
        v_transtype := 'AE';
      END IF;
    ELSIF v_asnstatus IN ('PEX', 'FA') THEN
      IF v_wmsins = 0
         AND v_repbccnt <> v_awrcnt
         AND v_repbccnt > 0
         AND v_awrcnt = 0
         AND v_ftcnt = 1 THEN
        v_transtype := 'ASN';
      ELSIF (v_repbccnt > 0 AND v_repbccnt = v_awrcnt)
            OR (v_repbccnt > 0 AND v_repbccnt = v_wmsins) THEN
        v_transtype := 'DEL';
      ELSE
        v_transtype := 'SKU';
      END IF;
    ...
    RETURN nvl(v_transtype, 'DEL');
  END f_qapretrans_get_transtype_by_asn;
