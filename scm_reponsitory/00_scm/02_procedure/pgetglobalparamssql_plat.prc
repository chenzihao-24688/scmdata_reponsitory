create or replace procedure scmdata.PGetGlobalParamsSQL_plat(p_userid       in varchar2,
                                                     p_platform_seq in varchar2,
                                                     p_sql          out varchar2) is
  /* lValue varchar(2000);
...
begin
  select max(user_account), max(user_type), max(user_id)
    ...
    from sys_user
   where upper(user_account) = upper(p_userid);
  if lValue is not null then
    p_sql := 'select ''user_account'' name,''账号'' caption,''' || lValue ||
             ''' value from dual' || chr(13) || chr(10);
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''user_type'',''用户类型'',''' || lValue1 ||
             ''' from dual' || chr(13) || chr(10);
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''userid'',''' || lValue || ''',''' || lValue ||
             '''from dual' || chr(13) || chr(10);
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''AuthorID'',''' || p_userid || ''',''' || p_userid ||
             '''from dual' || chr(13) || chr(10);
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''user_id'',''' || luser_id || ''',''' || luser_id ||
             '''from dual' || chr(13) || chr(10);
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''Current_UserID'',''' || luser_id || ''',''' ||
             luser_id || '''from dual' || chr(13) || chr(10);
    select max(a.company_id),
           max(b.logn_name),
           max(b.company_name),
           max(b.company_role)
      into lValue, lValue1, lValue2, lValue3
      from sys_user_company a
     inner join sys_company b
        on a.company_id = b.company_id
     where user_id = luser_id
       and a.company_id = p_platform_seq
       and a.pause = 0
       and b.pause = 0;
    if lValue is not null then
      ...
      select max(a.inside_supplier_code)
       ...
       inner join sys_company b
          on a.social_credit_code = b.licence_num
       where a.company_id = 'b6cc680ad0f599cde0531164a8c0337f' --固定三福企业下供应商
         and b.company_id = lValue;
      if lSup_ID_Base is not null then
        p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
                 'select ''sup_id_base'',''内部供应商ID'',''' || lSup_ID_Base ||
                 ''' from dual' || chr(13) || chr(10);
      else
        p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
                 'select ''sup_id_base'',''内部供应商ID'',null from dual' ||
                 chr(13) || chr(10);
      end if;
    
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''company_id'',''企业ID'',''' || lValue ||
               ''' from dual' || chr(13) || chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''logn_name'',''企业全称'',''' || lValue1 ||
               ''' from dual' || chr(13) || chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''company_name'',''企业简称'',''' || lValue2 ||
               ''' from dual' || chr(13) || chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''default_company_id'',''默认企业ID'',''' || lValue ||
               ''' from dual' || chr(13) || chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''default_company_role'',''默认企业角色ID'',''' || lValue3 ||
               ''' from dual' || chr(13) || chr(10);
    else
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''company_id'',''企业ID'',null from dual' || chr(13) ||
               chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''logn_name'',''企业全称'',null from dual' || chr(13) ||
               chr(10);
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''company_name'',''企业简称'',null from dual' || chr(13) ||
               chr(10);
    end if;
  
    select max(company_dept_id)
      into lValue1
      from sys_company_user_dept
     where user_id = luser_id
       and company_id = lValue;
    if lValue1 is not null then
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''default_dept_id'',''默认部门ID'',''' || lValue1 ||
               ''' from dual' || chr(13) || chr(10);
      --czh add合作分类数据权限
      --begin
      ...
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''coop_class_priv'',''合作分类数据权限'',''' || lValue3 ||
               ''' from dual' || chr(13) || chr(10);
      --end
      --czh add 新通用数据权限配置 
      --begin
      v_dflag := scmdata.pkg_data_privs.check_is_data_privs(p_company_id => lvalue,
                                                            ...
                                                            v_type       => 0);
      IF v_dflag > 0 THEN
        data_privs_json_strs := scmdata.pkg_data_privs.get_json_strs(p_company_id => lvalue,
                                                                     p_user_id    => luser_id);
        p_sql                := p_sql || 'union all' || chr(13) || chr(10) ||
                                'select ''data_privs_json_strs'',''数据权限JSON'',''' ||
                                data_privs_json_strs || ''' from dual' ||
                                chr(13) || chr(10);
      END IF;
      --end
      --czh add 原通用数据权限配置 
      ...
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''default_dept_id'',''默认部门ID'',null from dual' ||
               chr(13) || chr(10);
    end if;
    ...
    */
    ...
    if linner_user_id is not null then
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''inner_user_id'',''内部员工号'',''' || linner_user_id ||
               ''' from dual' || chr(13) || chr(10);
    else
      p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
               'select ''inner_user_id'',''内部员工号'',null from dual' ||
               chr(13) || chr(10);
    end if;
    ...
    SELECT nvl(max(1), 0)
     ...
     where b.company_id = lValue
       ...
       AND c.company_role_name in ('超级管理员', '管理员');
    p_sql := p_sql || 'union all' || chr(13) || chr(10) ||
             'select ''is_company_admin'',''是否企业管理员'',''' || vFlag ||
             ''' from dual' || chr(13) || chr(10);
  end if;
  ...
  lValue1 := '运维权限';
  p_sql   := p_sql || 'union all' || chr(13) || chr(10) || 'select ''' ||
             lValue || ''',''' || lValue1 || ''', ''' || vFlag ||
             ''' from dual' || chr(13) || chr(10);
  --end
