create or replace procedure scmdata.P_T_KPIORDER_QU_YEAR(t_type number,p_type number) IS
   V_Q_SQL clob;
   V_U_SQL clob;
   V_W_SQL clob;
   V_W1_SQL clob;
...
BEGIN
  ...
  USING (
    ...
    select k.company_id, k.year, k.group_name, k.order_money,z.sho_order_money,d.sho_order_money_desc, y.delivery_money,
      ...
      left join (select t2.company_id, t2.year, t2.group_name, sum(t2.order_money) delivery_money
                   from scmdata.pt_ordered t2
                  where t2.is_first_order = '0'
                  group by t2.company_id, t2.year,t2.group_name) y
      ...
      left join (select tp.company_id, tp.year, tp.group_name,sum(tp.sum_money) sho_order_money
                  from (select t3.company_id, t3.year, t3.group_name,(t3.fixed_price * ta.delivery_amount) sum_money
                        ...
                        union all
                        select t3a.company_id,t3a.year, t3a.group_name, (t3a.fixed_price * ta4.delivery_amount) sum_money
      ...
      left join (select tp.company_id, tp.year, tp.group_name,sum(tp.sum_money) sho_order_money_desc
                  from (select t3b.company_id, t3b.year, t3b.group_name,(t3b.fixed_price * ta3.delivery_amount) sum_money
                         ...
                         where (to_date(to_char(ta3.delivery_date, 'yyyy/mm/dd'), 'yyyy/mm/dd')) <= (to_date(to_char(t3b.delivery_date, 'yyyy/mm/dd'), 'yyyy/mm/dd'))
                           and t3b.is_twenty = 1
                        union all
                        select t3c.company_id,t3c.year,t3c.group_name, (t3c.fixed_price * ta2.delivery_amount) sum_money
    ...
    left join (select ta0.company_id, ta0.year, ta0.group_name,sum(ta0.sum1_date*ta0.sum1_money) delivery_order_money
              from (select t5.company_id, t5.year, t5.group_name,
    ...
    left join (select t6.company_id, t6.year,t6.group_name,sum(t6.sum2_date*t6.sum2_money) delivery_order_money_desc
                from (select t5a.company_id, t5a.year, t5a.group_name,
      ...
      left join (select t2.company_id, t2.year, t2.group_name, sum(t2.order_money) delivery_20_money
                     from scmdata.pt_ordered t2
                    where t2.is_first_order = '0' and t2.is_twenty = 1 
                    group by t2.company_id, t2.year, t2.group_name) qy
