create or replace procedure scmdata.PTESTGetGlobalParamsSQL(p_userid in varchar2,
...
begin
  ...
  select to_char(nvl(max(1), 0))
    into lValue
    from scmdata.sys_user u
   inner join scmdata.sys_user_company cu
      on cu.user_id = u.user_id
   where upper(u.user_account) = upper(p_userid)
     and cu.company_id = 'b6cc680ad0f599cde0531164a8c0337f'
     and cu.pause = 0;
  --lValue := '1';
  if lValue = '1' then
    lValue1 := 'DetectionFlag';
    p_sql   := p_sql || 'union all' || chr(13) || chr(10) || 'select ''' ||
               lValue1 || ''',''' || lValue1 || ''', ''' || lValue ||
               ''' from dual' || chr(13) || chr(10);
  end if;
  --end
