CREATE OR REPLACE FUNCTION SCMDATA.instr_priv(p_str1  VARCHAR2,
                                      p_str2  VARCHAR2,
                                      p_split VARCHAR2 DEFAULT ';')
  RETURN NUMBER IS
  v_str1  VARCHAR2(4000) := p_str1;
  v_str2  VARCHAR2(4000) := p_str2;
  v_count NUMBER := 0;
BEGIN
  ...
  ELSE
    IF instr(v_str2, p_split) > 0 THEN
      FOR str_rec IN (SELECT regexp_substr(v_str2,
                                           '[^' || p_split || ']+',
                                           1,
                                           LEVEL,
                                           'i') AS str
